...
describe('PerformanceAnalyzer', () => {
  ...
  it('should handle multiple signal files', async () => {
    // 创建多个日期的信号文件 - 使用最近的日期以确保在 30 天范围内
    const today = new Date();
    const dates = [
      new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 2 天前
      new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 1 天前
