...
| D-8 | 迁移脚本用 `.ts` + tsx 运行（而非卡面写的"纯 .mjs 不依赖 tsx"），以 import 运行时同一套 backfill 实现 | 实施前发现卡面自相矛盾（纯 .mjs 无法复用 TS 实现，重写即第二套时间线语义）；tsx 在本仓是既有加载方式，停机照跑。详见 `design/migration.md` §3.1 |
| D-7 | **范围补充（实施期）**：验收单返工续版 v2 必须带过「未过项 + **未裁决项**」；上一版无未过项时仍重新生成全新验收单 | 用户弹框裁定（2026-09-17，选"本轮顺带修"）。触发：t4 落地时发现既有实现只带 failed，pending 项被静默丢弃 → 极端路径"返工后 v2 全过即可归档，而若干项从未被裁决"，是"逐项验收"人工门的实效漏洞。本项**有意变更行为**（超出纯重构），已同步更新断言并加守卫用例（`tests/domain/acceptance-sheet.test.ts`） |
