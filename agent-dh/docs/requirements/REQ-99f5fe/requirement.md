...
| 复核时点 | **2026-09-20 二次复核**：本文档已按当前入库状态（`972b2262`）重写——写作期间 REQ-e3b6a0 的闸门链与文案硬化**已上线**，旧版中"三处硬伤/半接线/进行中"等描述已过期 |
...
| # | 业务行为 | 触发事件 | 现状位置 | 现状备注 |
|---|---------|---------|---------|---------|
| B1 | 立项捕获登记（待立项评估候选） | `user/message`（未绑定窗口） | `CaptureHook.ts` | 判定仍在适配器 |
| B2 | 承接推进（draft → brainstorming） | `user/message`（已绑定窗口） | 判定在 CaptureHook，动作在 `index.ts` 闭包 | 散落 |
| B3 | 阶段纪律提示词注入 + 注入留痕 | `user/message`（绑定 + 节点可注入） | 同上 | 投递通道已由 `AgentDeliverer` 修复（REQ-e3b6a0 t4） |
| B4 | 里程碑超时提醒（30 分钟未确认） | `user/message`（绑定 + 超时） | 同上 | 同上（此前从未投递，已修） |
| B5 | 节点结算 → 上下文隔离 | `turn/end`（本回合登记过结算） | 同上 + `node-settlement.ts` | 散落 |
| B6 | 待捕获清除 | `turn/end` | `CaptureHook.ts` | 判定仍在适配器 |
| B7 | 工具痕迹跟踪（done 凭证门） | `tool/call`（非忽略会话） | `CaptureHook.ts` | 判定仍在适配器 |
| B8 | 近期用户消息缓冲（证据核验） | `user/message`（清洗后非空） | `CaptureHook.ts` | 判定仍在适配器 |
| B9 | 捕获引导段注入 | systemPrompt 组装 | `capture-section.ts` + `index.ts` | 文案已被 e3b6a0 硬化（见 P1 现状） |
| B10 | 闸门确认后置链（推进/压缩/注入/唤醒/审计） | 弹框作答（声明 `gate`） | **REQ-e3b6a0 已上线**（`application/gate/`） | 本需求只对齐核验（RF-5），不重建 |
...
| **判定收敛度** | 业务 hook 判定代码所在文件数 | **2**（`CaptureHook.ts` + `index.ts`） | **1**（单一领域目录） |
...
**现象**：B1–B8 的"判定"在 `adapters/CaptureHook.ts`，"动作"在 `index.ts` 的闭包里；
闸门链上线后，`index.ts` 的 hook 动作面增至 **6 处**（4 个 CaptureHookDeps 闭包 + gateChain 装配块 + onTurnEnd 闭包）。
**证据**：`CaptureHookDeps` 声明 `onBoundWindowActivity` / `onStagePrompt` / `onNodeSettled` / `onTurnEnd`，实现在 `index.ts:269-313`。
**根因**：hook 按"哪里需要就在哪里接一根线"长出来，从未定义登记处。
**影响**：改一条行为要同时改两处；闸门链这种新面还在向组合根继续堆闭包，发散在加速。
...
`adapters/CaptureHook.ts` 退化为"规范化 + 执行"的薄适配器，`index.ts` 不再持有散落闭包。
...
| 版本 | 内容 | 理由 |
|------|------|------|
| **MVP（必须先上）** | RF-1（文案剩余缺口 + E2E 验证）+ RF-2/RF-3（领域聚合与单点出口）+ RF-8（不变式） | 没有 RF-1，硬化后的立项门在"方案追问"场景仍会漏；没有 RF-2/3，P2/P3 类静默故障会反复发生 |
...
- 顺序：RF-1 文案剩余缺口（小步先行，E2E 验证）→ 抽规则表（纯搬迁）→ 加决策留痕 → 迁 B1–B9 →
  闸门链核验（RF-5）→ 门禁固化（RF-4/RF-7）；
