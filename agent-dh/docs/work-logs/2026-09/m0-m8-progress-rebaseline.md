...
**完成状态分布（重新核算）**:
- ✅ 完全就绪：M0（**100%**，09-01 复核修正）、M3（**100%**，09-01 更新）、M4（100%）、M6（100%）、M7（100%）
- 🟡 进行中：M1（85%）、M2（67%）、M5（50%）
- 🔴 未启动：M8（0%）
...
**加权平均**：M0 **100%×5** + M1 85%×3 + M2 67%×3 + M3 **100%×3** + M4 100%×3 + M5 50%×2 + M6 100%×4 + M7 100%×3 + M8 0%×2 = (**500**+255+201+**300**+300+100+400+300+0)/28 = 2356/28 = **84%**
...
| 状态 | 数量 | 明细 |
|------|------|------|
| ✅ 完成 | 19 | M0-1/2/3/**4/5**、M1-1/2/3/自动化、M2-1/2、**M3-2（09-01 完成）**、M4-1/2/3、M6-1/2/3/4、M7-1/2/3（M2-3 记半完成） |
| ❌ 失败/停滞 | 1 | M2-3（后端修好前端缺） |
| ⏳ 未完成 | 6 | M3-3 回填、M5-1/2、M8-1/2 |

**19/26 = 73%**（含 M2-3 半完成按 0.5 记：19.5/26 = 75%；09-01 复核：M0-4/5、M3-2 实际已完成）
...
### ✅ M0-4/M0-5 实际已完成（09-01 复核修正）
- **M0-4 因子新鲜度门禁**：`/api/data/quality-report` 已返回 `factor_freshness`（factor_name/latest_date/coverage/stale_days/stale），阈值 5 个交易日，dac1451a（8/20）已集成——实测 cci/ma60/mfi14 等陈旧因子被正确标记 stale=true
