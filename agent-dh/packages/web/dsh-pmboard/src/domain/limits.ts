...
export const LIMITS = {
  ...
  /**
   * 交互式确认的**内联宽限窗口**（毫秒；REQ-260924213231-b1c4 FR-3）：`questions.ask`
   * 与本计时器**赛跑**——宽限内作答 = 旧阻塞语义逐字一致；超宽限 = 登记挂起 ticket、
   * 立即返回 `pending=true`（**不判失败**），后台继续等作答并落章。
   *
   * 取值纪律：必须**显著小于**调用方 run_code 的 120s 执行预算——否则「等不到人就先撞
   * deadline」的老事故（REQ-2cd3）原样复现。可调不可关。
   */
  confirmInlineGraceMs: 30_000,
  /**
