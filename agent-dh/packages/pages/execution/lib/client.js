window.__ModuleLoader__.load({
		...
		`,document.head.appendChild(e)}function Te(){let e=`dsh-exec-styles`;if(document.getElementById(e)!==null)return;let t=document.createElement(`style`);t.id=e,t.textContent=`
		.dsh-exec-entry {
		...
		`,(document.head??document.documentElement).appendChild(t)}const Ee=[`slots`,`sessions`,`workspaces`];function De(e){try{window.__dshExecCtx=e,window.__dshExecSessions=e?.sessions,window.__dshExecWorkspaces=e?.workspaces}catch{}try{we(),Te(),window.__dshExecClient?.dispose();let t=xe(),n=Se(t),r=e=>{let n=e.detail?.open;console.log(`[dashboard-execution] open-board event`,{open:n},`boardOpen:`,t.getSnapshot().boardOpen),n===!0?t.getSnapshot().boardOpen?t.closeBoard():t.openBoard():t.toggleBoard()};window.addEventListener(Q,r),window.__dshExecClient={dispose:()=>{window.removeEventListener(Q,r),n(),t.closeBoard()}};let i=e.slots;i?i.inject(`sidebar.footer.action`,()=>i.register({name:`sidebar.footer.action`,id:`dashboard-execution`,order:100,label:X},Ce)):console.warn(`[dashboard-execution] ctx.slots unavailable (inject missing "slots")`)}catch(e){console.error(`[dashboard-execution] client half failed to start:`,e)}}exports.apply=De,exports.inject=Ee,exports.name=`@pi-investment/dashboard-execution/client`;
