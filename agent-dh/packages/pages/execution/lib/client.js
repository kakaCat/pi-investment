window.__ModuleLoader__.load({
		...
		`,(document.head??document.documentElement).appendChild(t)}const ne=[`slots`];function re(e){try{ee(),te(),window.__dshExecClient?.dispose();let t=q(),n=J(t),r=e=>{let n=e.detail?.open;console.log(`[dashboard-execution] open-board event`,{open:n},`boardOpen:`,t.getSnapshot().boardOpen),n===!0?t.getSnapshot().boardOpen?t.closeBoard():t.openBoard():t.toggleBoard()};window.addEventListener(Z,r),window.__dshExecClient={dispose:()=>{window.removeEventListener(Z,r),n(),t.closeBoard()}};let i=e.slots;i?i.inject(`sidebar.footer.action`,()=>i.register({name:`sidebar.footer.action`,id:`dashboard-execution`,order:100,label:Y},Q)):console.warn(`[dashboard-execution] ctx.slots unavailable (inject missing "slots")`)}catch(e){console.error(`[dashboard-execution] client half failed to start:`,e)}}exports.apply=re,exports.inject=ne,exports.name=`@pi-investment/dashboard-execution/client`;
