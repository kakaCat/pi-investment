...
`,document.head.appendChild(e)}function d(t){let{wide:n}=t,i=r;return(0,e.createElement)(`button`,{type:`button`,className:n?`dsh-reqboard-foot wide`:`dsh-reqboard-foot rail`,title:i,"aria-label":i,onClick:()=>{window.dispatchEvent(new CustomEvent(c,{detail:{open:!0}}))}},n?[(0,e.createElement)(`span`,{className:`dsh-reqboard-foot-icon`,key:`i`},l),(0,e.createElement)(`span`,{className:`dsh-reqboard-foot-label`,key:`l`},i)]:(0,e.createElement)(`span`,{className:`dsh-reqboard-foot-icon`,key:`i`},l))}const f=`@pi-investment/dashboard-requirement/styles.css`;function p(){if(typeof document>`u`||document.querySelector(`style[data-plugin-css="${f}"]`))return;let e=document.createElement(`style`);e.dataset.pluginCss=f,e.textContent=`
...
`,document.head.appendChild(e)}const m=[`slots`];function h(e){try{u(),p(),window.__dshReqboardClient?.dispose();let t=a(),i=o(t),s=e=>{e.detail?.open===!0?t.getSnapshot().boardOpen?t.closeBoard():t.openBoard():t.toggleBoard()};window.addEventListener(c,s),window.__dshReqboardClient={dispose:()=>{window.removeEventListener(c,s),i(),t.closeBoard()}};let l=e.slots;l?l.inject(`sidebar.footer.action`,()=>l.register({name:`sidebar.footer.action`,id:n,order:110,label:r},d)):console.warn(`[dashboard-requirement] ctx.slots unavailable`)}catch(e){console.error(`[dashboard-requirement] client half failed to start:`,e)}}exports.apply=h,exports.inject=m,exports.name=`@pi-investment/dashboard-requirement/client`;