...
`,(document.head??document.documentElement).appendChild(t)}const H=[`slots`,`sessions`,`workspaces`];function U(e){try{try{window.__dshHldCtx=e,window.__dshHldSessions=e.sessions,window.__dshHldWorkspaces=e.workspaces}catch{}V(),I(`dsh-hld`),window.__dshHldClient?.dispose();let t=R(),n=z(t),r=B({isActive:()=>t.getSnapshot().boardOpen,toggle:()=>t.toggleBoard()});window.__dshHldClient={dispose:()=>{n(),r(),t.closeBoard()}}}catch(e){console.error(`[dashboard-holdings] client half failed to start:`,e)}}exports.apply=U,exports.inject=H,exports.name=`@pi-investment/dashboard-holdings/client`;