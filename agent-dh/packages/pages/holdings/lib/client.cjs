...
`,(document.head??document.documentElement).appendChild(t)}const $=[`slots`,`sessions`,`workspaces`];function ee(e){try{try{window.__dshHldCtx=e,window.__dshHldSessions=e.sessions,window.__dshHldWorkspaces=e.workspaces}catch{}Q(),q(`dsh-hld`),window.__dshHldClient?.dispose();let t=Y(),n=X(t),r=Z({isActive:()=>t.getSnapshot().boardOpen,toggle:()=>t.toggleBoard()});window.__dshHldClient={dispose:()=>{n(),r(),t.closeBoard()}}}catch(e){console.error(`[dashboard-holdings] client half failed to start:`,e)}}exports.apply=ee,exports.inject=$,exports.name=`@pi-investment/dashboard-holdings/client`;