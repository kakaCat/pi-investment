...
`,(document.head??document.documentElement).appendChild(t)}const z=[];function B(){try{R(),window.__dshHldClient?.dispose();let e=F(),t=I(e),n=L({isActive:()=>e.getSnapshot().boardOpen,toggle:()=>e.toggleBoard()});window.__dshHldClient={dispose:()=>{t(),n(),e.closeBoard()}}}catch(e){console.error(`[dashboard-holdings] client half failed to start:`,e)}}exports.apply=B,exports.inject=z,exports.name=`@pi-investment/dashboard-holdings/client`;