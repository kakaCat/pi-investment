...
</div>`}function w(e,t,n){if(t<=9){let r=``;for(let i=0;i<t;i++)r+=n(i,String(i+1),i===e);return r}let r=[...new Set([0,t-1,e-1,e,e+1].filter(e=>e>=0&&e<t))].sort((e,t)=>e-t),i=``,a=-2;for(let t of r)t-a>1&&(i+=`<span class="gap">…</span>`),i+=n(t,String(t+1),t===e),a=t;return i}function T(e,t){let n=Array.isArray(e.tradeHistory)?e.tradeHistory:[],r=Math.max(1,Math.ceil(n.length/8)),i=Math.min(Math.max(0,Math.trunc(Number(t)||0)),r-1);if(n.length===0)return`<div class="dsh-hld-card">
