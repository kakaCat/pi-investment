Object.defineProperty(exports,Symbol.toStringTag,{value:`Module`});let e=require("react");const t=`[data-dsh-hld-view]`,n=`data-dsh-hld-active`,r=[`data-dsh-atb-active`,`data-dsh-taskboard-active`,`data-dsh-ssh-active`,`data-dsh-exec-active`],i=`dsh-panel-activate`;function a(){return document.querySelector(`[data-pane="conversation"], [class*="centerCol"], .dshDesktopConversationSurface`)??void 0}const o=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]??e),s=(e,t=2)=>(Number.isFinite(Number(e))?Number(e):0).toLocaleString(`zh-CN`,{minimumFractionDigits:t,maximumFractionDigits:t}),c=e=>{let t=Number(e)||0;return(t>0?`+`:``)+t.toLocaleString(`zh-CN`,{maximumFractionDigits:2})},l=e=>{let t=Number(e)||0;return(t>0?`+`:``)+t.toFixed(2)+`%`},u=e=>{let t=Number(e)||0;return t>1e-4?`up`:t<-1e-4?`down`:`flat`},d=e=>{if(!e)return`—`;let t=new Date(e);if(Number.isNaN(t.getTime()))return String(e).slice(11,19);let n=e=>String(e).padStart(2,`0`);return n(t.getMonth()+1)+`-`+n(t.getDate())+` `+n(t.getHours())+`:`+n(t.getMinutes())},f=e=>{let t=e.context;if(typeof t==`string`)return t;try{return JSON.stringify(t??``)}catch{return``}},p={600519:`贵州茅台`,"000858":`五粮液`,"000568":`泸州老窖`,600809:`山西汾酒`,600600:`青岛啤酒`,601288:`农业银行`,601398:`工商银行`,601939:`建设银行`,601988:`中国银行`,600036:`招商银行`,"000001":`平安银行`,6e5:`浦发银行`,601166:`兴业银行`,600016:`民生银行`,601328:`交通银行`,601318:`中国平安`,601601:`中国太保`,601628:`中国人寿`,600030:`中信证券`,601688:`华泰证券`,600900:`长江电力`,601857:`中国石油`,600028:`中国石化`,601088:`中国神华`,600019:`宝钢股份`,600585:`海螺水泥`,601668:`中国建筑`,601390:`中国中铁`,601766:`中国中车`,600104:`上汽集团`,601633:`长城汽车`,601238:`广汽集团`,"000333":`美的集团`,"000651":`格力电器`,600690:`海尔智家`,300750:`宁德时代`,"002594":`比亚迪`,601012:`隆基绿能`,600438:`通威股份`,"002460":`赣锋锂业`,600276:`恒瑞医药`,603259:`药明康德`,"000538":`云南白药`,300760:`迈瑞医疗`,"002415":`海康威视`,"000063":`中兴通讯`,"002230":`科大讯飞`,"002475":`立讯精密`,"002241":`歌尔股份`,688981:`中芯国际`,688111:`金山办公`,603986:`兆易创新`,"002049":`紫光国微`,300782:`卓胜微`,"002371":`北方华创`,688012:`中微公司`,"002463":`沪电股份`,"002815":`崇达技术`,"002050":`三花智控`,"000807":`云铝股份`,601138:`工业富联`,"002352":`顺丰控股`,601888:`中国中免`,"000725":`京东方A`,"002714":`牧原股份`,300498:`温氏股份`,601111:`中国国航`,600029:`南方航空`,600150:`中国船舶`,601989:`中国重工`,600893:`航发动力`,"002179":`中航光电`,300059:`东方财富`,600031:`三一重工`};function m(e){let t={};for(let n of e){let e=f(n),r=/([\u4e00-\u9fa5]{2,10})\s*\(?0*(\d{6})\)?/g,i;for(;(i=r.exec(e))!==null;)t[i[2]]=i[1]}return t}function h(e,t){if(!e)return`—`;let n=String(e).replace(/\D/g,``).slice(-6);return p[n]??t[n]??n}const g=e=>String(e??``).replace(/\D/g,``);function _(e){return/^(30|68)/.test(e)?-.1:-.08}function v(e,t=`current`,n=0){let r=e.summary??{},i=Array.isArray(e.accounts)?e.accounts:[],a=e.currentAccount??``,s=Array.isArray(e.positions)?e.positions:[],c=Array.isArray(e.watchRules)?e.watchRules:[],l=m(c),u=c.filter(e=>{let t=e.account;return t==null||t===``||t===a}),f=i.length>1?`<div class="dsh-hld-acct">
       ...
       </div>`:``,p=r.lastUpdated?d(r.lastUpdated):`—`,h=r.totalValue?r.dailyChange/(r.totalValue-r.dailyChange)*100:0;return`<div class="dsh-hld-board">
  ...
  <div class="tblwrap"><table>
    ...
    ${a}
...
</div>`}function C(e,t,n){if(t<=9){let r=``;for(let i=0;i<t;i++)r+=n(i,String(i+1),i===e);return r}let r=[...new Set([0,t-1,e-1,e,e+1].filter(e=>e>=0&&e<t))].sort((e,t)=>e-t),i=``,a=-2;for(let t of r)t-a>1&&(i+=`<span class="gap">…</span>`),i+=n(t,String(t+1),t===e),a=t;return i}function w(e,t){let n=Array.isArray(e.tradeHistory)?e.tradeHistory:[],r=Math.max(1,Math.ceil(n.length/8)),i=Math.min(Math.max(0,Math.trunc(Number(t)||0)),r-1);if(n.length===0)return`<div class="dsh-hld-card">
      ...
      <div class="dsh-hld-emptybox">该账户暂无历史交易记录</div>
        ...
        <span class="dsh-hld-pg-nums">${C(i,r,(e,t,n)=>`<button type="button" class="dsh-hld-pgb${n?` act`:``}"${e===i?` aria-current="page"`:``} onclick="window.__dshHldHistoryPage && window.__dshHldHistoryPage(${e})">${t}</button>`)}</span>
      ...
      </div>`;return`<div class="dsh-hld-card">
  ...
  ${v}
</div>`}function T(e,t,n,r,i){let a=e=>e.account??``,s=e,c=s.filter(e=>a(e)===``),l=s.filter(e=>a(e)===n).concat(c),u=[];for(let e of s){let t=a(e);t!==``&&t!==n&&!u.includes(t)&&u.push(t)}let d=new Map;for(let e of r)d.set(e.account_name,e.display_name||e.account_name);let p=e=>d.get(e)??e,m=new Set([`current`,`all`,...u]).has(i)?i:`current`,_=m===`all`?s:m===`current`?l:s.filter(e=>a(e)===m),v=(e,t,n,r)=>`<button type="button" class="dsh-hld-wtab${m===e?` act`:``}" data-wkey="${e}" title="${o(r)}" onclick="window.__dshHldWatchTab && window.__dshHldWatchTab('${e.replace(/'/g,``)}')">${o(t)}<i class="c">${n}</i></button>`,y=[v(`current`,`本账户`,l.length,`当前账户归属 + 通用观察（${p(n)}）`)];for(let e of u)y.push(v(e,p(e),s.filter(t=>a(t)===e).length,`归属账户：${e}`));y.push(v(`all`,`全部`,s.length,`全部账户规则汇总`));let b=_.map(e=>{let r=f(e),i=r.replace(/\s+/g,` `).trim(),s=(e.conditions??[]).map(E).filter(Boolean),c=s.slice(0,3).join(` <span class="dim">·</span> `)+(s.length>3?` <span class="dim">+`+(s.length-3)+`</span>`:``),l=D(r),u=a(e),d=u===``?`off`:u===n?`on`:`oth`,m=u===``?`通用观察`:u===n?`本账户`:p(u);return`<tr>
    ...
    </tr>`}).join(``),x=m===`current`?`本账户暂无归属/通用观察规则 — 开仓后 agent 会自动挂上止损/止盈盯盘；可切换上方其他账户 / 全部 tab`:m===`all`?`暂无任何盯盘规则`:`该账户暂无归属盯盘规则`,S=_.length===0?`<tr class="dsh-hld-empty"><td colspan="6">`+x+`</td></tr>`:``;return`<div class="dsh-hld-card">
  <div class="hd"><span class="t">盯盘中心（${s.length}）</span><span class="more">账户归属 tab · 默认本账户（含通用观察）· 触发后由 agent 决策，无需人工盯盘</span></div>
  <div class="dsh-hld-wtabs">${y.join(``)}</div>
  <div class="tblwrap"><table>
    <tr><th>股票</th><th>监控性质</th><th>触发条件</th><th>状态</th><th>归属账户</th><th>监控摘要</th></tr>
    ${b}${S}
  </table></div>
</div>`}function E(e){let t=e.params;if(e.type===`price_break`||String(e.operator||``).toLowerCase().includes(`price`)){if(t&&t.price!==void 0&&t.price!==null){let e=t.direction===`above`?`突破`:t.direction===`below`?`跌破`:`触碰`;return`<span class="cond-${t.direction===`above`?`up`:`down`}">${e}${s(t.price)}</span>`}if(e.threshold!==void 0&&e.threshold!==null)return`价格 `+e.threshold}return e.threshold!==void 0&&e.threshold!==null&&e.operator?String(e.operator||e.type).toUpperCase()+` `+e.threshold:String(e.type??e.operator??`条件`)}function D(e){return/止损|风控|破位|减仓保护/.test(e)?{cls:`warn`,text:`止损监控`}:/买入|低吸|介入|加仓|建仓|补仓/.test(e)?{cls:`buy`,text:`买入提醒`}:/卖出|止盈|减仓|高抛|目标价/.test(e)?{cls:`sell`,text:`卖出提醒`}:{cls:`on`,text:`常规监控`}}function O(){let e=!1,a=`agent_virtual`,o=`current`,s=0,c,l,u=()=>{if(!e){e=!0,console.log(`[dashboard-holdings] opening board`),document.documentElement.setAttribute(n,``);for(let e of r)document.documentElement.removeAttribute(e);window.dispatchEvent(new CustomEvent(i,{detail:`dashboard-holdings`})),h(),_(a)}},d=()=>{e&&(e=!1,console.log(`[dashboard-holdings] closing board`),document.documentElement.removeAttribute(n),g())},f=()=>{e?d():u()},p=()=>{console.log(`[dashboard-holdings] manual refresh`),_(a)},m=e=>{console.log(`[dashboard-holdings] switching account to`,e),a=e,o=`current`,s=0,_(e)},h=()=>{g(),l=window.setInterval(()=>{e&&_(a)},15e3)},g=()=>{l!==void 0&&(clearInterval(l),l=void 0)},_=async e=>{try{let t=`/dashboard/api/holdings?account=${encodeURIComponent(e)}`,n=await(await fetch(t)).json();if(!n.success)throw Error(n.error||`Unknown error`);let r=n.data;y(r)}catch(e){console.error(`[dashboard-holdings] fetch failed:`,e),b(String(e))}},y=e=>{c=e;let n=document.querySelector(t);n&&(n.innerHTML=v(e,o,s))},b=e=>{let n=document.querySelector(t);n&&(n.innerHTML=`
...
`,document.head.appendChild(e)}function I(){let e=`dsh-hld-styles`;if(document.getElementById(e)!==null)return;let t=document.createElement(`style`);t.id=e,t.textContent=`
.dsh-hld-entry {
...
`,(document.head??document.documentElement).appendChild(t)}const L=[`slots`];function R(e){try{F(),I(),window.__dshHldClient?.dispose();let t=O(),n=k(t),r=e=>{let n=e.detail?.open;console.log(`[dashboard-holdings] open-board event`,{open:n},`boardOpen:`,t.getSnapshot().boardOpen),n===!0?t.getSnapshot().boardOpen?t.closeBoard():t.openBoard():t.toggleBoard()};window.addEventListener(M,r),window.__dshHldClient={dispose:()=>{window.removeEventListener(M,r),n(),t.closeBoard()}};let i=e.slots;i?i.inject(`sidebar.footer.action`,()=>i.register({name:`sidebar.footer.action`,id:`dashboard-holdings`,order:200,label:A},N)):console.warn(`[dashboard-holdings] ctx.slots unavailable (inject missing "slots")`)}catch(e){console.error(`[dashboard-holdings] client half failed to start:`,e)}}exports.apply=R,exports.inject=L,exports.name=`@pi-investment/dashboard-holdings/client`;