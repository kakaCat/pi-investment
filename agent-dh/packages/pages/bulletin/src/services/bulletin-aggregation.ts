// 公告板数据聚合服务
// 职责：从 Agent OS 公告板独立存储（RFC 014，/api/v1/board/posts，与 board_* 工具同源）
// 拉取帖子，按 board_read 口径归一 + 过滤 + 计数 + 分页，产出 BulletinData 信封。
...
export class BulletinAggregationService {
  ...
  /** 拉取帖子全集：独立存储精确查询（RFC 014，不再走 memory 语义搜索），counts/活跃/终态同快照 */
  private async fetchBoardMemories(baseURL: string, timeoutMs: number): Promise<any[]> {
    const url = baseURL + '/api/v1/board/posts?status=all&limit=200'
    ...
    const posts = json?.posts ?? json?.data?.posts ?? []
    return Array.isArray(posts) ? posts : []
  }

  /** 归一为 Post（RFC 014 扁平字段，派生规则与 board_read 逐一对应） */
  private normalize(m: any): Post {
    ...
    return {
      ...
      status: (BULLETIN_STATUS_SET.has(md.status) ? md.status : 'open') as BoardStatus,
