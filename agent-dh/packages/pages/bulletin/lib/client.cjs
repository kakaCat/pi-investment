...
`,(document.head??document.documentElement).appendChild(t)}const T=[`sessions`,`workspaces`];function E(e){try{window.__dshBbdCtx=e,window.__dshBbdSessions=e?.sessions,window.__dshBbdWorkspaces=e?.workspaces}catch{}try{w(),window.__dshBbdClient?.dispose?.();let e=b(),t=S(e),n=C({isActive:()=>e.getSnapshot().boardOpen,toggle:()=>e.toggleBoard()});window.__dshBbdClient={dispose:()=>{try{t()}catch{}try{n()}catch{}try{e.closeBoard()}catch{}}},console.info(`[dashboard-bulletin] phase2 board client ready`)}catch(e){console.error(`[dashboard-bulletin] client half failed to start:`,e)}}exports.apply=E,exports.inject=T,exports.name=`@pi-investment/dashboard-bulletin/client`;