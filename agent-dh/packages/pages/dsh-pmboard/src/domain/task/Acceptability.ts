...
export function checkAcceptance(key: string, acceptance: string): AcceptanceVerdict {
  if (acceptance.length === 0) {
    return { ok: false, code: 'invalid_input', reason: '计划任务 ' + key + ' 缺验收标准（acceptance）——"怎么算做完"必须可验证（跑什么命令、看什么输出/路径）' }
  }
  if (VACUOUS_ACCEPTANCE.test(acceptance)) {
    return { ok: false, code: 'invalid_input', reason: '计划任务 ' + key + ' 的验收标准是空话（"' + acceptance + '"）——必须可证伪：写清跑什么命令、看到什么算过（如"npx vitest run 全绿"、"详情页含 8 个进度点"）' }
  }
  if (!VERIFIABLE_ANCHOR.test(acceptance)) {
    return { ok: false, code: 'invalid_input', reason: '计划任务 ' + key + ' 的验收标准缺少可验证锚点（"' + acceptance + '"）——至少含一项：文件路径（.ts/.md/…）、命令（npx/vitest/curl/…）或断言（通过/拒绝/可见/包含/返回/一致/不变…）' }
  }
  return { ok: true }
}
...
export function checkPlanTaskReferences(tasks: readonly PlanTaskRef[]): AcceptanceVerdict {
  ...
  for (const t of tasks) {
    for (const dep of t.dependsOn) {
      if (dep === t.key) {
        return { ok: false, code: 'invalid_input', reason: '计划任务 ' + t.key + ' 不能依赖自身' }
      }
      if (!keys.has(dep)) {
        return { ok: false, code: 'invalid_input', reason: '计划任务 ' + t.key + ' 依赖了计划中不存在的 key：' + dep }
      }
      ...
      if ((keyIndex.get(dep) ?? -1) > (keyIndex.get(t.key) ?? -1)) {
        return { ok: false, code: 'invalid_input', reason: '计划任务 ' + t.key + ' 依赖了后定义的 key：' + dep + '（前向引用）——decompose 按任务表顺序解析依赖，请把被依赖任务排在前面' }
      }
    }
  }
  return { ok: true }
