/**
 * CaptureRequirement 用例（REQ-e3b6a0 t8 / FR-7）——立项三问 pm 专有弹框：一次调用一把梭。
 *
 * 为什么不复用 `reqboard_ask_confirm`：立项三问是**表单取值 + 创建**，根本没有产物可落章
 ...
 *   ② 经 `deps.questions.ask(三问)` 弹框（装饰器在此刻登记 G0 的后置链）；
 *   ③ 答案映射：名称自定义优先、缺项回落既有默认并**记进 defaults_used**（不静默猜）；
 *   ④ `createRequirementDirect`：创建即立项（含 draft 入口快照）；
...
function notCreated(
  ...
  return {
    ...
    requirement_id: '',
    status: '',
    answers: mapped?.answers ?? { title: '', category: '', difficulty: '' },
  ...
  try {
    const result = await deps.repo.mutate('requirement-moved', (ledger) => {
      ...
      transitionRequirement(req, 'brainstorming', {
        ...
        reason: '立项三问作答即立项（reqboard_capture 原子推进 draft → brainstorming）',
...
/**
 * 立项三问弹框用例（`reqboard_capture`）：三问作答 → 创建即立项 → 绑定本窗口 → 推进 brainstorming。
...
export async function captureRequirement(deps: UseCaseDeps, args: unknown, exec: any): Promise<unknown> {
  ...
  if (!deps.questions.available()) {
    return notCreated(undefined, {
      fallback: 'board',
      note: '弹框通道不可用（userQuestions 服务缺失）：本次未立项。可稍后重试 reqboard_capture，或请用户直接给出名称/类型/难度后调 reqboard_create 立项。',
    })
  }
  ...
  try {
    ...
    if (code === 'DELEGATED_CALLER' || code === 'CALLER_NOT_LIVE') {
      return notCreated(undefined, {
        fallback: 'board',
        note: '当前调用方无弹框权限（subagent / 非活窗口）：本次未立项。请顶层窗口在直接人工回合重试，或由用户直接给出名称/类型/难度后调 reqboard_create。',
      })
    }
    return notCreated(undefined, {
  ...
  if (mapped.title.length === 0) {
    return notCreated(mapped, {
      note: '未取到需求名称（三问答复里名称为空）：本次未立项——名称是唯一没有默认值的问项，不猜不补。可重试 reqboard_capture。',
    })
  }

  // ④ 创建即立项（draft + 入口快照；actor 口径与既有 reqboard_create 一致）
  ...
  return {
    ...
    defaults_used: mapped.defaultsUsed,
    note: fmt(
      '已立项并绑定本窗口：{id}（{category} / {difficulty}）。三问作答即立项，按 brainstorming 阶段纪律继续（无需用户再发消息）。{advanced}',
      {
        id: req.id,
        category: mapped.category,
