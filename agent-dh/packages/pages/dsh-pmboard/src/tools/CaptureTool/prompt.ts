/**
 * CaptureTool 提示词（REQ-e3b6a0 t8 / FR-7）——立项三问 pm 专有弹框的触发纪律。
 *
 * 口径事实源：三问（需求名称 / 需求类型 / 提示词难度）以本工具的 schema 与三处注入文案
...
用本工具一次完成「立项三问 + 创建 + 绑定窗口」——用户作答即立项，不需要再调别的工具补建。
弹框三问（选项顺序即推荐顺序）：
问题一【需求名称】可直接选候选或自定义输入（候选经 title_options 传入，最贴切一项放首位）；
