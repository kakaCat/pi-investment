...
`);const ee=[`draft`,`brainstorming`,`planning`,`decomposing`,`implementing`,`accepting`,`archived`];[...ee];function te(e){let t=e.startsWith(`session-`)?e.slice(8):e;return`w-${(t.split(`-`)[0]??t).slice(0,8)}`}const N=ee.filter(e=>e!==`canceled`),P={feature:{stages:N,confirmGates:Object.keys(M),note:`全流水线，五门全开`},bug:{stages:[`draft`,`planning`,`decomposing`,`implementing`,`accepting`,`archived`],confirmGates:[`planning>decomposing`,`decomposing>implementing`,`accepting>archived`],note:`免需求分析门：业务文档+复现定位即上下文，并入修复方案产物`},refactor:{stages:[`draft`,`planning`,`decomposing`,`implementing`,`accepting`,`archived`],confirmGates:[`planning>decomposing`,`decomposing>implementing`,`accepting>archived`],note:`免需求分析：现状+目标态并入技术设计`},spike:{stages:[`draft`,`implementing`,`accepting`,`archived`],confirmGates:[`accepting>archived`],note:`研究即实施，产物=研究报告`},doc:{stages:[`draft`,`implementing`,`accepting`,`archived`],confirmGates:[`accepting>archived`],note:`写作即实施`},chore:{stages:[`draft`,`implementing`,`accepting`,`archived`],confirmGates:[`accepting>archived`],note:`最简流程`}};function F(e){return P[e]??P.feature}function ne(e,t,n){let r=`${t}>${n}`;if(F(e).confirmGates.includes(r))return M[r]}function I(e,t){let n=e.statusHistory;if(n!==void 0&&n.length>0)return n;let r=[{status:t,at:e.createdAt,by:{kind:`human`},reason:`创建`,inferred:!0}];return e.status!==void 0&&e.status!==t&&e.updatedAt!==void 0&&r.push({status:e.status,at:Math.max(e.updatedAt,e.createdAt),by:e.updatedBy??{kind:`human`},reason:`按 updatedAt 回填（当时无事件留痕）`,inferred:!0}),r}function re(e,t,n,i,a){let o=I(e,i),s=new Map;o.forEach((e,t)=>{s.has(e.status)||s.set(e.status,t)});let c=o.map((e,t)=>({e,i:t})).filter(e=>!t.includes(e.e.status)).map(e=>e.e.status),l=(e,t)=>{if(t===void 0)return`<div class="dsh-pm-tl-row pending" data-status="`+e+`"><span class="dsh-pm-tl-label">`+(n[e]??e)+`</span><span class="dsh-pm-tl-time">—</span><span class="dsh-pm-tl-dur"></span></div>`;let i=o[t],s=o[t+1],c=t===o.length-1,l=(s?.at??a)-i.at,u=c?O(i.status)?``:`已停留 `+D(l):`停留 `+D(l),d=i.by.kind+(i.by.sessionId===void 0?``:` `+m(i.by.sessionId));return`<div class="dsh-pm-tl-row`+(c?` current`:``)+`" data-status="`+r(i.status)+`"><span class="dsh-pm-tl-label">`+(n[e]??e)+`</span><span class="dsh-pm-tl-time">`+r(h(i.at))+`</span><span class="dsh-pm-tl-dur">`+r(u)+`</span><span class="dsh-pm-tl-by">`+r(d)+`</span>`+(i.inferred===!0?`<span class="dsh-pm-tl-inferred" title="历史回填：老记录无事件留痕，由创建时间与评论反推">回填</span>`:``)+`</div>`},u=t.map(e=>l(e,s.get(e))).join(``)+c.map(e=>l(e,s.get(e))).join(``),d=o[0].at,f=o[o.length-1],p=(O(f.status)?f.at:a)-d;return`<div class="dsh-pm-timeline">`+u+`<div class="dsh-pm-tl-total">创建 `+r(h(d))+(O(f.status)?` · 总耗时 `:` · 至今 `)+r(D(p))+`</div></div>`}function ie(e,t){return re(e,[...p,`archived`],l,`draft`,t)}function ae(e,t){return re(e,se,u,`todo`,t)}function oe(e){let t=I(e,`draft`),n=new Map;for(let e of t)n.has(e.status)||n.set(e.status,e);let i=[...n.values()].map(e=>`<span class="dsh-pm-strip-item" data-status="`+r(e.status)+`">`+(l[e.status]??e.status)+` <b>`+r(h(e.at))+`</b></span>`);return i.length===0?``:`<div class="dsh-pm-strip">`+i.join(`<span class="dsh-pm-strip-arrow">→</span>`)+`</div>`}const se=[`todo`,`in_progress`,`integrating`,`testing`,`in_review`,`done`],ce=[`draft`,`brainstorming`,`decomposing`,`implementing`,`accepting`,`done`,`archived`];function le(e,t){let n=I(e,`todo`),r=O(e.status);return n.map((i,a)=>{let o=n[a+1]?.at??(r?Math.max(e.updatedAt,i.at):t);return{status:i.status,from:i.at,to:o}})}function ue(e,t,n){if(t.length===0)return`<div class="dsh-pm-empty">尚未拆分任务</div>`;let i=[...t].sort((e,t)=>e.createdAt-t.createdAt),a=[n];for(let e of i)for(let t of I(e,`todo`))a.push(t.at);for(let t of I(e,`draft`))a.push(t.at);let o=Math.min(...a),s=Math.max(...a),c=Math.max(s-o,36e5),d=e=>190+(e-o)/c*620,f=34+i.length*22+10,p=[];p.push(`<svg class="dsh-pm-gantt" viewBox="0 0 822 `+f+`" width="100%" height="`+f+`" preserveAspectRatio="xMinYMin meet" role="img" aria-label="任务甘特图">`);for(let e=0;e<=4;e++){let t=o+c*e/4,n=d(t).toFixed(1);p.push(`<line class="dsh-pm-gantt-grid" x1="`+n+`" y1="26" x2="`+n+`" y2="`+(f-10)+`" />`),p.push(`<text class="dsh-pm-gantt-axis" x="`+n+`" y="20" text-anchor="middle">`+r(h(t))+`</text>`)}for(let t of I(e,`draft`)){if(!ce.includes(t.status))continue;let n=d(t.at).toFixed(1);p.push(`<line class="dsh-pm-gantt-mile" data-status="`+r(t.status)+`" x1="`+n+`" y1="28" x2="`+n+`" y2="`+(f-10)+`">`),p.push(`<title>`+r(e.id+` `+(l[t.status]??t.status)+` `+h(t.at))+`</title></line>`)}if(i.forEach((e,t)=>{let i=34+t*22;p.push(`<text class="dsh-pm-gantt-rowlabel" x="6" y="`+(i+13)+`">`+r(k(e.id+` `+e.title,24))+`</text>`),p.push(`<rect class="dsh-pm-gantt-track" x="190" y="`+(i+4)+`" width="620" height="13" rx="3" />`);for(let t of le(e,n)){let n=d(t.from),a=Math.max(2,d(t.to)-n);p.push(`<rect class="dsh-pm-gantt-bar" data-status="`+r(t.status)+`" x="`+n.toFixed(1)+`" y="`+(i+4)+`" width="`+a.toFixed(1)+`" height="13" rx="3">`),p.push(`<title>`+r(e.id+` `+e.title+`｜`+(u[t.status]??t.status)+` `+h(t.from)+` → `+h(t.to)+`（`+D(t.to-t.from)+`）`)+`</title></rect>`)}}),n>=o&&n<=s){let e=d(n).toFixed(1);p.push(`<line class="dsh-pm-gantt-now" x1="`+e+`" y1="28" x2="`+e+`" y2="`+(f-10)+`"><title>现在</title></line>`)}p.push(`</svg>`);let m=`<div class="dsh-pm-gantt-legend">`+se.map(e=>`<span class="dsh-pm-gantt-legend-item"><i data-status="`+e+`"></i>`+u[e]+`</span>`).join(``)+`<span class="dsh-pm-gantt-legend-item"><i class="mile"></i>需求里程碑</span></div>`;return`<div class="dsh-pm-gantt-wrap">`+p.join(``)+`</div>`+m}function de(e,t){return`<table class="dsh-pm-ttable"><thead><tr><th>任务</th><th>标题</th><th>状态</th><th>阶段</th><th>端侧</th><th>依赖</th><th>创建</th><th>耗时</th></tr></thead><tbody>`+[...e].sort((e,t)=>e.createdAt-t.createdAt).map(e=>{let n=I(e,`todo`),i=n[0].at,a=n[n.length-1],o=n.find(e=>e.status===`done`)?.at,s=O(e.status)?`共 `+D((o??a.at)-i):`已用 `+D(t-i);return`<tr class="dsh-pm-trow" data-task="`+r(e.id)+`" data-action="open-task"><td class="dsh-pm-tid">`+r(e.id)+`</td><td class="dsh-pm-ttitle">`+r(e.title)+`</td><td><span class="dsh-pm-status" data-status="`+r(e.status)+`">`+(u[e.status]??e.status)+`</span></td><td>`+r(d[e.phase]??e.phase)+`</td><td>`+r(e.side)+`</td><td class="dsh-pm-tdeps">`+(e.dependsOn.length>0?r(e.dependsOn.join(` `)):`—`)+`</td><td>`+r(h(i))+`</td><td>`+r(s)+`</td></tr>`}).join(``)+`</tbody></table>`}function fe(e,t=Date.now()){let n=e.requirements.map(t=>({req:t,tasks:e.tasks.filter(e=>e.requirementId===t.id)})).filter(e=>e.tasks.length>0).sort((e,t)=>t.req.updatedAt-e.req.updatedAt),i=`<div class="dsh-pm-head"><button type="button" class="dsh-pm-btn" data-action="back" title="返回泳道看板">← 看板</button><h1 class="dsh-pm-title">任务</h1><span class="dsh-pm-rev">`+e.tasks.length+` 个任务 · `+n.length+` 个需求 · rev `+e.revision+`</span><button type="button" class="dsh-pm-btn" data-action="refresh" title="刷新">刷新</button></div>`;if(n.length===0)return`<div class="dsh-pm-board">`+i+`<div class="dsh-pm-empty">还没有任务。两种来源：① 需求详情页点「+ 任务」人工建卡；② 窗口 agent 调用 reqboard_decompose 真拆分落库（推荐，含依赖 DAG）</div></div>`;let a=n.map(e=>{let n=e.tasks.filter(e=>e.status===`done`).length;return`<div class="dsh-pm-tasks-group"><div class="dsh-pm-tasks-group-head"><span class="dsh-pm-card-id">`+r(e.req.id)+`</span><span class="dsh-pm-status" data-status="`+r(e.req.status)+`">`+(l[e.req.status]??e.req.status)+`</span><span class="dsh-pm-tasks-group-title">`+r(e.req.title)+`</span><span class="dsh-pm-hint">`+n+`/`+e.tasks.length+` 完成</span><button type="button" class="dsh-pm-btn sm" data-action="open-req" data-req="`+r(e.req.id)+`">打开需求</button></div>`+oe(e.req)+`<div class="dsh-pm-detail-section"><h3>甘特图</h3>`+ue(e.req,e.tasks,t)+`</div><div class="dsh-pm-detail-section"><h3>任务清单</h3>`+de(e.tasks,t)+`</div></div>`}).join(``);return`<div class="dsh-pm-board">`+i+`<div class="dsh-pm-tasks-page">`+a+`</div></div>`}const pe={requirement:{icon:`📄`,label:`需求文档`},ui:{icon:`🎨`,label:`UI 文档`},proposal:{icon:`📐`,label:`设计文档`},plan:{icon:`📝`,label:`实施计划`},decomposition:{icon:`🧩`,label:`拆分方案`},task_detail:{icon:`🗂️`,label:`任务卡`},verification:{icon:`✅`,label:`验收材料`},archive:{icon:`📦`,label:`归档材料`},retro:{icon:`🔁`,label:`复盘`},notes:{icon:`📒`,label:`其他`}};function me(e){let t=e===void 0?-1:N.indexOf(e);return t<0?N.length:t}function he(e){let t=[],n=new Set,r=(e,r)=>{let i=(r??``).trim();if(!i||n.has(i))return;n.add(i);let a=pe[e];t.push({icon:a?.icon??`📒`,label:a?.label??e,path:i})},i=[...e.artifacts??[]].sort((e,t)=>me(e.stage)-me(t.stage));for(let e of i)r(e.kind,e.path);e.docLinks?.requirement&&r(`requirement`,e.docLinks.requirement),e.docLinks?.ui&&r(`ui`,e.docLinks.ui),e.docLinks?.proposal&&r(`proposal`,e.docLinks.proposal);for(let r of e.docLinks?.extras??[]){let e=(r.path??``).trim();!e||n.has(e)||(n.add(e),t.push({icon:`🎁`,label:r.label||`成果文件`,path:e}))}e.plan?.path&&r(`plan`,e.plan.path);for(let t of e.archive?.docs??[])r(t.kind,t.path);return t}function ge(e){let t=he(e);return t.length===0?`<div class="dsh-pm-empty">暂无文档记录。窗口 agent 可用 <code>reqboard_archive_submit</code> 提交文档清单，或通过需求更新接口填充 docLinks（requirement/ui/proposal）。</div>`:`<ul class="dsh-pm-doc-list">`+t.map(e=>`<li data-doc-path="`+r(e.path)+`"><span class="dsh-pm-doc-icon">`+e.icon+`</span><span class="dsh-pm-doc-label">`+r(e.label)+`</span><button type="button" class="dsh-pm-doc-path" data-action="open-doc" data-path="`+r(e.path)+`">`+r(e.path)+`</button></li>`).join(``)+`</ul>`}function _e(e){return e.status===`accepting`?e.verification===void 0?`<span class="dsh-pm-flag verify-pending" title="验收态但还没提交验收材料">待验收材料</span>`:`<span class="dsh-pm-flag verify-pending" title="验收材料已提交，等人工审核">待人工审核</span>`:``}function ve(e){return e.status===`done`?e.archive===void 0?`<span class="dsh-pm-flag archive-pending" title="已完成，等窗口准备归档材料">待归档材料</span>`:`<span class="dsh-pm-flag archive-pending" title="归档材料已备，等人点归档">待归档</span>`:``}function ye(e){let t=e.verification;if(t===void 0)return`<div class="dsh-pm-block is-empty">`+(e.status===`implementing`||e.status===`accepting`?`窗口尚未提交验收材料。人工审核前需要证据：窗口用 <code>reqboard_verify_submit</code> 提交「做了什么 + 怎么验的 + 看到什么结果」。`:`尚未进入验收阶段。`)+`</div>`;let n=t.decision===`pass`?`<span class="dsh-pm-review" data-state="pass">人工审核通过 `+r(t.reviewedAt===void 0?``:h(t.reviewedAt))+`</span>`:t.decision===`rework`?`<span class="dsh-pm-review" data-state="rework">已退回返工 `+r(t.reviewedAt===void 0?``:h(t.reviewedAt))+`</span>`:`<span class="dsh-pm-review" data-state="pending">待人工审核</span>`,i=e.status===`accepting`?`<span class="dsh-pm-hint">请在详情头「本阶段操作」条点「验收通过」或「退回返工」</span>`:``,a=t.evidence.map(e=>`<li>`+r(e)+`</li>`).join(``);return`<div class="dsh-pm-block"><div class="dsh-pm-block-head">`+n+`<span class="dsh-pm-hint">提交 `+r(h(t.submittedAt))+`</span>`+i+`</div><div class="dsh-pm-block-summary">`+r(t.summary)+`</div><ul class="dsh-pm-evidence">`+a+`</ul>`+(t.reviewNote===void 0?``:`<div class="dsh-pm-block-note">审核意见：`+r(t.reviewNote)+`</div>`)+`</div>`}function be(e){let t=e.archive;if(t===void 0)return`<div class="dsh-pm-block is-empty">`+(e.status===`done`?`窗口尚未准备归档材料。归档不是挪目录：窗口用 <code>reqboard_archive_submit</code> 提交需求目录、文档清单、合并去向（只允许既有规范目录：docs/ 或 agent-dh/docs/ 下的 adr|architecture|guides|rfcs|work-logs|strategy-research）与一句话索引条目，人再点归档；必填文档与合并去向按需求类型限定，规范见 agent-dh/docs/architecture/requirement-archive.md。`:`归档在需求完成（done）后进行；不同需求类型的必填文档与合并去向见 agent-dh/docs/architecture/requirement-archive.md。`)+`</div>`;let n=t.archivedAt===void 0?`<span class="dsh-pm-review" data-state="pending">待归档（材料已备）</span>`:`<span class="dsh-pm-review" data-state="pass">已归档 `+r(h(t.archivedAt))+`</span>`,i=e.status===`done`&&t.archivedAt===void 0?`<span class="dsh-pm-hint">请在详情头「本阶段操作」条点「归档」</span>`:``,a=t.docs.map(e=>`<li><span class="dsh-pm-doc-kind">`+r(xe[e.kind]??e.kind)+`</span> <code>`+r(e.path)+`</code></li>`).join(``),o=t.mergedInto.map(e=>`<li><code>`+r(e)+`</code></li>`).join(``);return`<div class="dsh-pm-block"><div class="dsh-pm-block-head">`+n+`<code class="dsh-pm-block-path">`+r(t.dir)+`</code><span class="dsh-pm-hint">材料提交 `+r(h(t.submittedAt))+`</span>`+i+`</div><div class="dsh-pm-block-summary">索引条目：`+r(t.indexEntry)+`</div><div class="dsh-pm-doc-group"><span class="dsh-pm-hint">需求目录内的文档</span><ul class="dsh-pm-doc-list">`+a+`</ul></div><div class="dsh-pm-doc-group"><span class="dsh-pm-hint">合并进的项目文档</span><ul class="dsh-pm-doc-list">`+o+`</ul></div>`+Se(t)+`</div>`}const xe={requirement:`需求说明`,plan:`实施计划`,verification:`验收材料`,retro:`复盘`,notes:`其他`};function Se(e){let t=e.manualUpdates??[];return t.length===0?e.manualNote===void 0?``:`<div class="dsh-pm-doc-group"><span class="dsh-pm-hint">项目说明书更新</span><div class="dsh-pm-block-summary">无（`+r(e.manualNote)+`）</div></div>`:`<div class="dsh-pm-doc-group"><span class="dsh-pm-hint">项目说明书更新（金字塔向上生长）</span><ul class="dsh-pm-doc-list">`+t.map(e=>`<li><code>`+r(e.path)+`</code><span class="dsh-pm-doc-kind">`+r(e.section)+`</span><span>`+r(e.summary)+`</span></li>`).join(``)+`</ul></div>`}const L={requirement:`需求文档`,plan:`实施计划`,decomposition:`拆分方案`,task_detail:`任务卡`,verification:`验收材料`,archive:`归档材料`};function R(e){let t=F(e.category),n=[];for(let r of t.confirmGates){let t=M[r];if(t===void 0)continue;let i=(e.artifacts??[]).find(e=>e.kind===t);i===void 0?n.push({kind:t,status:`missing`}):i.confirmedAt===void 0?n.push({kind:t,status:`pending`,artifact:i}):n.push({kind:t,status:`confirmed`,artifact:i})}return n}function Ce(e){let t=A[e.status];if(t!==void 0&&t.length!==0)for(let n of t){let t=ne(e.category,e.status,n);if(t!==void 0)return t}}function we(e){let t=R(e);return t.length===0?``:`<div class="dsh-pm-artifact-chips">`+t.map(t=>{let n=L[t.kind]??t.kind;return t.status===`confirmed`?`<span class="dsh-pm-artifact-chip confirmed" title="`+r(n)+`已确认">✓ `+r(n)+`</span>`:t.status===`pending`?`<button type="button" class="dsh-pm-artifact-chip pending" data-action="confirm-artifact" data-id="`+r(e.id)+`" data-kind="`+r(t.kind)+`" title="点击确认`+r(n)+`">⏳ `+r(n)+`</button>`:`<span class="dsh-pm-artifact-chip missing" title="`+r(n)+`缺失">✗ `+r(n)+`</span>`}).join(``)+`</div>`}function Te(e){let t=Ce(e);if(t===void 0)return``;let n=(e.artifacts??[]).find(e=>e.kind===t);if(n===void 0||n.confirmedAt!==void 0)return``;let i=L[t]??t;return`<button type="button" class="dsh-pm-btn sm primary dsh-pm-confirm-artifact" data-action="confirm-artifact" data-id="`+r(e.id)+`" data-kind="`+r(t)+`" title="一键确认`+r(i)+`，放行下一阶段">确认产物</button>`}function Ee(e){let t=F(e.category),n=[];for(let e of t.stages){let t=j[e];t!==void 0&&n.push(...t)}let r=(e.artifacts??[]).length,i=n.length,a=R(e).filter(e=>e.status===`pending`).length;if(i===0&&a===0)return``;let o=[];return i>0&&o.push(`产物 `+r+`/`+i),a>0&&o.push(a+` 门待确认`),`<div class="dsh-pm-artifact-derived">`+o.join(` · `)+`</div>`}function De(e,t,n=_){let{req:i,tasks:a,doneCount:o,totalCount:s,readyIds:c,blocked:l}=e,u=s>0?Math.round(o/s*100):0,d=i.category?`<span class="dsh-pm-cat" data-cat="${i.category}">${f[i.category]??i.category}</span>`:``,p=Ae(i)+_e(i)+ve(i),m=l?`<span class="dsh-pm-flag blocked">阻塞</span>`:``,h=i.paused?`<span class="dsh-pm-flag paused">暂停</span>`:``,v=c.length>0?`<span class="dsh-pm-flag ready">${c.length} ready</span>`:``,y=ke(i,t),S=b(i,n)+x(a,n),C=Oe(i),w=we(i),T=Te(i),E=Ee(i);return`
    ...
    </div>`,f=n===`list`?Ie(e,t,i,a):`<div class="dsh-pm-lanes">${s}</div>`;return`
    <div class="dsh-pm-board">
      ...
      ${f}
      ${n===`list`?``:u}
    </div>`}const B=[10,20,50];function V(e){return e===`updated`||e===`created`||e===`progress`?`desc`:`asc`}const Me=[{key:`stage`,label:`阶段`},{key:`progress`,label:`进度`},{key:`updated`,label:`最近更新`},{key:`created`,label:`创建时间`},{key:`title`,label:`名称`}],Ne={implementing:0,accepting:1,decomposing:2,planning:3,brainstorming:4,draft:5,done:6};function Pe(e){return e.totalCount>0?e.doneCount/e.totalCount:0}function Fe(e){switch(e){case`stage`:return(e,t)=>(Ne[e.req.status]??99)-(Ne[t.req.status]??99);case`progress`:return(e,t)=>Pe(e)-Pe(t);case`created`:return(e,t)=>e.req.createdAt-t.req.createdAt;case`title`:return(e,t)=>e.req.title.localeCompare(t.req.title,`zh-Hans-CN`);default:return(e,t)=>e.req.updatedAt-t.req.updatedAt}}function Ie(e,t=Date.now(),n={},r=_){let i=n.sortKey??`stage`,o=n.sortDir??V(i),s=n.pageSize!==void 0&&B.includes(n.pageSize)?n.pageSize:10,c=je(e),l=c.filter(e=>e.req.status!==`done`&&e.req.status!==`archived`),u=c.filter(e=>e.req.status===`done`||e.req.status===`archived`),d=Fe(i),f=o===`asc`?1:-1,p=(e,t)=>d(e,t)*f||t.req.updatedAt-e.req.updatedAt;l.sort(p),u.sort(p);let m=[...l,...u],h=Le(i,o,c.length,l.length,u.length,s);if(m.length===0)return`<div class="dsh-pm-list">${h}<div class="dsh-pm-list-empty">暂无进行中的需求</div></div>`;let g=Math.max(1,Math.ceil(m.length/s)),v=Math.min(Math.max(1,n.page??1),g),y=m.slice((v-1)*s,v*s),b=l.length,x=(v-1)*s,S=[];for(let e=0;e<y.length;e++){let n=x+e;l.length>0&&(n===0||n===x&&n<b)&&S.push(`<tr class="dsh-pm-list-grouphead" data-group="active"><td colspan="8">进行中 ${l.length}${n>0?`（续）`:``}</td></tr>`),u.length>0&&(n===b||n===x&&n>=b)&&S.push(`<tr class="dsh-pm-list-grouphead" data-group="done"><td colspan="8">已完成 ${u.length}${n>b?`（续）`:``}</td></tr>`),S.push(Re(y[e],t,r))}let C=m.length>s?`<div class="dsh-pm-pager">${a({page:v,total:g,totalItems:m.length,pageAttr:`data-pmpage`})}</div>`:``;return`<div class="dsh-pm-list">${h}<table class="dsh-pm-table"><thead><tr><th>ID</th><th>标题</th><th>分类</th><th>状态</th><th>进度</th><th>负责人</th><th>更新时间</th><th>操作</th></tr></thead><tbody>${S.join(``)}</tbody></table>${C}</div>`}function Le(e,t,n,r,i,a){return`
    <div class="dsh-pm-list-toolbar">
      <span class="dsh-pm-list-toolbar-label">排序</span>
      ${Me.map(({key:n,label:r})=>{let i=n===e;return`<button type="button" class="dsh-pm-sortbtn${i?` active`:``}" data-action="list-sort" data-key="${n}" title="按${r}排序（再点一次切换升降序）">${r}${i?t===`asc`?` ↑`:` ↓`:``}</button>`}).join(``)}
    ...
    </div>`}function Re(e,t,n=_){let{req:i,tasks:a,doneCount:o,totalCount:s,blocked:c}=e,d=s>0?Math.round(o/s*100):0,p=a.filter(e=>e.status!==`todo`&&e.status!==`done`&&e.status!==`canceled`).length,g=i.category?`<span class="dsh-pm-cat" data-cat="${i.category}">${f[i.category]??i.category}</span>`:``,v=c?`<span class="dsh-pm-flag blocked">阻塞</span>`:``,b=i.sourceSessionId,x=b!==void 0&&b.length>0&&n.has(b),S=b!==void 0&&b.length>0?y({sid:b,label:m(b),cls:`dsh-pm-window`,kind:`立项来源窗口`,archived:x}):`<span class="dsh-pm-list-nowindow">人工建卡</span>`;a.length===0||(()=>{let e={};for(let t of a)e[t.status]=(e[t.status]??0)+1;return[`in_progress`,`integrating`,`testing`,`in_review`,`todo`,`done`].filter(t=>(e[t]??0)>0).map(t=>`<span class="dsh-pm-list-seg" data-status="${t}">${u[t]??t} ${e[t]}</span>`).join(``)})();let C=i.status===`archived`&&i.archive===void 0?`<span class="dsh-pm-chip is-warn">归档材料待补</span>`:``,w=`dsh-pm-list-row`+(c?` is-blocked`:``)+(i.status===`done`||i.status===`archived`?` is-archived`:``),T=b!==void 0&&b.length>0&&!x?`<button type="button" class="dsh-pm-btn sm" data-action="jump-session" data-sid="${r(b)}" title="跳转到来源会话">会话</button>`:``;return`
      <tr class="${w}" data-req="${r(i.id)}" data-action="open-req">
        ...
        <td><div class="dsh-pm-list-actions">${Oe(i)}${T}</div></td>
      </tr>`}function ze(e,t){let n=e.filter(e=>e.status===`pending`),i=t.requirements.filter(e=>e.status!==`archived`&&e.status!==`canceled`),a=n.map(e=>{let t=e.suggestedAction===`create_req`&&!e.suggestedTargetId,n=e.suggestedTargetId?`${e.suggestedAction===`bind_req`?`绑定需求`:e.suggestedAction===`bind_task`?`绑定任务`:`新建需求`} ${r(e.suggestedTargetId)}`:e.suggestedAction===`create_req`?`新建需求${e.suggestedCategory?` · ${f[e.suggestedCategory]??e.suggestedCategory}`:``}`:``,i=t?`
    ...
    </div>`}const Be={draft:{label:`立项`,color:`#9aa4b2`,order:0},brainstorming:{label:`需求分析`,color:`#f0a020`,order:1},planning:{label:`技术设计`,color:`#c2255c`,order:2},decomposing:{label:`拆分`,color:`#8e44ad`,order:3},implementing:{label:`实施`,color:`#4a7dff`,order:4},accepting:{label:`验收`,color:`#17a2b8`,order:5},done:{label:`完成`,color:`#28a745`,order:6},archived:{label:`归档`,color:`#28a745`,order:7}},Ve=[`draft`,`brainstorming`,`planning`,`decomposing`,`implementing`,`accepting`,`done`,`archived`];function He(e){return Be[e]?.order??-1}function Ue(e){let t=He(e);return`<div class="dsh-pm-progress-dots">${Ve.map(e=>{let{label:n,order:i}=Be[e];return`
    ...
    </div>`}function Ge(e,t,n,i,a){return`
    ...
    <div class="dsh-pm-tab-content active" data-tab-content="overview">
      ...
      <details class="dsh-pm-fold">
        ...
        <div class="dsh-pm-section-content">${ge(e)}</div>
      </details>
    </div>
    ...
    <div class="dsh-pm-tab-content" data-tab-content="timeline">
      <div class="dsh-pm-section">
        ...
        <div class="dsh-pm-section-content">${ie(e,a)}</div>
      </div>
      <div class="dsh-pm-section">
    ...
    <div class="dsh-pm-tab-content" data-tab-content="archive">
      ...
      <div class="dsh-pm-section">
        ...
        <div class="dsh-pm-section-content">${be(e)}</div>
      </div>
    </div>`}function Ke(e,t,n=Date.now(),i=_){let a=t.filter(t=>t.requirementId===e.id),o=Ye(a),s=w(e.comments),c=qe(e.status),u=Je(e);return`
    <div class="dsh-pm-detail" data-detail-req="${r(e.id)}">
      ...
      ${We()}
      ${Ge(e,a,o,s,n)}
    </div>`}function qe(e){let t={draft:`已立项：窗口接手开工后自动进入需求分析，人可在上方操作条手动催办。`,brainstorming:`需求分析中：窗口 agent 会自行推进到技术设计，人可在上方操作条确认方案或退回立项。`,planning:`技术设计中：计划提交后请在上方操作条点「批准计划」——批准前拆分会被告代码级拒绝。`,decomposing:`拆分中：任务落库/开工后系统自动推进到实施，人可在上方操作条确认拆分。`,implementing:`实施中：任务全部完成时自动进入验收，人可在上方操作条提交验收。`,accepting:`验收中：看完验收材料后，在上方操作条点「验收通过」或「退回返工」。`,done:`已完成（历史状态）：验收通过现已直接归档，此状态仅存在于旧记录。`}[e];return t===void 0?``:`<div class="dsh-pm-gate">`+t+`</div>`}function Je(e){let t=[],n=(n,i,a,o=!1)=>{let s=o?`dsh-pm-btn primary`:`dsh-pm-btn`;t.push(`<button type="button" class="`+s+`" data-action="`+n+`" data-id="`+r(e.id)+`" title="`+r(a)+`">`+i+`</button>`)},i=(n,i,a,o=!1)=>{let s=o?`dsh-pm-btn primary`:`dsh-pm-btn`;t.push(`<button type="button" class="`+s+`" data-action="move-req" data-to="`+n+`" data-id="`+r(e.id)+`" title="`+r(a)+`">`+i+`</button>`)};switch(e.status){case`draft`:i(`brainstorming`,`→ 需求分析`,`进入需求分析；窗口接手开工时会自动进入`,!0),i(`canceled`,`取消`,`取消该需求（仅人可操作）`);break;case`brainstorming`:i(`planning`,`→ 技术设计`,`方案谈定 → 进入技术设计；请提交计划并待批准`,!0),i(`draft`,`退回立项`,`方案要重谈 → 退回立项`);break;case`planning`:i(`decomposing`,`→ 拆分`,`计划获批后落库任务卡；未获批会被代码级拒绝`,!0),i(`brainstorming`,`退回重谈`,`方案要改 → 退回需求分析`);break;case`decomposing`:i(`implementing`,`→ 实施`,`确认拆分，进入实施；任务开工时系统会自动推进`,!0);break;case`implementing`:i(`accepting`,`→ 验收`,`提交验收；任务全部完成时系统会自动推进`,!0)}return e.plan!==void 0&&e.plan.approvedAt===void 0&&(n(`plan-approve`,`批准计划`,`批准实施计划，解锁 reqboard_decompose 拆分`,!0),n(`plan-reject`,`退回计划`,`退回实施计划（窗口按理由重写）`)),e.status===`accepting`&&e.verification!==void 0&&(n(`verify-pass`,`验收通过`,`人工审核通过，需求进入完成`,!0),n(`verify-rework`,`退回返工`,`退回返工（需填写意见）`)),e.status===`done`&&e.archive!==void 0&&e.archive.archivedAt===void 0&&n(`archive-req`,`归档`,`归档：把产出并进项目文档`,!0),t.length===0?``:`<div class="dsh-pm-action-bar" data-req="`+r(e.id)+`"><span class="dsh-pm-action-bar-label">本阶段操作</span>`+t.join(``)+`</div>`}function Ye(e){if(e.length===0)return`<div class="dsh-pm-empty">暂无任务</div>`;let t=new Map,n=new Map(e.map(e=>[e.id,e])),i=(e,r)=>{if(t.has(e.id))return t.get(e.id);if(r.has(e.id))return 0;r.add(e.id);let a=e.dependsOn.filter(e=>n.has(e)),o=a.length===0?0:1+Math.max(...a.map(e=>i(n.get(e),r)));return t.set(e.id,o),o};e.forEach(e=>i(e,new Set));let a=Math.max(...t.values()),o=Array.from({length:a+1},()=>[]);return e.forEach(e=>o[t.get(e.id)].push(e)),`<div class="dsh-pm-dag"><div class="dsh-pm-dag-title">🔀 任务依赖关系</div><div class="dsh-pm-dag-layers">`+o.map((e,t)=>`
    ...
    </div>`).join(``)+`</div></div>`}function Xe(e){if(e.length===0)return`<div class="dsh-pm-empty">暂无任务</div>`;let t=e=>e===`done`?`✓`:e===`in_progress`?`◐`:e===`todo`?`○`:`◐`;return`
    ...
    ${s}
    ${l}
    ${u}`}function et(e){let t=0,n=0,i=0,a=0,o=[],s=e.executions[e.executions.length-1];s?.evidence&&s.evidence.forEach(e=>{let r=e.match(/(\d+)\s*passed.*?(\d+)\s*failed.*?(\d+)\s*skipped/i);r&&(n=parseInt(r[1],10),i=parseInt(r[2],10),a=parseInt(r[3],10),t=n+i+a)}),s?.error&&s.error.split(`
`).forEach(e=>{let t=e.match(/(.+?):(\d+)\s*Expected:\s*(.+?)\s*Actual:\s*(.+)/);t&&o.push({name:`测试用例`,file:t[1]+`:`+t[2],expected:t[3],actual:t[4]})});let c=t>0?(n/t*100).toFixed(1):`0`,l=o.length>0?o.map(e=>`
    ...
    ${f}
    ${p}`}function rt(e){let t=e.title.toLowerCase();if(t.includes(`拆分`)||t.includes(`decompose`))return`decompose`;if(e.status===`integrating`||t.includes(`集成`)||t.includes(`联调`))return`merge`;switch(e.phase){case`doc`:return`doc`;case`ui`:return`ui`;case`analysis`:return`analysis`;case`implement`:return`implement`;case`test`:return`test`;case`review`:return`review`;case`merge`:return`merge`;default:return`generic`}}const it={decompose:`🔀`,implement:`⚙️`,test:`🧪`,review:`👀`,merge:`🔀`,doc:`📝`,ui:`🎨`,analysis:`🔍`,generic:`📋`},at={decompose:`拆分任务`,implement:`实施任务`,test:`测试任务`,review:`评审任务`,merge:`合并任务`,doc:`文档任务`,ui:`UI设计`,analysis:`分析任务`,generic:`任务`};function ot(e,t,n=Date.now(),i=[],a=_){let o=rt(e),s=it[o],c=at[o],l=st(e,o,t,i),d=ct(e,n,a);return`
    <div class="dsh-pm-taskdetail" data-detail-task="${r(e.id)}" data-node-type="${o}">
      ...
      ${l}
      ${d}
    </div>`}function st(e,t,n,r){switch(t){case`decompose`:return lt(e,n,r);case`implement`:return ut(e);case`test`:return et(e);case`review`:return tt(e);case`merge`:return nt(e);case`doc`:return Ze(e);case`ui`:return Qe(e);case`analysis`:return $e(e);default:return``}}function ct(e,t,n=_){let i=e.executions.map(e=>`
    ...
    </details>`}function lt(e,t,n){if(!t)return`<div class="dsh-pm-detail-section"><div class="dsh-pm-empty">需求数据不可用</div></div>`;let i=n.filter(e=>e.requirementId===t.id),a=i.length,o=i.filter(e=>e.status===`done`).length,s={};return i.forEach(e=>{let t=e.side===`frontend`?`UI 轨道`:e.side===`backend`?`后端轨道`:e.side===`doc`?`文档轨道`:`全栈轨道`;s[t]||(s[t]=[]),s[t].push(e)}),`
    <div class="dsh-pm-detail-section dsh-pm-specialized">
      <h3>📊 拆分结果</h3>
      <div class="dsh-pm-stats">
        <div class="dsh-pm-stat">
    ...
    <div class="dsh-pm-detail-section">
      <h3>🌳 依赖关系 DAG</h3>
      ${Ye(i)}
    </div>`}function ut(e){let t=[],n=e.executions[e.executions.length-1];n?.evidence&&n.evidence.forEach(e=>{let n=e.match(/^(.+?)\s*\(?\+(\d+)(?:,\s*-(\d+))?\)?$/);n&&t.push({path:n[1].trim(),added:parseInt(n[2],10),deleted:parseInt(n[3]||`0`,10)})});let i=t.reduce((e,t)=>e+t.added,0),a=t.reduce((e,t)=>e+t.deleted,0),o=t.length>0?t.map(e=>`
    ...
    </div>`}const dt=8e3;var ft=class extends Error{code;constructor(e,t){super(e),this.code=t}};async function pt(e){let t=await e;if(!t.ok)throw new ft(`HTTP `+t.status);let n=await t.json().catch(()=>({}));if(n.success!==!0)throw new ft(n.error??`API 返回失败`,n.code);return n.data}const H=e=>pt(fetch(e,{signal:AbortSignal.timeout(dt)})),U=(e,t)=>pt(fetch(e,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t),signal:AbortSignal.timeout(dt)})),mt=()=>H(`/dashboard/api/reqboard/`),ht=()=>H(`/dashboard/api/reqboard/triage`);function gt(e){return U(`/dashboard/api/reqboard/req/create`,e)}function _t(e){return U(`/dashboard/api/reqboard/req/move`,e)}function vt(e){return U(`/dashboard/api/reqboard/req/plan/approve`,e)}function yt(e){return U(`/dashboard/api/reqboard/req/plan/reject`,e)}function bt(e){return U(`/dashboard/api/reqboard/req/verify/pass`,e)}function xt(e){return U(`/dashboard/api/reqboard/req/verify/rework`,e)}function St(e){return U(`/dashboard/api/reqboard/req/verdicts`,e)}function Ct(e){return U(`/dashboard/api/reqboard/req/archive`,e)}function wt(e){return H(`/dashboard/api/reqboard/requirements/`+encodeURIComponent(e)+`/stages`)}function Tt(e){return U(`/dashboard/api/reqboard/req/artifact/confirm`,e)}function Et(e){return U(`/dashboard/api/reqboard/task/create`,e)}function Dt(e){return U(`/dashboard/api/reqboard/comment`,e)}function Ot(e){return U(`/dashboard/api/reqboard/triage/confirm`,e)}function kt(e){return U(`/dashboard/api/reqboard/triage/rebind`,e)}function At(e){return U(`/dashboard/api/reqboard/triage/reject`,e)}function jt(e){let t=new EventSource(`/dashboard/api/reqboard/events`);return t.onmessage=t=>{try{let n=JSON.parse(t.data);e(n.revision,n.kind)}catch{}},()=>t.close()}function Mt(e){return encodeURIComponent(e).replace(/%3A/gi,`:`)}function Nt(e){return e.split(`/`).map(Mt).join(`/`)}function Pt(e,t){let n=t.replace(/\\/g,`/`).replace(/^(?:\.\/)+/,``);return`dsh-resource://file/session/${Mt(e)}/${Nt(n)}`}function Ft(e){let t=e?.sidebarRight;if(!(typeof t!=`object`||!t))return t}function It(){try{let e=window,t=(e.__dshPmSessions??e.__dshPmCtx?.sessions)?.list?.getSnapshot?.(),n=t?.current??t?.currentSessionId;if(typeof n==`string`&&n.length>0)return n}catch{}}function Lt(e,t,n){if(typeof n!=`string`||n.length===0)return console.error(`[dsh-pmboard] open-doc: 取不到会话 id，无法构造 session 地址`,{path:t}),!1;let r=Ft(e);if(r===void 0||typeof r.openResource!=`function`)return console.error(`[dsh-pmboard] open-doc: ctx.sidebarRight 不可用（官方右侧栏未加载）`,{path:t}),!1;let i=Pt(n,t);try{return r.openResource(i),!0}catch(e){return console.error(`[dsh-pmboard] open-doc: openResource 调用失败`,{address:i,error:String(e)}),!1}}function W(){let e=()=>window;return{getSessions:()=>{try{let t=e().__dshPmSessions??e().__dshPmCtx?.sessions;if(t&&typeof t.open==`function`&&t.list)return t}catch{}},getWorkspaces:()=>{try{let t=e().__dshPmWorkspaces??e().__dshPmCtx?.workspaces;if(t&&t.list)return t}catch{}}}}function Rt(e=W()){try{let t=e.getWorkspaces()?.list.getSnapshot().archivedSessionIds??[];return new Set(t)}catch{return new Set}}async function zt(e,t){let n=e.getSessions();if(n===void 0)return`unavailable`;let r=e=>{try{return n.list.getSnapshot().byId[e]!==void 0}catch{return!1}};if(r(t))return(e.getWorkspaces()?.list.getSnapshot().archivedSessionIds??[]).includes(t)?`archived`:(n.open(t),`opened`);try{await n.refresh()}catch{}return r(t)?(e.getWorkspaces()?.list.getSnapshot().archivedSessionIds??[]).includes(t)?`archived`:(n.open(t),`opened`):`missing`}const Bt={draft:`立项`,brainstorming:`需求分析`,planning:`技术设计`,decomposing:`拆分`,implementing:`实施`,accepting:`验收`,archived:`归档`},Vt={requirement:`需求文档`,plan:`实施计划`,decomposition:`拆分方案`,task_detail:`任务卡`,verification:`验收材料`,archive:`归档材料`},Ht=[`requirement`,`plan`,`decomposition`,`task_detail`,`verification`,`archive`],Ut={todo:`待开始`,in_progress:`开发中`,integrating:`联调中`,testing:`测试中`,in_review:`待评审`,done:`已完成`,canceled:`已取消`},Wt={todo:`○`,in_progress:`◐`,integrating:`◐`,testing:`◐`,in_review:`◐`,done:`✓`,canceled:`✕`},Gt={done:`✓`,current:`●`,pending:`○`,skipped:`—`};function G(e){let t=new Date(e),n=e=>String(e).padStart(2,`0`);return t.getFullYear()+`-`+n(t.getMonth()+1)+`-`+n(t.getDate())+` `+n(t.getHours())+`:`+n(t.getMinutes())}function K(e,t=Date.now()){let n=t-e;if(n<6e4)return`刚刚`;if(n<36e5)return Math.floor(n/6e4)+` 分钟前`;if(n<864e5)return Math.floor(n/36e5)+` 小时前`;let r=new Date(e),i=e=>String(e).padStart(2,`0`);return n<1728e5?`昨天 `+i(r.getHours())+`:`+i(r.getMinutes()):i(r.getMonth()+1)+`-`+i(r.getDate())}function q(e,t){return e.length>t?e.slice(0,t)+`…`:e}function J(e){if(e===void 0)return``;if(e.kind===`human`)return`人`;if(e.kind===`system`)return`系统`;let t=e.sessionId;return typeof t!=`string`||t.length===0?`Agent`:t.startsWith(`session-`)?te(t):`sub `+te(t)}function Kt(e){let t=e.executions??[],n=t.length>0?t[t.length-1].sessionId:e.claimedBy;return J(n===void 0?void 0:{kind:`agent`,sessionId:n})}function qt(e){let t=e.executions??[],n=t.filter(e=>e.outcome===`failed`).length;if(e.status===`done`){let e=[...t].reverse().find(e=>e.outcome===`succeeded`&&e.endedAt!==void 0),n=e?.endedAt===void 0?``:G(e.endedAt);return{text:(n.length>0?n+` `:``)+`完成`,failed:!1}}if(e.status===`in_progress`||e.status===`integrating`||e.status===`testing`){let r=[...t].reverse().find(e=>e.outcome===`running`),i=r===void 0?``:`（`+K(r.startedAt)+`开始）`,a=n>0?` · 失败 `+n+` 次`:``;return{text:(Ut[e.status]??e.status)+i+a,failed:n>0}}return n>0?{text:`失败 `+n+` 次`,failed:!0}:{text:Ut[e.status]??e.status,failed:!1}}const Jt=e=>{let t=e.body,n=[t.sourceWindow?`来源窗口 `+t.sourceWindow:``,t.createdAt?G(t.createdAt):``].filter(e=>e.length>0).join(` · `);return`<div class="dsh-pm-sn-body" data-stage="draft"><div class="dsh-pm-sn-req-title">`+r(t.title)+(t.category?` <span class="dsh-pm-sn-dim">（`+r(t.category)+`）</span>`:``)+`</div>`+(t.description?`<div class="dsh-pm-sn-text">`+r(t.description)+`</div>`:`<div class="dsh-pm-sn-empty">暂无描述</div>`)+(n.length>0?`<div class="dsh-pm-sn-dim">`+r(n)+`</div>`:``)+`</div>`},Yt=e=>{let t=e.body.comments??[];return t.length===0?`<div class="dsh-pm-sn-body" data-stage="brainstorming"><div class="dsh-pm-sn-empty">暂无评论</div></div>`:`<div class="dsh-pm-sn-body" data-stage="brainstorming">`+t.map(e=>{let t=J(e.createdBy);return`<div class="dsh-pm-sn-comment"><div class="dsh-pm-sn-comment-who" data-actor="`+r(e.createdBy?.kind??`agent`)+`">`+r(t)+`</div><div class="dsh-pm-sn-comment-text">`+r(e.body)+`</div></div>`}).join(``)+`</div>`},Xt=e=>{let t=e.body;if(!t.plan)return`<div class="dsh-pm-sn-body" data-stage="planning"><div class="dsh-pm-sn-empty">尚未提交实施计划</div></div>`;let n=t.plan,i=n.approvedAt===void 0?n.rejectedAt===void 0?`提交：`+G(n.submittedAt)+` · 待批准`:`退回：`+G(n.rejectedAt)+(n.rejectedReason?` · `+n.rejectedReason:``):`批准：`+(n.approvedBy?.kind===`human`?`人`:`Agent`)+` · `+G(n.approvedAt);return`<div class="dsh-pm-sn-body" data-stage="planning">`+(n.summary?`<div class="dsh-pm-sn-text">`+r(n.summary)+`</div>`:``)+`<div class="dsh-pm-sn-dim">`+n.tasks.length+` 个任务 · `+r(i)+`</div></div>`};function Zt(e){let t=new Map(e.map(e=>[e.id,e])),n=new Map;function r(e){if(n.has(e))return n.get(e);let i=t.get(e);if(!i||!i.dependsOn||i.dependsOn.length===0)return n.set(e,0),0;let a=1+Math.max(...i.dependsOn.map(e=>r(e)));return n.set(e,a),a}for(let t of e)r(t.id);let i=new Map;for(let t of e){let e=n.get(t.id)??0;i.has(e)||i.set(e,[]),i.get(e).push(t)}return i}const Qt=e=>{let t=e.body.tasks??[];if(t.length===0)return`<div class="dsh-pm-sn-body" data-stage="decomposing"><div class="dsh-pm-sn-empty">尚未拆分任务</div></div>`;let n=Zt(t),i=[];for(let[e,t]of[...n.entries()].sort((e,t)=>e[0]-t[0])){let n=e===0?`第 1 层 · 无依赖`:`第 `+(e+1)+` 层`+(t.length>1?` · `+t.length+` 个可并行`:``),a=t.map(e=>`<div class="dsh-pm-sn-dag-task"><span class="dsh-pm-sn-task-id">`+r(e.id)+`</span><span class="dsh-pm-sn-text">`+r(e.title)+`</span></div>`).join(``);i.push(`<div class="dsh-pm-sn-dag-layer"><div class="dsh-pm-sn-dag-label">`+r(n)+`</div>`+a+`</div>`)}return`<div class="dsh-pm-sn-body" data-stage="decomposing">`+i.join(``)+`</div>`},$t=e=>{let t=e.body,n=t.tasks??[];if(n.length===0)return`<div class="dsh-pm-sn-body" data-stage="implementing"><div class="dsh-pm-sn-empty">暂无执行任务</div></div>`;let i=n.filter(e=>e.status===`in_progress`||e.status===`integrating`||e.status===`testing`),a=n.filter(e=>e.status===`done`),o=n.filter(e=>e.status===`todo`||e.status===`in_review`),s=e=>{let t=Wt[e.status]??`○`,n=Kt(e),a=qt(e),o=(i.includes(e)?` is-current`:``)+(a.failed?` is-failed`:``),s=[e.id,n,a.text].filter(e=>e.length>0).join(` · `),c=e.cardDoc?` · <button type="button" class="dsh-pm-sn-doc" data-action="open-doc" data-path="`+r(e.cardDoc)+`">任务卡</button>`:``;return`<div class="dsh-pm-sn-task`+o+`" data-status="`+r(e.status)+`"><div class="dsh-pm-sn-task-line1"><span class="dsh-pm-sn-task-glyph">`+t+`</span><span class="dsh-pm-sn-task-title">`+r(e.title)+`</span></div><div class="dsh-pm-sn-task-line2">`+r(s)+c+`</div></div>`},c=[];i.length>0&&c.push(`<div class="dsh-pm-sn-group"><div class="dsh-pm-sn-group-label is-active">进行中 `+i.length+` 个</div>`+i.map(s).join(``)+`</div>`),a.length>0&&c.push(`<div class="dsh-pm-sn-group"><div class="dsh-pm-sn-group-label">已完成 `+a.length+` 个</div>`+a.map(s).join(``)+`</div>`),o.length>0&&c.push(`<div class="dsh-pm-sn-group"><div class="dsh-pm-sn-group-label">待开始 `+o.length+` 个</div>`+o.map(s).join(``)+`</div>`);let l=Object.entries(t.byWindow??{}),u=l.length>1?`<div class="dsh-pm-sn-dim">窗口分工：`+l.map(([e,t])=>r(e)+` `+t.length+` 任务`).join(` · `)+`</div>`:``;return`<div class="dsh-pm-sn-body" data-stage="implementing">`+c.join(``)+u+`</div>`};function en(e,t){let n=t.sheet;if(n===void 0||n.items.length===0)return``;let i=e.requirementId??``,a={pending:`⬜ 待验`,passed:`✅ 通过`,failed:`❌ 不通过`},o=n.items.map(e=>{let t=e.status!==`pending`,n=e.source.kind===`requirement`?`requirement`:e.source.taskId;return`<div class="dsh-pm-vitem" data-item-id="`+r(e.id)+`" data-source="`+r(n)+`"><div class="dsh-pm-vitem-head"><span class="dsh-pm-vitem-badge">`+(a[e.status]??r(e.status))+`</span><span class="dsh-pm-vitem-src">`+r(e.source.kind===`requirement`?`需求级`:e.source.taskId)+`</span></div><div class="dsh-pm-sn-text">`+r(q(e.criterion,200))+`</div>`+(t?e.opinion?`<div class="dsh-pm-sn-warn">意见：`+r(q(e.opinion,200))+`</div>`:``:`<div class="dsh-pm-vitem-actions"><label><input type="radio" name="verdict-`+r(e.id)+`" value="passed"> 通过</label><label><input type="radio" name="verdict-`+r(e.id)+`" value="failed"> 不通过</label><input type="text" class="dsh-pm-vitem-opinion" placeholder="不通过时填意见（必填）"></div>`)+`</div>`}).join(``),s=n.items.every(e=>e.status!==`pending`);return`<div class="dsh-pm-vsheet" data-req="`+r(i)+`" data-version="`+n.version+`"><div class="dsh-pm-sn-label">验收单 v`+n.version+(n.reworkOnly===!0?`（返工续验：只含未过项）`:``)+` · 共 `+n.items.length+` 项</div>`+o+(s?`<div class="dsh-pm-sn-dim">本轮已裁决完毕：全部通过请点上方「验收通过」归档；有未过项已自动打回返工。</div>`:`<button type="button" class="dsh-pm-btn sm primary" data-action="submit-verdicts" data-req="`+r(i)+`" data-version="`+n.version+`">提交裁决</button>`)+`</div>`}const tn={draft:{renderBody:Jt},brainstorming:{renderBody:Yt},planning:{renderBody:Xt},decomposing:{renderBody:Qt},implementing:{renderBody:$t},accepting:{renderBody:e=>{let t=e.body;if(!t.verification)return`<div class="dsh-pm-sn-body" data-stage="accepting"><div class="dsh-pm-sn-empty">尚未提交验收材料</div></div>`;let n=t.verification,i=en(e,n),a=(n.evidence??[]).slice(0,5),o=a.map(e=>`<div class="dsh-pm-sn-line"><span class="dsh-pm-sn-text">- `+r(q(e,140))+`</span></div>`).join(``);return`<div class="dsh-pm-sn-body" data-stage="accepting">`+(n.summary?`<div class="dsh-pm-sn-text">`+r(n.summary)+`</div>`:``)+i+(a.length>0?`<div class="dsh-pm-sn-label">证据（`+(n.evidence??[]).length+` 条）</div>`+o:``)+(n.reviewNote?`<div class="dsh-pm-sn-warn">审核意见：`+r(q(n.reviewNote,200))+`</div>`:``)+`</div>`}},archived:{renderBody:e=>{let t=e.body;if(!t.archive)return`<div class="dsh-pm-sn-body" data-stage="archived"><div class="dsh-pm-sn-empty">暂无归档材料</div></div>`;let n=t.archive,i=(n.mergedInto?.length??0)>0?`<div class="dsh-pm-sn-dim">合并去向：`+(n.mergedInto??[]).map(e=>`<button type="button" class="dsh-pm-sn-doc" data-action="open-doc" data-path="`+r(e)+`">`+r(e)+`</button>`).join(` · `)+`</div>`:``;return`<div class="dsh-pm-sn-body" data-stage="archived"><div class="dsh-pm-sn-text">归档目录：`+r(n.dir)+`</div>`+(n.indexEntry?`<div class="dsh-pm-sn-text">`+r(n.indexEntry)+`</div>`:``)+i+`</div>`}}};function nn(e){return e.length===0?``:`<div class="dsh-pm-sn-label">动态</div>`+e.slice(-5).reverse().map(e=>{let t=J(e.by),n=e.reason!==void 0&&e.reason.length>0?e.reason:Bt[e.status]??e.status,i=e.inferred===!0?` <span class="dsh-pm-sn-dim">(回填)</span>`:``;return`<div class="dsh-pm-sn-comment"><div class="dsh-pm-sn-comment-who">`+r([t,K(e.at)].filter(e=>e.length>0).join(` · `))+i+`</div><div class="dsh-pm-sn-comment-text">`+r(n)+`</div></div>`}).join(``)}function rn(e){let t=e.artifacts??[],n=j[e.stage]??[];if(t.length===0&&n.length===0)return``;let i=new Map;for(let e of t){let t=i.get(e.kind)??[];t.push(e),i.set(e.kind,t)}let a=[];for(let e of Ht){let t=i.get(e);if(!t||t.length===0)continue;let n=Vt[e]??e;if(e===`task_detail`){a.push(`<span class="dsh-pm-trace-node" data-kind="`+r(e)+`"><span class="dsh-pm-sn-dim">`+r(n)+`×`+t.length+`</span></span>`);continue}for(let i of t)a.push(`<span class="dsh-pm-trace-node" data-kind="`+r(e)+`"><button type="button" class="dsh-pm-sn-doc dsh-pm-trace-path" data-action="open-doc" data-path="`+r(i.path)+`">`+r(n)+`</button></span>`)}let o=new Set(t.map(e=>e.kind));for(let e of n)if(!o.has(e)){let t=Vt[e]??e;a.push(`<span class="dsh-pm-trace-node is-missing" data-kind="`+r(e)+`"><span class="dsh-pm-sn-doc is-missing">`+r(t)+`（缺失）</span></span>`)}return a.length===0?``:`<div class="dsh-pm-trace-chain">`+a.join(`<span class="dsh-pm-trace-arrow">→</span>`)+`</div>`}function an(e){if(!e.enabled)return`本分类跳过`;switch(e.stage){case`draft`:return`已立项`;case`brainstorming`:{let t=e.body.comments?.length??0;return t>0?t+` 条评论`:`需求分析`}case`planning`:{let t=e.body;return t.plan?t.plan.approvedAt===void 0?t.plan.rejectedAt===void 0?`计划待批准`:`计划被退回`:`计划已批准`:`待提交计划`}case`decomposing`:{let t=e.body.tasks?.length??0;return t>0?t+` 个任务`:`待拆分`}case`implementing`:{let t=e.body.tasks??[],n=t.length;if(n===0)return`暂无任务`;let r=t.filter(e=>e.status===`done`).length,i=t.find(e=>e.status===`in_progress`||e.status===`integrating`||e.status===`testing`),a=r+`/`+n+` 完成`;return n-r>0&&(a+=` · 剩 `+(n-r)+` 个`),i!==void 0&&(a+=` · 进行中 `+i.id),a}case`accepting`:{let t=e.body;return t.verification?t.verification.decision===`pass`?`验收通过`:t.verification.decision===`rework`?`验收被退回返工`:`待人工审核`:`待提交验收材料`}case`archived`:return`已归档`}}function on(e,t={}){Bt[e.stage]??e.stage;let n=e.enabled?t.state??`current`:`skipped`;Gt[n];let i=an(e),a=e.timeline.length>0?e.timeline[e.timeline.length-1].at:void 0,o=e.pendingConfirmation?`<div class="dsh-pm-sn-warn">⚠ 有产物待人工确认</div>`:``,s=tn[e.stage],c=e.enabled?s?s.renderBody(e):`<div class="dsh-pm-sn-empty">未知阶段</div>`:`<div class="dsh-pm-sn-body" data-stage="`+r(e.stage)+`"><div class="dsh-pm-sn-empty">该阶段在当前分类流程中不适用</div></div>`;return`<div class="dsh-pm-stage-panel dsh-pm-sn" data-stage="`+r(e.stage)+`" data-state="`+n+`"><div class="dsh-pm-sn-head"><span class="dsh-pm-sn-title">`+r(i)+`</span>`+(a===void 0?``:`<span class="dsh-pm-sn-time">`+r(K(a))+`</span>`)+`</div>`+rn(e)+o+c+nn(e.timeline)+`</div>`}function sn(e,t){let n=e.stages.find(e=>e.stage===t);if(n===void 0||!n.enabled)return`skipped`;let r=N.indexOf(e.currentStage),i=N.indexOf(t);return r>=0&&i<r?`done`:i===r?`current`:`pending`}function cn(e,t){let n=e.stages.find(e=>e.stage===t)??e.stages[0];return n===void 0?`<div class="dsh-pm-sn-empty">未知节点</div>`:on(n,{state:sn(e,n.stage)})}const ln=`dsh-pmboard:view`;function un(){try{return sessionStorage.getItem(ln)===`list`?`list`:`lanes`}catch{return`lanes`}}function dn(e){try{sessionStorage.setItem(ln,e)}catch{}}const fn=`dsh-pmboard:list`,Y=[`stage`,`progress`,`updated`,`created`,`title`];function pn(){let e={sortKey:`stage`,sortDir:`asc`,pageSize:10};try{let t=sessionStorage.getItem(fn);if(t===null)return e;let n=JSON.parse(t),r=typeof n.sortKey==`string`&&Y.includes(n.sortKey)?n.sortKey:e.sortKey;return{sortKey:r,sortDir:n.sortDir===`asc`||n.sortDir===`desc`?n.sortDir:V(r),pageSize:typeof n.pageSize==`number`&&B.includes(n.pageSize)?n.pageSize:10}}catch{return e}}function mn(e,t){let n=t.length>18?t.slice(0,18)+`…`:t;switch(e){case`archived`:return`该会话已归档（`+n+`）：日志保留、侧栏不可见，无法跳转`;case`missing`:return`该会话不在当前会话列表（`+n+`）：可能已删除或不在当前工作区`;case`unavailable`:return`会话服务暂不可用（页面注入未就绪），请刷新页面后重试`;case`opened`:return``;default:return`会话跳转结果未知：`+String(e)}}function hn(){return{openBoard:()=>{},closeBoard:()=>{},toggleBoard:()=>{},getSnapshot:()=>({boardOpen:!1}),refresh:()=>{}}}function gn(e){let t,n=[],r={kind:`board`},a=un(),s=pn(),l=s.sortKey,u=s.sortDir,d=s.pageSize,f=1,p,m,h,g=()=>({sortKey:l,sortDir:u,page:f,pageSize:d}),_=()=>Rt(),v=()=>{try{sessionStorage.setItem(fn,JSON.stringify({sortKey:l,sortDir:u,pageSize:d}))}catch{}},y=e=>{m!==void 0&&m.querySelectorAll(`[data-action="load-stage"]`).forEach(t=>{e!==void 0&&t.dataset.stage===e?t.setAttribute(`data-active`,`true`):t.removeAttribute(`data-active`)})},b=()=>{if(m!==void 0){if(t===void 0){m.innerHTML=T();return}switch(r.kind){case`board`:m.innerHTML=z(t,Date.now(),a,g(),_());break;case`req`:{let e=t.requirements.find(e=>e.id===r.reqId);m.innerHTML=e?Ke(e,t.tasks,Date.now(),_()):z(t,Date.now(),a,g(),_()),e?(y(p),D(),O(e.id,e.status)):r={kind:`board`};break}case`task`:{let e=t.tasks.find(e=>e.id===r.taskId),n=e?t.requirements.find(t=>t.id===e.requirementId):void 0;m.innerHTML=e?ot(e,n,Date.now(),t.tasks,_()):z(t,Date.now(),a,g(),_()),e||(r={kind:`board`});break}case`tasks`:m.innerHTML=fe(t);break;case`triage`:m.innerHTML=ze(n,t)}}},x=async()=>{try{let[e,r]=await Promise.all([mt(),ht()]);t=e,n=r.pending,b()}catch(e){m!==void 0&&(m.innerHTML=E(String(e)))}},S=()=>{h?.(),h=jt(()=>{x()})},C=e=>{let i=e.target,o=i.closest(`[data-pmpage]`);if(o!==null&&t!==void 0){let e=Number(o.dataset.pmpage);Number.isFinite(e)&&e>=1&&(f=e,b());return}let s=i.closest(`[data-action]`);if(s!==null&&t!==void 0)switch(s.dataset.action??``){case`refresh`:x();return;case`switch-view`:{let e=s.dataset.view;(e===`lanes`||e===`list`)&&(a=e,dn(e),r={kind:`board`},b());return}case`list-sort`:{let e=s.dataset.key;if(e===void 0||!Y.includes(e))return;e===l?u=u===`asc`?`desc`:`asc`:(l=e,u=V(e)),f=1,v(),b();return}case`open-doc`:{let e=s.dataset.path;e&&Lt(window.__dshPmCtx,e,It());return}case`switch-tab`:{let e=i.closest(`.dsh-pm-tab`);if(!e)return;let t=e.dataset.tab;if(!t)return;let n=e.closest(`.dsh-pm-detail`);if(!n)return;n.querySelectorAll(`.dsh-pm-tab`).forEach(e=>{e.classList.remove(`active`)}),e.classList.add(`active`),n.querySelectorAll(`.dsh-pm-tab-content`).forEach(e=>{e.classList.remove(`active`)});let r=n.querySelector(`.dsh-pm-tab-content[data-tab-content="${t}"]`);r&&r.classList.add(`active`);return}case`load-stage`:{let e=s.dataset.req,t=s.dataset.stage;e&&t&&(p=t,y(t),O(e,t));return}case`confirm-artifact`:{let e=s.dataset.id,t=s.dataset.kind;e&&t&&Tt({id:e,kind:t}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`submit-verdicts`:{let e=s.dataset.req,t=Number(s.dataset.version),n=s.closest(`.dsh-pm-vsheet`);if(!e||!Number.isFinite(t)||n===null)return;let r=[];if(n.querySelectorAll(`.dsh-pm-vitem`).forEach(e=>{let t=e.dataset.itemId;if(t===void 0)return;let n=e.querySelector(`input[type="radio"]:checked`);if(n===null)return;let i=e.querySelector(`.dsh-pm-vitem-opinion`)?.value.trim()??``;r.push({itemId:t,status:n.value===`passed`?`passed`:`failed`,...i.length>0?{opinion:i}:{}})}),r.length===0){window.alert(`请先逐项选择 通过/不通过`);return}St({id:e,version:t,verdicts:r}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`new-req`:{let e=window.prompt(`需求标题`);e&&e.trim()&&gt({title:e.trim()}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`open-req`:s.dataset.req&&(p=void 0,r={kind:`req`,reqId:s.dataset.req},b());return;case`open-task`:s.dataset.task&&(r={kind:`task`,taskId:s.dataset.task},b());return;case`open-tasks`:p=void 0,r={kind:`tasks`},b();return;case`new-task`:{let e=s.dataset.id;if(!e)return;let t=window.prompt(`任务标题`);t&&t.trim()&&Et({requirementId:e,title:t.trim(),phase:`implement`,side:`fullstack`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`back`:p=void 0,r={kind:`board`},b();return;case`back-req`:p=void 0,r={kind:`req`,reqId:s.dataset.req??``},b();return;case`move-req`:{let e=s.dataset.id??(r.kind===`req`?r.reqId:void 0),t=s.dataset.to;e&&t&&_t({id:e,to:t,actor:`human`,reason:s.dataset.id?`看板泳道卡面操作`:`需求详情页操作`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`add-comment`:{let e=(m?.querySelector(`[data-role="comment-input"]`))?.value.trim();e&&s.dataset.target&&s.dataset.id&&Dt({target:s.dataset.target,id:s.dataset.id,body:e,actor:`human`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`jump-session`:{let e=s.dataset.sid;if(!e)return;if(s.dataset.archived===`true`){window.alert(mn(`archived`,e));return}zt(W(),e).then(t=>{let n=mn(t,e);n!==``&&window.alert(n)}).catch(e=>window.alert(`会话跳转失败：`+String(e)));return}case`verify-pass`:{let e=s.dataset.id;e&&bt({id:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`verify-rework`:{let e=s.dataset.id;if(!e)return;let t=window.prompt(`退回返工的意见（窗口会按它整改）`);if(t===null)return;xt({id:e,note:t.trim()||`（未填意见）`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`archive-req`:{let e=s.dataset.id;e&&Ct({id:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`plan-approve`:{let e=s.dataset.id;e&&vt({id:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`plan-reject`:{let e=s.dataset.id;if(!e)return;let t=window.prompt(`退回理由（窗口会按它重写计划）`);if(t===null)return;yt({id:e,reason:t.trim()||`（未填理由）`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`triage-confirm`:{let e=s.dataset.triage;if(!e)return;let t=n.find(t=>t.id===e);if(t?.suggestedAction===`bind_req`&&t.suggestedTargetId)Ot({triageId:e,action:`bind_req`,targetId:t.suggestedTargetId}).then(()=>x()).catch(e=>window.alert(String(e)));else{let t=s.closest(`.dsh-pm-triage`),n=t?.querySelector(`[data-role="triage-title"]`)?.value.trim(),r=t?.querySelector(`[data-role="triage-category"]`)?.value;Ot({triageId:e,action:`create_req`,...n?{title:n}:{},...r?{category:r}:{}}).then(()=>x()).catch(e=>window.alert(String(e)))}return}case`triage-rebind`:{let e=m?.querySelector(`.dsh-pm-rebind-host`);e&&(e.style.display=`flex`,e.dataset.triage=s.dataset.triage??``);return}case`triage-rebind-confirm`:{let e=m?.querySelector(`.dsh-pm-rebind-host`),t=e?.dataset.triage,n=e?.querySelector(`[data-role="rebind-select"]`);t&&n?.value&&kt({triageId:t,targetId:n.value}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`triage-reject`:{let e=s.dataset.triage;e&&At({triageId:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}}},w=e=>{let t=e.target;if(t===null||typeof t.closest!=`function`)return;let n=t.closest(`[data-action="list-size"]`);if(n!==null){let e=Number(n.value);if(!B.includes(e))return;d=e,f=1,v(),b();return}},D=async()=>{if(m===void 0)return;let e=m.querySelectorAll(`[data-doc-path]`);await Promise.all(Array.from(e).map(async e=>{let t=e.dataset.docPath;if(!t)return;let n=!1;try{n=(await(await fetch(`/dashboard/api/reqboard/file?path=`+encodeURIComponent(t))).json()).success===!0}catch{n=!1}if(!n){e.classList.add(`is-missing`),e.setAttribute(`title`,`文件不存在（未落盘或路径错误）`);let t=e.querySelector(`[data-action="open-doc"]`);t&&(t.removeAttribute(`data-action`),t.classList.add(`dsh-pm-doc-missing`))}}))},O=async(e,t)=>{let n=document.getElementById(`dsh-pm-stage-detail-container`);if(n!==null){n.innerHTML=`<div class="dsh-pm-empty">详情加载中…</div>`;try{let r=await fetch(`/dashboard/api/reqboard/requirements/`+encodeURIComponent(e)+`/stages`,{signal:AbortSignal.timeout(8e3)});if(!r.ok){n.innerHTML=`<div class="dsh-pm-empty">详情暂不可用（HTTP `+r.status+`）</div>`;return}let i=await r.json();n.innerHTML=i.success===!0&&i.data!==void 0?cn(i.data,t):`<div class="dsh-pm-empty">详情暂不可用</div>`}catch(e){n.innerHTML=`<div class="dsh-pm-empty">详情加载失败：`+String(e)+`</div>`}}},k=i({prefix:`dsh-pm`,panelName:o,activeAttr:`data-dsh-pm-active`,otherActiveAttrs:c,pollMs:2e4,pauseOnHidden:!0,buildContainer:()=>{let e=document.createElement(`div`);return e.dataset.dshPmView=``,e.className=`dsh-pm-view`,e},onMount:e=>(m=e,e.addEventListener(`click`,C),e.addEventListener(`change`,w),x(),S(),()=>{e.removeEventListener(`click`,C),e.removeEventListener(`change`,w),h?.(),h=void 0,m=void 0}),onPoll:()=>{x()},onOpen:()=>{x()}}),A=e;return A.openBoard=k.open,A.closeBoard=k.close,A.toggleBoard=k.toggle,A.getSnapshot=()=>({boardOpen:k.isActive()}),A.refresh=()=>{x()},()=>{k.dispose()}}const _n=`dsh-pmboard/footer-action.css`,X=`dsh-pmboard:open-board`,vn={draft:`立项`,brainstorming:`需求分析`,planning:`技术设计`,decomposing:`拆分`,implementing:`实施中`,accepting:`待验收`,done:`完成`},yn=(0,e.createElement)(`svg`,{viewBox:`0 0 16 16`,width:`16`,height:`16`,fill:`none`,stroke:`currentColor`,"stroke-width":`1.4`,"stroke-linecap":`round`,"stroke-linejoin":`round`,"aria-hidden":`true`},(0,e.createElement)(`rect`,{x:`2`,y:`2`,width:`5`,height:`5`,rx:`1`}),(0,e.createElement)(`rect`,{x:`9`,y:`2`,width:`5`,height:`5`,rx:`1`}),(0,e.createElement)(`rect`,{x:`2`,y:`9`,width:`5`,height:`5`,rx:`1`}),(0,e.createElement)(`rect`,{x:`9`,y:`9`,width:`5`,height:`5`,rx:`1`}));function bn(){if(typeof document>`u`||document.querySelector(`style[data-plugin-css="${_n}"]`))return;let e=document.createElement(`style`);e.dataset.pluginCss=_n,e.textContent=`
.dsh-reqboard-foot {
...
`,document.head.appendChild(e)}function xn(t){let{wide:n}=t,r=s,[i,a]=(0,e.useState)(!1),[o,c]=(0,e.useState)(null),[l,u]=(0,e.useState)(null),d=(0,e.useRef)(0),f=(0,e.useRef)(null),p=(0,e.useRef)(null),m=(0,e.useRef)(new Set),h=(0,e.useCallback)(async e=>{if(!(!e&&d.current>0&&Date.now()-d.current<15e3))try{let e=await(await fetch(`/dashboard/api/reqboard/requirements/summary`,{signal:AbortSignal.timeout(8e3)})).json().catch(()=>({}));e.success===!0?(m.current=Rt(),c(e.data?.requirements??[]),u(null),d.current=Date.now()):u(e.error??`加载失败`)}catch(e){u(String(e))}},[]),g=(0,e.useCallback)(()=>{p.current!==null&&(window.clearTimeout(p.current),p.current=null),!i&&(f.current=window.setTimeout(()=>{a(!0),h(!1)},120))},[i,h]),_=(0,e.useCallback)(()=>{f.current!==null&&(window.clearTimeout(f.current),f.current=null),p.current=window.setTimeout(()=>a(!1),200)},[]),v=(0,e.useCallback)(e=>{let t=e.sourceSessionId;if(t===null||t.length===0||m.current.has(t)){window.dispatchEvent(new CustomEvent(X,{detail:{open:!0}}));return}zt(W(),t).then(e=>{e===`archived`?window.alert(`该会话已归档（日志保留，侧栏不可见）`):e===`missing`?window.alert(`该会话不在当前列表（可能已删除）`):e===`unavailable`&&window.alert(`会话服务暂不可用，已打开项目看板`),e!==`opened`&&window.dispatchEvent(new CustomEvent(X,{detail:{open:!0}}))})},[]),y=(0,e.createElement)(`button`,{type:`button`,className:n?`dsh-reqboard-foot wide`:`dsh-reqboard-foot rail`,title:`${r}（悬停查看进行中需求）`,"aria-label":r,onClick:()=>{window.dispatchEvent(new CustomEvent(X,{detail:{open:!0}}))}},n?[(0,e.createElement)(`span`,{className:`dsh-reqboard-foot-icon`,key:`i`},yn),(0,e.createElement)(`span`,{className:`dsh-reqboard-foot-label`,key:`l`},r)]:(0,e.createElement)(`span`,{className:`dsh-reqboard-foot-icon`,key:`i`},yn));if(!i)return(0,e.createElement)(`div`,{className:`dsh-reqboard-foot-wrap${n?``:` rail`}`,onMouseEnter:g,onMouseLeave:_},y);let b=o===null?(0,e.createElement)(`div`,{className:`dsh-reqboard-empty`},l===null?`加载中…`:`加载失败：${l}`):o.length===0?(0,e.createElement)(`div`,{className:`dsh-reqboard-empty`},`暂无进行中的需求`):o.map(t=>{let n=Number.isFinite(t.percentage)?t.percentage:0,r=t.sourceSessionId,i=r!==null&&r.length>0,a=i&&m.current.has(r);return(0,e.createElement)(`button`,{key:t.id,type:`button`,className:`dsh-reqboard-item${a?` is-archived`:``}`,title:a?`${t.id}《${t.title}》— 来源会话 ${r} 已归档，无法跳转（点击打开看板）`:i?`${t.id}《${t.title}》— 点击跳转到会话 ${r}`:`${t.id}《${t.title}》— 人工建卡，无来源会话（点击打开看板）`,onClick:()=>v(t)},(0,e.createElement)(`div`,{className:`dsh-reqboard-item-top`,key:`top`},[(0,e.createElement)(`span`,{className:`dsh-reqboard-item-title`,key:`t`},`${t.id} ${t.title}`),(0,e.createElement)(`span`,{className:`dsh-reqboard-badge`,"data-status":t.status,key:`b`},vn[t.status]??t.status)]),(0,e.createElement)(`div`,{className:`dsh-reqboard-item-meta`,key:`meta`},[t.windowCode===null?(0,e.createElement)(`span`,{key:`w`},`人工建卡`):(0,e.createElement)(`span`,{className:`dsh-reqboard-win${a?` is-archived`:``}`,key:`w`,title:a?`该会话已归档`:void 0},t.windowCode),(0,e.createElement)(`span`,{key:`s`},t.tasksTotal>0?`${t.tasksTotal} 任务${t.tasksActive>0?` · ${t.tasksActive} 进行中`:``}`:`未拆分`),(0,e.createElement)(`span`,{className:`dsh-reqboard-jump${a?` is-archived`:``}`,key:`j`},a?`已归档`:i?`跳转 →`:`看板 →`)]),(0,e.createElement)(`div`,{className:`dsh-reqboard-bar`,key:`bar`},[(0,e.createElement)(`span`,{className:`dsh-reqboard-bar-track`,key:`tr`},(0,e.createElement)(`i`,{className:`dsh-reqboard-bar-fill`,style:{width:`${n}%`}})),(0,e.createElement)(`span`,{className:`dsh-reqboard-bar-num`,key:`n`},`${t.tasksDone}/${t.tasksTotal}`)]))}),x=(0,e.createElement)(`div`,{className:`dsh-reqboard-drop`,onMouseEnter:g,onMouseLeave:_},[(0,e.createElement)(`div`,{className:`dsh-reqboard-drop-head`,key:`h`},[(0,e.createElement)(`span`,{key:`a`},`进行中需求`),(0,e.createElement)(`span`,{className:`n`,key:`b`},o===null?``:String(o.length))]),b,(0,e.createElement)(`div`,{className:`dsh-reqboard-drop-foot`,key:`f`},[(0,e.createElement)(`span`,{key:`l`},`点击条目跳转会话`),(0,e.createElement)(`button`,{key:`r`,type:`button`,onClick:()=>{h(!0)}},`刷新`)])]);return(0,e.createElement)(`div`,{className:`dsh-reqboard-foot-wrap${n?``:` rail`}`,onMouseEnter:g,onMouseLeave:_},[y,x])}const Z=[{key:`draft`,label:`立项`},{key:`brainstorming`,label:`需求分析`},{key:`planning`,label:`技术设计`},{key:`decomposing`,label:`拆分`},{key:`implementing`,label:`实施`},{key:`accepting`,label:`验收`},{key:`archived`,label:`归档`}],Q={draft:`立项`,brainstorming:`需求分析`,planning:`技术设计`,decomposing:`拆分`,implementing:`实施中`,accepting:`待验收`,done:`完成`,archived:`归档`,canceled:`已取消`};function Sn(e){if(typeof e==`string`&&e.length>0)return e;try{let e=window,t=(e.__dshPmSessions??e.__dshPmCtx?.sessions)?.list?.getSnapshot?.(),n=t?.current??t?.currentSessionId;if(typeof n==`string`&&n.length>0)return n}catch{}}function Cn(e,t){return t<0?`pending`:e<t?`done`:e===t?`current`:`pending`}function wn(t){let n=t?.sessionId,[r,i]=(0,e.useState)(null),[a,o]=(0,e.useState)(!1),[s,c]=(0,e.useState)(null),[l,u]=(0,e.useState)(null),[d,f]=(0,e.useState)(!1),[p,m]=(0,e.useState)(``),h=(0,e.useRef)(null),g=(0,e.useRef)(void 0);(0,e.useEffect)(()=>{let e=!0,t=async()=>{let t=Sn(n);if(t===void 0){e&&(g.current=void 0,i(null));return}t!==g.current&&(g.current=t,e&&i(null));try{let n=await fetch(`/dashboard/api/reqboard/session/${encodeURIComponent(t)}/progress`,{signal:AbortSignal.timeout(8e3)});if(!n.ok)return;let r=await n.json();if(!e)return;i(r.success===!0?r.data??null:null)}catch{}};t();let r=window.setInterval(()=>{t()},15e3);return()=>{e=!1,window.clearInterval(r)}},[n]),(0,e.useEffect)(()=>{if(!a)return;let e=e=>{let t=h.current;t!==null&&!t.contains(e.target)&&o(!1)};return document.addEventListener(`mousedown`,e),()=>document.removeEventListener(`mousedown`,e)},[a]);let _=r?.requirement?.id,v=r?.requirement?.updatedAt;(0,e.useEffect)(()=>{if(!a||_===void 0){u(null);return}let e=!0;return f(!0),m(``),wt(_).then(t=>{e&&u(t)}).catch(t=>{e&&m(t.message)}).finally(()=>{e&&f(!1)}),()=>{e=!1}},[a,_,v]),(0,e.useEffect)(()=>{let e=e=>{let t=e.target.closest(`[data-action="open-doc"]`);if(t===null)return;e.preventDefault(),e.stopPropagation();let r=t.getAttribute(`data-path`);r!==null&&r.length!==0&&Lt(window.__dshPmCtx,r,Sn(n))};return document.addEventListener(`click`,e),()=>document.removeEventListener(`click`,e)},[n]);let y=r?.requirement;if(r===null||r.hasRequirement!==!0||y==null)return null;r.progress?.percentage;let b=r.progress?.done??0,x=r.progress?.total??0,S=y.status??`draft`,C=Z.findIndex(e=>e.key===S),w=y.title??`（未命名需求）`,T=r.closed===!0,E=y.category??`feature`,D=P[E]??P.feature,O=new Set(Z.filter(e=>!D.stages.includes(e.key)).map(e=>e.key)),k=[];Z.forEach((t,n)=>{let r=Cn(n,C),i=s===t.key,a=O.has(t.key);k.push((0,e.createElement)(`div`,{key:`n-${t.key}`,className:`dsh-pm-flow-node`,"data-state":a?`skipped`:r,"data-selected":i?`true`:void 0,title:a?`本分类（${E}）跳过「${t.label}」`:void 0,onClick:e=>{e.stopPropagation(),c(t.key),o(!0)}},[(0,e.createElement)(`span`,{key:`d`,className:`dsh-pm-flow-dot`},a?`—`:r===`done`?`✓`:r===`current`?`●`:n+1),(0,e.createElement)(`span`,{key:`l`,className:`dsh-pm-flow-label`},t.label)])),n<Z.length-1&&k.push((0,e.createElement)(`div`,{key:`l-${t.key}`,className:`dsh-pm-flow-link`,"data-state":n<C?`done`:`pending`}))});let A=(0,e.createElement)(`div`,{key:`flow`,className:`dsh-pm-cprog-inline${T?` is-closed`:``}`,title:`${y.id??``}《${w}》${Q[S]??S}${T?`（本会话最近完成）`:``} · 点击节点查看详情`,"aria-expanded":a},[(0,e.createElement)(`div`,{key:`f`,className:`dsh-pm-flow`},k),(0,e.createElement)(`span`,{key:`c`,className:`dsh-pm-cprog-inline-count`},x>0?`${b}/${x}`:Q[S]??S)]);if(!a)return(0,e.createElement)(`div`,{className:`dsh-pm-cprog`,ref:h},A);let j=(0,e.createElement)(`div`,{key:`panel`,className:`dsh-pm-cprog-detail-panel`},[(0,e.createElement)(`div`,{key:`h`,className:`dsh-pm-cprog-panel-head`},[(0,e.createElement)(`span`,{key:`t`,className:`dsh-pm-cprog-panel-title`},`${y.id??``} · ${w}`),(0,e.createElement)(`span`,{key:`s`,className:`dsh-pm-status-badge`,"data-status":S},Q[S]??S)]),T?(0,e.createElement)(`div`,{key:`closed`,className:`dsh-pm-cprog-panel-note`},`本会话已无进行中需求 —— 以上是最近关联的需求（已完成/已归档），可作为「这个会话做了什么」的回顾。`):null,(0,e.createElement)(`div`,{key:`ov`,className:`dsh-pm-cprog-sec`},[d&&l===null?(0,e.createElement)(`div`,{key:`ld`,className:`dsh-pm-cprog-empty`},`详情加载中…`):p.length>0&&l===null?(0,e.createElement)(`div`,{key:`er`,className:`dsh-pm-cprog-empty`},`详情暂不可用：`+p):l===null?(0,e.createElement)(`div`,{key:`ne`,className:`dsh-pm-cprog-empty`},`暂无详情`):(0,e.createElement)(`div`,{key:`ovr`,className:`dsh-pm-cprog-stage-detail`,dangerouslySetInnerHTML:{__html:cn(l,s??l.currentStage)}})]),(0,e.createElement)(`div`,{key:`ft`,className:`dsh-pm-cprog-foot`},[(0,e.createElement)(`button`,{key:`open`,type:`button`,className:`dsh-pm-btn sm`,onClick:()=>{window.dispatchEvent(new CustomEvent(X,{detail:{open:!0}}))}},`打开项目看板`),(0,e.createElement)(`button`,{key:`rf`,type:`button`,className:`dsh-pm-btn sm`,onClick:()=>{o(!1),c(null)}},`收起`)])]);return(0,e.createElement)(`div`,{className:`dsh-pm-cprog`,ref:h},[A,j])}const $=`dsh-pmboard/styles.css`;function Tn(){if(typeof document>`u`||document.querySelector(`style[data-plugin-css="${$}"]`))return;let e=document.createElement(`style`);e.dataset.pluginCss=$,e.textContent=`
/* ================================================================== */
...
`,document.head.appendChild(e)}const En=[`slots`,`sessions`,`workspaces`,`sidebarRight`];function Dn(e){try{bn(),Tn(),window.__dshReqboardClient?.dispose(),window.__dshPmCtx=e,window.__dshPmSessions=e.sessions,window.__dshPmWorkspaces=e.workspaces;let t=hn(),n=gn(t),r=e=>{e.detail?.open===!0?t.getSnapshot().boardOpen?t.closeBoard():t.openBoard():t.toggleBoard()};window.addEventListener(X,r),window.__dshReqboardClient={dispose:()=>{window.removeEventListener(X,r),n(),t.closeBoard(),delete window.__dshPmCtx,delete window.__dshPmSessions,delete window.__dshPmWorkspaces}};let i=e.slots;if(i){try{i.inject(`sidebar.footer.action`,()=>i.register({name:`sidebar.footer.action`,id:o,order:110,label:s},xn))}catch(e){console.error(`[dsh-pmboard] Failed to register sidebar.footer.action:`,e)}try{i.inject(`conversation.session.header.utilities`,()=>i.register({name:`conversation.session.header.utilities`,id:`dsh-pmboard:progress`,order:5,inject:e=>({sessionId:e})},wn))}catch(e){console.error(`[dsh-pmboard] Failed to register conversation.session.header.utilities:`,e)}}else console.warn(`[dsh-pmboard] ctx.slots unavailable`)}catch(e){console.error(`[dsh-pmboard] client half failed to start:`,e)}}exports.apply=Dn,exports.inject=En,exports.name=`dsh-pmboard/client`;