...
`)}function C(e){let t=e.createdBy,n=t?.kind??`human`;if(n===`human`)return{actor:`human`,text:`人`};if(n===`system`)return{actor:`system`,text:`系统`};let r=t?.sessionId;return{actor:`agent`,text:r!==void 0&&r.length>0?`窗口 `+m(r):`窗口`}}function w(e){return e.length===0?`<div class="dsh-pm-empty">暂无评论</div>`:`<div class="dsh-pm-comments">`+e.map(e=>{let t=C(e);return`
    ...
    <div class="dsh-pm-list-toolbar">
      ...
      <select class="dsh-pm-pagesize" data-action="list-size" title="每页条数">${z.map(e=>`<option value="${e}"${e===a?` selected`:``}>${e}</option>`).join(``)}</select>
    ...
    </div>`}function V(e){if(e.length===0)return`<div class="dsh-pm-injection-info"><div class="dsh-pm-empty">尚无记录</div></div>`;let t=e.slice().reverse().map(Ze).join(``);return`<div class="dsh-pm-injection-info" data-count="${e.length}">
      <div class="dsh-pm-injection-info-head">本次注入（最近 ${e.length} 条）</div>
      ${t}
    </div>`}function Qe(e){let t=qe(e);return`<div class="dsh-pm-progress-dots">${Ke.map(e=>{let{label:n,order:i}=Ge[e];return`
        <div class="dsh-pm-dot-wrapper ${i<t?`completed`:i===t?`current`:``}">
          <div class="dsh-pm-dot"></div>
          <span class="dsh-pm-dot-label">${r(n)}</span>
        </div>`}).join(``)}</div>`}function $e(){return`
    ...
    </div>`}function et(e,t,n,i,a){return`
    ...
    <div class="dsh-pm-tab-content active" data-tab-content="overview">
      ...
      <details class="dsh-pm-fold">
        ...
        <div class="dsh-pm-section-content" id="dsh-pm-injection-info-container">${V([])}</div>
      </details>
      <details class="dsh-pm-fold">
    ...
    </div>`}function tt(e,t,n=Date.now(),i=_){let a=t.filter(t=>t.requirementId===e.id),o=it(a),s=w(e.comments),c=nt(e.status),u=rt(e);return`
    <div class="dsh-pm-detail" data-detail-req="${r(e.id)}">
      ...
      ${$e()}
      ${et(e,a,o,s,n)}
    </div>`}function nt(e){let t={draft:`已立项：窗口接手开工后自动进入需求分析，人可在上方操作条手动催办。`,brainstorming:`需求分析中：窗口 agent 会自行推进到技术设计，人可在上方操作条确认方案或退回立项。`,planning:`技术设计中：计划提交后请在上方操作条点「批准计划」——批准前拆分会被告代码级拒绝。`,decomposing:`拆分中：任务落库/开工后系统自动推进到实施，人可在上方操作条确认拆分。`,implementing:`实施中：任务全部完成时自动进入验收，人可在上方操作条提交验收。`,accepting:`验收中：看完验收材料后，在上方操作条点「验收通过」或「退回返工」。`,done:`已完成（历史状态）：验收通过现已直接归档，此状态仅存在于旧记录。`}[e];return t===void 0?``:`<div class="dsh-pm-gate">`+t+`</div>`}function rt(e){let t=[],n=(n,i,a,o=!1)=>{let s=o?`dsh-pm-btn primary`:`dsh-pm-btn`;t.push(`<button type="button" class="`+s+`" data-action="`+n+`" data-id="`+r(e.id)+`" title="`+r(a)+`">`+i+`</button>`)},i=(n,i,a,o=!1)=>{let s=o?`dsh-pm-btn primary`:`dsh-pm-btn`;t.push(`<button type="button" class="`+s+`" data-action="move-req" data-to="`+n+`" data-id="`+r(e.id)+`" title="`+r(a)+`">`+i+`</button>`)};switch(e.status){case`draft`:i(`brainstorming`,`→ 需求分析`,`进入需求分析；窗口接手开工时会自动进入`,!0),i(`canceled`,`取消`,`取消该需求（仅人可操作）`);break;case`brainstorming`:i(`planning`,`→ 技术设计`,`方案谈定 → 进入技术设计；请提交计划并待批准`,!0),i(`draft`,`退回立项`,`方案要重谈 → 退回立项`);break;case`planning`:i(`decomposing`,`→ 拆分`,`计划获批后落库任务卡；未获批会被代码级拒绝`,!0),i(`brainstorming`,`退回重谈`,`方案要改 → 退回需求分析`);break;case`decomposing`:i(`implementing`,`→ 实施`,`确认拆分，进入实施；任务开工时系统会自动推进`,!0);break;case`implementing`:i(`accepting`,`→ 验收`,`提交验收；任务全部完成时系统会自动推进`,!0)}return e.plan!==void 0&&e.plan.approvedAt===void 0&&(n(`plan-approve`,`批准计划`,`批准实施计划，解锁 reqboard_decompose 拆分`,!0),n(`plan-reject`,`退回计划`,`退回实施计划（窗口按理由重写）`)),e.status===`accepting`&&e.verification!==void 0&&(n(`verify-pass`,`验收通过`,`人工审核通过，需求进入完成`,!0),n(`verify-rework`,`退回返工`,`退回返工（需填写意见）`)),e.status===`done`&&e.archive!==void 0&&e.archive.archivedAt===void 0&&n(`archive-req`,`归档`,`归档：把产出并进项目文档`,!0),t.length===0?``:`<div class="dsh-pm-action-bar" data-req="`+r(e.id)+`"><span class="dsh-pm-action-bar-label">本阶段操作</span>`+t.join(``)+`</div>`}function it(e){if(e.length===0)return`<div class="dsh-pm-empty">暂无任务</div>`;let t=new Map,n=new Map(e.map(e=>[e.id,e])),i=(e,r)=>{if(t.has(e.id))return t.get(e.id);if(r.has(e.id))return 0;r.add(e.id);let a=e.dependsOn.filter(e=>n.has(e)),o=a.length===0?0:1+Math.max(...a.map(e=>i(n.get(e),r)));return t.set(e.id,o),o};e.forEach(e=>i(e,new Set));let a=Math.max(...t.values()),o=Array.from({length:a+1},()=>[]);return e.forEach(e=>o[t.get(e.id)].push(e)),`<div class="dsh-pm-dag"><div class="dsh-pm-dag-title">🔀 任务依赖关系</div><div class="dsh-pm-dag-layers">`+o.map((e,t)=>`
    ...
    </div>`).join(``)+`</div></div>`}function at(e){if(e.length===0)return`<div class="dsh-pm-empty">暂无任务</div>`;let t=e=>e===`done`?`✓`:e===`in_progress`?`◐`:e===`todo`?`○`:`◐`;return`
    ...
    ${s}
    ${l}
    ${u}`}function lt(e){let t=0,n=0,i=0,a=0,o=[],s=e.executions[e.executions.length-1];s?.evidence&&s.evidence.forEach(e=>{let r=e.match(/(\d+)\s*passed.*?(\d+)\s*failed.*?(\d+)\s*skipped/i);r&&(n=parseInt(r[1],10),i=parseInt(r[2],10),a=parseInt(r[3],10),t=n+i+a)}),s?.error&&s.error.split(`
`).forEach(e=>{let t=e.match(/(.+?):(\d+)\s*Expected:\s*(.+?)\s*Actual:\s*(.+)/);t&&o.push({name:`测试用例`,file:t[1]+`:`+t[2],expected:t[3],actual:t[4]})});let c=t>0?(n/t*100).toFixed(1):`0`,l=o.length>0?o.map(e=>`
    ...
    ${f}
    ${p}`}function ft(e){let t=e.title.toLowerCase();if(t.includes(`拆分`)||t.includes(`decompose`))return`decompose`;if(e.status===`integrating`||t.includes(`集成`)||t.includes(`联调`))return`merge`;switch(e.phase){case`doc`:return`doc`;case`ui`:return`ui`;case`analysis`:return`analysis`;case`implement`:return`implement`;case`test`:return`test`;case`review`:return`review`;case`merge`:return`merge`;default:return`generic`}}const pt={decompose:`🔀`,implement:`⚙️`,test:`🧪`,review:`👀`,merge:`🔀`,doc:`📝`,ui:`🎨`,analysis:`🔍`,generic:`📋`},mt={decompose:`拆分任务`,implement:`实施任务`,test:`测试任务`,review:`评审任务`,merge:`合并任务`,doc:`文档任务`,ui:`UI设计`,analysis:`分析任务`,generic:`任务`};function ht(e,t,n=Date.now(),i=[],a=_){let o=ft(e),s=pt[o],c=mt[o],l=gt(e,o,t,i),d=_t(e,n,a);return`
    <div class="dsh-pm-taskdetail" data-detail-task="${r(e.id)}" data-node-type="${o}">
      ...
      ${l}
      ${d}
    </div>`}function gt(e,t,n,r){switch(t){case`decompose`:return vt(e,n,r);case`implement`:return yt(e);case`test`:return lt(e);case`review`:return ut(e);case`merge`:return dt(e);case`doc`:return ot(e);case`ui`:return st(e);case`analysis`:return ct(e);default:return``}}function _t(e,t,n=_){let i=e.executions.map(e=>`
    ...
    </details>`}function vt(e,t,n){if(!t)return`<div class="dsh-pm-detail-section"><div class="dsh-pm-empty">需求数据不可用</div></div>`;let i=n.filter(e=>e.requirementId===t.id),a=i.length,o=i.filter(e=>e.status===`done`).length,s={};return i.forEach(e=>{let t=e.side===`frontend`?`UI 轨道`:e.side===`backend`?`后端轨道`:e.side===`doc`?`文档轨道`:`全栈轨道`;s[t]||(s[t]=[]),s[t].push(e)}),`
    <div class="dsh-pm-detail-section dsh-pm-specialized">
      <h3>📊 拆分结果</h3>
      <div class="dsh-pm-stats">
        <div class="dsh-pm-stat">
    ...
    <div class="dsh-pm-detail-section">
      <h3>🌳 依赖关系 DAG</h3>
      ${it(i)}
    </div>`}function yt(e){let t=[],n=e.executions[e.executions.length-1];n?.evidence&&n.evidence.forEach(e=>{let n=e.match(/^(.+?)\s*\(?\+(\d+)(?:,\s*-(\d+))?\)?$/);n&&t.push({path:n[1].trim(),added:parseInt(n[2],10),deleted:parseInt(n[3]||`0`,10)})});let i=t.reduce((e,t)=>e+t.added,0),a=t.reduce((e,t)=>e+t.deleted,0),o=t.length>0?t.map(e=>`
    ...
    </div>`}const bt=8e3;var xt=class extends Error{code;constructor(e,t){super(e),this.code=t}};async function St(e){let t=await e;if(!t.ok)throw new xt(`HTTP `+t.status);let n=await t.json().catch(()=>({}));if(n.success!==!0)throw new xt(n.error??`API 返回失败`,n.code);return n.data}const H=e=>St(fetch(e,{signal:AbortSignal.timeout(bt)})),U=(e,t)=>St(fetch(e,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t),signal:AbortSignal.timeout(bt)})),Ct=()=>H(`/dashboard/api/reqboard/`),wt=()=>H(`/dashboard/api/reqboard/triage`);function Tt(e,t=20){let n=new URLSearchParams({k:String(t)});return e!==void 0&&e.length>0&&n.set(`window`,e),H(`/dashboard/api/reqboard/injection-log?`+n.toString())}function Et(e){return U(`/dashboard/api/reqboard/req/create`,e)}function Dt(e){return U(`/dashboard/api/reqboard/req/move`,e)}function Ot(e){return U(`/dashboard/api/reqboard/req/plan/approve`,e)}function kt(e){return U(`/dashboard/api/reqboard/req/plan/reject`,e)}function At(e){return U(`/dashboard/api/reqboard/req/verify/pass`,e)}function jt(e){return U(`/dashboard/api/reqboard/req/verify/rework`,e)}function Mt(e){return U(`/dashboard/api/reqboard/req/verdicts`,e)}function Nt(e){return U(`/dashboard/api/reqboard/req/archive`,e)}function Pt(e){return H(`/dashboard/api/reqboard/requirements/`+encodeURIComponent(e)+`/stages`)}function Ft(e){return U(`/dashboard/api/reqboard/req/artifact/confirm`,e)}function It(e){return U(`/dashboard/api/reqboard/task/create`,e)}function Lt(e){return U(`/dashboard/api/reqboard/comment`,e)}function Rt(e){return U(`/dashboard/api/reqboard/triage/confirm`,e)}function zt(e){return U(`/dashboard/api/reqboard/triage/rebind`,e)}function Bt(e){return U(`/dashboard/api/reqboard/triage/reject`,e)}function Vt(e){let t=new EventSource(`/dashboard/api/reqboard/events`);return t.onmessage=t=>{try{let n=JSON.parse(t.data);e(n.revision,n.kind)}catch{}},()=>t.close()}function Ht(e){return encodeURIComponent(e).replace(/%3A/gi,`:`)}function Ut(e){return e.split(`/`).map(Ht).join(`/`)}function Wt(e,t){let n=t.replace(/\\/g,`/`).replace(/^(?:\.\/)+/,``);return`dsh-resource://file/session/${Ht(e)}/${Ut(n)}`}function Gt(e){let t=e?.sidebarRight;if(!(typeof t!=`object`||!t))return t}function Kt(){try{let e=window,t=(e.__dshPmSessions??e.__dshPmCtx?.sessions)?.list?.getSnapshot?.(),n=t?.current??t?.currentSessionId;if(typeof n==`string`&&n.length>0)return n}catch{}}function qt(e,t,n){if(typeof n!=`string`||n.length===0)return console.error(`[dsh-pmboard] open-doc: 取不到会话 id，无法构造 session 地址`,{path:t}),!1;let r=Gt(e);if(r===void 0||typeof r.openResource!=`function`)return console.error(`[dsh-pmboard] open-doc: ctx.sidebarRight 不可用（官方右侧栏未加载）`,{path:t}),!1;let i=Wt(n,t);try{return r.openResource(i),!0}catch(e){return console.error(`[dsh-pmboard] open-doc: openResource 调用失败`,{address:i,error:String(e)}),!1}}function W(){let e=()=>window;return{getSessions:()=>{try{let t=e().__dshPmSessions??e().__dshPmCtx?.sessions;if(t&&typeof t.open==`function`&&t.list)return t}catch{}},getWorkspaces:()=>{try{let t=e().__dshPmWorkspaces??e().__dshPmCtx?.workspaces;if(t&&t.list)return t}catch{}}}}function Jt(e=W()){try{let t=e.getWorkspaces()?.list.getSnapshot().archivedSessionIds??[];return new Set(t)}catch{return new Set}}async function Yt(e,t){let n=e.getSessions();if(n===void 0)return`unavailable`;let r=e=>{try{return n.list.getSnapshot().byId[e]!==void 0}catch{return!1}};if(r(t))return(e.getWorkspaces()?.list.getSnapshot().archivedSessionIds??[]).includes(t)?`archived`:(n.open(t),`opened`);try{await n.refresh()}catch{}return r(t)?(e.getWorkspaces()?.list.getSnapshot().archivedSessionIds??[]).includes(t)?`archived`:(n.open(t),`opened`):`missing`}const Xt={draft:`立项`,brainstorming:`需求分析`,planning:`技术设计`,decomposing:`拆分`,implementing:`实施`,accepting:`验收`,archived:`归档`},Zt={requirement:`需求文档`,plan:`实施计划`,decomposition:`拆分方案`,task_detail:`任务卡`,verification:`验收材料`,archive:`归档材料`},Qt=[`requirement`,`plan`,`decomposition`,`task_detail`,`verification`,`archive`],$t={todo:`待开始`,in_progress:`开发中`,integrating:`联调中`,testing:`测试中`,in_review:`待评审`,done:`已完成`,canceled:`已取消`},en={todo:`○`,in_progress:`◐`,integrating:`◐`,testing:`◐`,in_review:`◐`,done:`✓`,canceled:`✕`};function G(e){let t=new Date(e),n=e=>String(e).padStart(2,`0`);return t.getFullYear()+`-`+n(t.getMonth()+1)+`-`+n(t.getDate())+` `+n(t.getHours())+`:`+n(t.getMinutes())}function K(e,t=Date.now()){let n=t-e;if(n<6e4)return`刚刚`;if(n<36e5)return Math.floor(n/6e4)+` 分钟前`;if(n<864e5)return Math.floor(n/36e5)+` 小时前`;let r=new Date(e),i=e=>String(e).padStart(2,`0`);return n<1728e5?`昨天 `+i(r.getHours())+`:`+i(r.getMinutes()):i(r.getMonth()+1)+`-`+i(r.getDate())}function q(e,t){return e.length>t?e.slice(0,t)+`…`:e}function J(e){if(e===void 0)return``;if(e.kind===`human`)return`人`;if(e.kind===`system`)return`系统`;let t=e.sessionId;return typeof t!=`string`||t.length===0?`Agent`:t.startsWith(`session-`)?ie(t):`sub `+ie(t)}function tn(e){let t=e.executions??[],n=t.length>0?t[t.length-1].sessionId:e.claimedBy;return J(n===void 0?void 0:{kind:`agent`,sessionId:n})}function nn(e){let t=e.executions??[],n=t.filter(e=>e.outcome===`failed`).length;if(e.status===`done`){let e=[...t].reverse().find(e=>e.outcome===`succeeded`&&e.endedAt!==void 0),n=e?.endedAt===void 0?``:G(e.endedAt);return{text:(n.length>0?n+` `:``)+`完成`,failed:!1}}if(e.status===`in_progress`||e.status===`integrating`||e.status===`testing`){let r=[...t].reverse().find(e=>e.outcome===`running`),i=r===void 0?``:`（`+K(r.startedAt)+`开始）`,a=n>0?` · 失败 `+n+` 次`:``;return{text:($t[e.status]??e.status)+i+a,failed:n>0}}return n>0?{text:`失败 `+n+` 次`,failed:!0}:{text:$t[e.status]??e.status,failed:!1}}const rn=e=>{let t=e.body,n=[t.sourceWindow?`来源窗口 `+t.sourceWindow:``,t.createdAt?G(t.createdAt):``].filter(e=>e.length>0).join(` · `);return`<div class="dsh-pm-sn-body" data-stage="draft"><div class="dsh-pm-sn-req-title">`+r(t.title)+(t.category?` <span class="dsh-pm-sn-dim">（`+r(t.category)+`）</span>`:``)+`</div>`+(t.description?`<div class="dsh-pm-sn-text">`+r(t.description)+`</div>`:`<div class="dsh-pm-sn-empty">暂无描述</div>`)+(n.length>0?`<div class="dsh-pm-sn-dim">`+r(n)+`</div>`:``)+`</div>`},an=e=>{let t=e.body.comments??[];return t.length===0?`<div class="dsh-pm-sn-body" data-stage="brainstorming"><div class="dsh-pm-sn-empty">暂无评论</div></div>`:`<div class="dsh-pm-sn-body" data-stage="brainstorming">`+t.map(e=>{let t=J(e.createdBy);return`<div class="dsh-pm-sn-comment"><div class="dsh-pm-sn-comment-who" data-actor="`+r(e.createdBy?.kind??`agent`)+`">`+r(t)+`</div><div class="dsh-pm-sn-comment-text">`+r(e.body)+`</div></div>`}).join(``)+`</div>`},on=e=>{let t=e.body;if(!t.plan)return`<div class="dsh-pm-sn-body" data-stage="planning"><div class="dsh-pm-sn-empty">尚未提交实施计划</div></div>`;let n=t.plan,i=n.approvedAt===void 0?n.rejectedAt===void 0?`提交：`+G(n.submittedAt)+` · 待批准`:`退回：`+G(n.rejectedAt)+(n.rejectedReason?` · `+n.rejectedReason:``):`批准：`+(n.approvedBy?.kind===`human`?`人`:`Agent`)+` · `+G(n.approvedAt);return`<div class="dsh-pm-sn-body" data-stage="planning">`+(n.summary?`<div class="dsh-pm-sn-text">`+r(n.summary)+`</div>`:``)+`<div class="dsh-pm-sn-dim">`+n.tasks.length+` 个任务 · `+r(i)+`</div></div>`};function sn(e){let t=new Map(e.map(e=>[e.id,e])),n=new Map;function r(e){if(n.has(e))return n.get(e);let i=t.get(e);if(!i||!i.dependsOn||i.dependsOn.length===0)return n.set(e,0),0;let a=1+Math.max(...i.dependsOn.map(e=>r(e)));return n.set(e,a),a}for(let t of e)r(t.id);let i=new Map;for(let t of e){let e=n.get(t.id)??0;i.has(e)||i.set(e,[]),i.get(e).push(t)}return i}const cn=e=>{let t=e.body.tasks??[];if(t.length===0)return`<div class="dsh-pm-sn-body" data-stage="decomposing"><div class="dsh-pm-sn-empty">尚未拆分任务</div></div>`;let n=sn(t),i=[];for(let[e,t]of[...n.entries()].sort((e,t)=>e[0]-t[0])){let n=e===0?`第 1 层 · 无依赖`:`第 `+(e+1)+` 层`+(t.length>1?` · `+t.length+` 个可并行`:``),a=t.map(e=>`<div class="dsh-pm-sn-dag-task"><span class="dsh-pm-sn-task-id">`+r(e.id)+`</span><span class="dsh-pm-sn-text">`+r(e.title)+`</span></div>`).join(``);i.push(`<div class="dsh-pm-sn-dag-layer"><div class="dsh-pm-sn-dag-label">`+r(n)+`</div>`+a+`</div>`)}return`<div class="dsh-pm-sn-body" data-stage="decomposing">`+i.join(``)+`</div>`},ln=e=>{let t=e.body,n=t.tasks??[];if(n.length===0)return`<div class="dsh-pm-sn-body" data-stage="implementing"><div class="dsh-pm-sn-empty">暂无执行任务</div></div>`;let i=n.filter(e=>e.status===`in_progress`||e.status===`integrating`||e.status===`testing`),a=n.filter(e=>e.status===`done`),o=n.filter(e=>e.status===`todo`||e.status===`in_review`),s=e=>{let t=en[e.status]??`○`,n=tn(e),a=nn(e),o=(i.includes(e)?` is-current`:``)+(a.failed?` is-failed`:``),s=[e.id,n,a.text].filter(e=>e.length>0).join(` · `),c=e.cardDoc?` · <button type="button" class="dsh-pm-sn-doc" data-action="open-doc" data-path="`+r(e.cardDoc)+`">任务卡</button>`:``;return`<div class="dsh-pm-sn-task`+o+`" data-status="`+r(e.status)+`"><div class="dsh-pm-sn-task-line1"><span class="dsh-pm-sn-task-glyph">`+t+`</span><span class="dsh-pm-sn-task-title">`+r(e.title)+`</span></div><div class="dsh-pm-sn-task-line2">`+r(s)+c+`</div></div>`},c=[];i.length>0&&c.push(`<div class="dsh-pm-sn-group"><div class="dsh-pm-sn-group-label is-active">进行中 `+i.length+` 个</div>`+i.map(s).join(``)+`</div>`),a.length>0&&c.push(`<div class="dsh-pm-sn-group"><div class="dsh-pm-sn-group-label">已完成 `+a.length+` 个</div>`+a.map(s).join(``)+`</div>`),o.length>0&&c.push(`<div class="dsh-pm-sn-group"><div class="dsh-pm-sn-group-label">待开始 `+o.length+` 个</div>`+o.map(s).join(``)+`</div>`);let l=Object.entries(t.byWindow??{}),u=l.length>1?`<div class="dsh-pm-sn-dim">窗口分工：`+l.map(([e,t])=>r(e)+` `+t.length+` 任务`).join(` · `)+`</div>`:``;return`<div class="dsh-pm-sn-body" data-stage="implementing">`+c.join(``)+u+`</div>`};function un(e,t){let n=t.sheet;if(n===void 0||n.items.length===0)return``;let i=e.requirementId??``,a={...ne},o=n.items.map(e=>{let t=e.status!==`pending`,n=e.source.kind===`requirement`?`requirement`:e.source.taskId;return`<div class="dsh-pm-vitem" data-item-id="`+r(e.id)+`" data-source="`+r(n)+`"><div class="dsh-pm-vitem-head"><span class="dsh-pm-vitem-badge">`+(a[e.status]??r(e.status))+`</span><span class="dsh-pm-vitem-src">`+r(e.source.kind===`requirement`?`需求级`:e.source.taskId)+`</span></div><div class="dsh-pm-sn-text">`+r(q(e.criterion,200))+`</div>`+(t?e.opinion?`<div class="dsh-pm-sn-warn">意见：`+r(q(e.opinion,200))+`</div>`:``:`<div class="dsh-pm-vitem-actions"><label><input type="radio" name="verdict-`+r(e.id)+`" value="passed"> 通过</label><label><input type="radio" name="verdict-`+r(e.id)+`" value="failed"> 不通过</label><input type="text" class="dsh-pm-vitem-opinion" placeholder="不通过时填意见（必填）"></div>`)+`</div>`}).join(``),s=n.items.every(e=>e.status!==`pending`);return`<div class="dsh-pm-vsheet" data-req="`+r(i)+`" data-version="`+n.version+`"><div class="dsh-pm-sn-label">验收单 v`+n.version+(n.reworkOnly===!0?`（返工续验：只含未过项）`:``)+` · 共 `+n.items.length+` 项</div>`+o+(s?`<div class="dsh-pm-sn-dim">本轮已裁决完毕：全部通过请点上方「验收通过」归档；有未过项已自动打回返工。</div>`:`<button type="button" class="dsh-pm-btn sm primary" data-action="submit-verdicts" data-req="`+r(i)+`" data-version="`+n.version+`">提交裁决</button>`)+`</div>`}const dn={draft:{renderBody:rn},brainstorming:{renderBody:an},planning:{renderBody:on},decomposing:{renderBody:cn},implementing:{renderBody:ln},accepting:{renderBody:e=>{let t=e.body;if(!t.verification)return`<div class="dsh-pm-sn-body" data-stage="accepting"><div class="dsh-pm-sn-empty">尚未提交验收材料</div></div>`;let n=t.verification,i=un(e,n),a=(n.evidence??[]).slice(0,5),o=a.map(e=>`<div class="dsh-pm-sn-line"><span class="dsh-pm-sn-text">- `+r(q(e,140))+`</span></div>`).join(``);return`<div class="dsh-pm-sn-body" data-stage="accepting">`+(n.summary?`<div class="dsh-pm-sn-text">`+r(n.summary)+`</div>`:``)+i+(a.length>0?`<div class="dsh-pm-sn-label">证据（`+(n.evidence??[]).length+` 条）</div>`+o:``)+(n.reviewNote?`<div class="dsh-pm-sn-warn">审核意见：`+r(q(n.reviewNote,200))+`</div>`:``)+`</div>`}},archived:{renderBody:e=>{let t=e.body;if(!t.archive)return`<div class="dsh-pm-sn-body" data-stage="archived"><div class="dsh-pm-sn-empty">暂无归档材料</div></div>`;let n=t.archive,i=(n.mergedInto?.length??0)>0?`<div class="dsh-pm-sn-dim">合并去向：`+(n.mergedInto??[]).map(e=>`<button type="button" class="dsh-pm-sn-doc" data-action="open-doc" data-path="`+r(e)+`">`+r(e)+`</button>`).join(` · `)+`</div>`:``;return`<div class="dsh-pm-sn-body" data-stage="archived"><div class="dsh-pm-sn-text">归档目录：`+r(n.dir)+`</div>`+(n.indexEntry?`<div class="dsh-pm-sn-text">`+r(n.indexEntry)+`</div>`:``)+i+`</div>`}}};function fn(e){return e.length===0?``:`<div class="dsh-pm-sn-label">动态</div>`+e.slice(-5).reverse().map(e=>{let t=J(e.by),n=e.reason!==void 0&&e.reason.length>0?e.reason:Xt[e.status]??e.status,i=e.inferred===!0?` <span class="dsh-pm-sn-dim">(回填)</span>`:``;return`<div class="dsh-pm-sn-comment"><div class="dsh-pm-sn-comment-who">`+r([t,K(e.at)].filter(e=>e.length>0).join(` · `))+i+`</div><div class="dsh-pm-sn-comment-text">`+r(n)+`</div></div>`}).join(``)}function pn(e){let t=e.artifacts??[],n=te[e.stage]??[];if(t.length===0&&n.length===0)return``;let i=new Map;for(let e of t){let t=i.get(e.kind)??[];t.push(e),i.set(e.kind,t)}let a=[];for(let e of Qt){let t=i.get(e);if(!t||t.length===0)continue;let n=Zt[e]??e;if(e===`task_detail`){a.push(`<span class="dsh-pm-trace-node" data-kind="`+r(e)+`"><span class="dsh-pm-sn-dim">`+r(n)+`×`+t.length+`</span></span>`);continue}for(let i of t)a.push(`<span class="dsh-pm-trace-node" data-kind="`+r(e)+`"><button type="button" class="dsh-pm-sn-doc dsh-pm-trace-path" data-action="open-doc" data-path="`+r(i.path)+`">`+r(n)+`</button></span>`)}let o=new Set(t.map(e=>e.kind));for(let e of n)if(!o.has(e)){let t=Zt[e]??e;a.push(`<span class="dsh-pm-trace-node is-missing" data-kind="`+r(e)+`"><span class="dsh-pm-sn-doc is-missing">`+r(t)+`（缺失）</span></span>`)}return a.length===0?``:`<div class="dsh-pm-trace-chain">`+a.join(`<span class="dsh-pm-trace-arrow">→</span>`)+`</div>`}function mn(e){if(!e.enabled)return`本分类跳过`;switch(e.stage){case`draft`:return`已立项`;case`brainstorming`:{let t=e.body.comments?.length??0;return t>0?t+` 条评论`:`需求分析`}case`planning`:{let t=e.body;return t.plan?t.plan.approvedAt===void 0?t.plan.rejectedAt===void 0?`计划待批准`:`计划被退回`:`计划已批准`:`待提交计划`}case`decomposing`:{let t=e.body.tasks?.length??0;return t>0?t+` 个任务`:`待拆分`}case`implementing`:{let t=e.body.tasks??[],n=t.length;if(n===0)return`暂无任务`;let r=t.filter(e=>e.status===`done`).length,i=t.find(e=>e.status===`in_progress`||e.status===`integrating`||e.status===`testing`),a=r+`/`+n+` 完成`;return n-r>0&&(a+=` · 剩 `+(n-r)+` 个`),i!==void 0&&(a+=` · 进行中 `+i.id),a}case`accepting`:{let t=e.body;return t.verification?t.verification.decision===`pass`?`验收通过`:t.verification.decision===`rework`?`验收被退回返工`:`待人工审核`:`待提交验收材料`}case`done`:return;case`archived`:return`已归档`}}function hn(e,t={}){let n=e.enabled?t.state??`current`:`skipped`,i=mn(e),a=e.timeline.length>0?e.timeline[e.timeline.length-1].at:void 0,o=e.pendingConfirmation?`<div class="dsh-pm-sn-warn">⚠ 有产物待人工确认</div>`:``,s=e.stage===`done`?void 0:dn[e.stage],c=e.enabled?s?s.renderBody(e):`<div class="dsh-pm-sn-empty">未知阶段</div>`:`<div class="dsh-pm-sn-body" data-stage="`+r(e.stage)+`"><div class="dsh-pm-sn-empty">该阶段在当前分类流程中不适用</div></div>`;return`<div class="dsh-pm-stage-panel dsh-pm-sn" data-stage="`+r(e.stage)+`" data-state="`+n+`"><div class="dsh-pm-sn-head"><span class="dsh-pm-sn-title">`+r(i)+`</span>`+(a===void 0?``:`<span class="dsh-pm-sn-time">`+r(K(a))+`</span>`)+`</div>`+pn(e)+o+c+fn(e.timeline)+`</div>`}function gn(e,t){let n=e.stages.find(e=>e.stage===t);if(n===void 0||!n.enabled)return`skipped`;let r=P.indexOf(e.currentStage),i=P.indexOf(t);return r>=0&&i<r?`done`:i===r?`current`:`pending`}function _n(e,t){let n=e.stages.find(e=>e.stage===t)??e.stages[0];return n===void 0?`<div class="dsh-pm-sn-empty">未知节点</div>`:hn(n,{state:gn(e,n.stage)})}const vn=`dsh-pmboard:view`;function yn(){try{return sessionStorage.getItem(vn)===`list`?`list`:`lanes`}catch{return`lanes`}}function bn(e){try{sessionStorage.setItem(vn,e)}catch{}}const xn=`dsh-pmboard:list`,Sn=[`stage`,`progress`,`updated`,`created`,`title`];function Cn(){let e={sortKey:`stage`,sortDir:`asc`,pageSize:10};try{let t=sessionStorage.getItem(xn);if(t===null)return e;let n=JSON.parse(t),r=typeof n.sortKey==`string`&&Sn.includes(n.sortKey)?n.sortKey:e.sortKey;return{sortKey:r,sortDir:n.sortDir===`asc`||n.sortDir===`desc`?n.sortDir:B(r),pageSize:typeof n.pageSize==`number`&&z.includes(n.pageSize)?n.pageSize:10}}catch{return e}}function Y(e,t){let n=t.length>18?t.slice(0,18)+`…`:t;switch(e){case`archived`:return`该会话已归档（`+n+`）：日志保留、侧栏不可见，无法跳转`;case`missing`:return`该会话不在当前会话列表（`+n+`）：可能已删除或不在当前工作区`;case`unavailable`:return`会话服务暂不可用（页面注入未就绪），请刷新页面后重试`;case`opened`:return``;default:return`会话跳转结果未知：`+String(e)}}function wn(){return{openBoard:()=>{},closeBoard:()=>{},toggleBoard:()=>{},getSnapshot:()=>({boardOpen:!1}),refresh:()=>{}}}function Tn(e){let t,n=[],r={kind:`board`},a=yn(),s=Cn(),l=s.sortKey,u=s.sortDir,d=s.pageSize,f=1,p,m,h,g=()=>({sortKey:l,sortDir:u,page:f,pageSize:d}),_=()=>Jt(),v=()=>{try{sessionStorage.setItem(xn,JSON.stringify({sortKey:l,sortDir:u,pageSize:d}))}catch{}},y=e=>{m!==void 0&&m.querySelectorAll(`[data-action="load-stage"]`).forEach(t=>{e!==void 0&&t.dataset.stage===e?t.setAttribute(`data-active`,`true`):t.removeAttribute(`data-active`)})},b=()=>{if(m===void 0)return;if(t===void 0){m.innerHTML=T();return}let e=r;switch(e.kind){case`board`:m.innerHTML=R(t,Date.now(),a,g(),_());break;case`req`:{let n=t.requirements.find(t=>t.id===e.reqId);m.innerHTML=n?tt(n,t.tasks,Date.now(),_()):R(t,Date.now(),a,g(),_()),n?(y(p),D(),O(n.id,n.status),k(n.sourceSessionId)):r={kind:`board`};break}case`task`:{let n=t.tasks.find(t=>t.id===e.taskId),i=n?t.requirements.find(e=>e.id===n.requirementId):void 0;m.innerHTML=n?ht(n,i,Date.now(),t.tasks,_()):R(t,Date.now(),a,g(),_()),n||(r={kind:`board`});break}case`tasks`:m.innerHTML=he(t);break;case`triage`:m.innerHTML=We(n,t)}},x=async()=>{try{let[e,r]=await Promise.all([Ct(),wt()]);t=e,n=r.pending,b()}catch(e){m!==void 0&&(m.innerHTML=E(String(e)))}},S=()=>{h?.(),h=Vt(()=>{x()})},C=e=>{let i=e.target,o=i.closest(`[data-pmpage]`);if(o!==null&&t!==void 0){let e=Number(o.dataset.pmpage);Number.isFinite(e)&&e>=1&&(f=e,b());return}let s=i.closest(`[data-action]`);if(s!==null&&t!==void 0)switch(s.dataset.action??``){case`refresh`:x();return;case`switch-view`:{let e=s.dataset.view;(e===`lanes`||e===`list`)&&(a=e,bn(e),r={kind:`board`},b());return}case`list-sort`:{let e=s.dataset.key;if(e===void 0||!Sn.includes(e))return;e===l?u=u===`asc`?`desc`:`asc`:(l=e,u=B(e)),f=1,v(),b();return}case`open-doc`:{let e=s.dataset.path;e&&qt(window.__dshPmCtx,e,Kt());return}case`switch-tab`:{let e=i.closest(`.dsh-pm-tab`);if(!e)return;let t=e.dataset.tab;if(!t)return;let n=e.closest(`.dsh-pm-detail`);if(!n)return;n.querySelectorAll(`.dsh-pm-tab`).forEach(e=>{e.classList.remove(`active`)}),e.classList.add(`active`),n.querySelectorAll(`.dsh-pm-tab-content`).forEach(e=>{e.classList.remove(`active`)});let r=n.querySelector(`.dsh-pm-tab-content[data-tab-content="${t}"]`);r&&r.classList.add(`active`);return}case`load-stage`:{let e=s.dataset.req,t=s.dataset.stage;e&&t&&(p=t,y(t),O(e,t));return}case`confirm-artifact`:{let e=s.dataset.id,t=s.dataset.kind;e&&t&&Ft({id:e,kind:t}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`submit-verdicts`:{let e=s.dataset.req,t=Number(s.dataset.version),n=s.closest(`.dsh-pm-vsheet`);if(!e||!Number.isFinite(t)||n===null)return;let r=[];if(n.querySelectorAll(`.dsh-pm-vitem`).forEach(e=>{let t=e.dataset.itemId;if(t===void 0)return;let n=e.querySelector(`input[type="radio"]:checked`);if(n===null)return;let i=e.querySelector(`.dsh-pm-vitem-opinion`)?.value.trim()??``;r.push({itemId:t,status:n.value===`passed`?`passed`:`failed`,...i.length>0?{opinion:i}:{}})}),r.length===0){window.alert(`请先逐项选择 通过/不通过`);return}Mt({id:e,version:t,verdicts:r}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`new-req`:{let e=window.prompt(`需求标题`);e&&e.trim()&&Et({title:e.trim()}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`open-req`:s.dataset.req&&(p=void 0,r={kind:`req`,reqId:s.dataset.req},b());return;case`open-task`:s.dataset.task&&(r={kind:`task`,taskId:s.dataset.task},b());return;case`open-tasks`:p=void 0,r={kind:`tasks`},b();return;case`new-task`:{let e=s.dataset.id;if(!e)return;let t=window.prompt(`任务标题`);t&&t.trim()&&It({requirementId:e,title:t.trim(),phase:`implement`,side:`fullstack`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`back`:p=void 0,r={kind:`board`},b();return;case`back-req`:p=void 0,r={kind:`req`,reqId:s.dataset.req??``},b();return;case`move-req`:{let e=s.dataset.id??(r.kind===`req`?r.reqId:void 0),t=s.dataset.to;e&&t&&Dt({id:e,to:t,actor:`human`,reason:s.dataset.id?`看板泳道卡面操作`:`需求详情页操作`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`add-comment`:{let e=(m?.querySelector(`[data-role="comment-input"]`))?.value.trim();e&&s.dataset.target&&s.dataset.id&&Lt({target:s.dataset.target,id:s.dataset.id,body:e,actor:`human`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`jump-session`:{let e=s.dataset.sid;if(!e)return;if(s.dataset.archived===`true`){window.alert(Y(`archived`,e));return}Yt(W(),e).then(t=>{let n=Y(t,e);n!==``&&window.alert(n)}).catch(e=>window.alert(`会话跳转失败：`+String(e)));return}case`verify-pass`:{let e=s.dataset.id;e&&At({id:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`verify-rework`:{let e=s.dataset.id;if(!e)return;let t=window.prompt(`退回返工的意见（窗口会按它整改）`);if(t===null)return;jt({id:e,note:t.trim()||`（未填意见）`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`archive-req`:{let e=s.dataset.id;e&&Nt({id:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`plan-approve`:{let e=s.dataset.id;e&&Ot({id:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`plan-reject`:{let e=s.dataset.id;if(!e)return;let t=window.prompt(`退回理由（窗口会按它重写计划）`);if(t===null)return;kt({id:e,reason:t.trim()||`（未填理由）`}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`triage-confirm`:{let e=s.dataset.triage;if(!e)return;let t=n.find(t=>t.id===e);if(t?.suggestedAction===`bind_req`&&t.suggestedTargetId)Rt({triageId:e,action:`bind_req`,targetId:t.suggestedTargetId}).then(()=>x()).catch(e=>window.alert(String(e)));else{let t=s.closest(`.dsh-pm-triage`),n=t?.querySelector(`[data-role="triage-title"]`)?.value.trim(),r=t?.querySelector(`[data-role="triage-category"]`)?.value;Rt({triageId:e,action:`create_req`,...n?{title:n}:{},...r?{category:r}:{}}).then(()=>x()).catch(e=>window.alert(String(e)))}return}case`triage-rebind`:{let e=m?.querySelector(`.dsh-pm-rebind-host`);e&&(e.style.display=`flex`,e.dataset.triage=s.dataset.triage??``);return}case`triage-rebind-confirm`:{let e=m?.querySelector(`.dsh-pm-rebind-host`),t=e?.dataset.triage,n=e?.querySelector(`[data-role="rebind-select"]`);t&&n?.value&&zt({triageId:t,targetId:n.value}).then(()=>x()).catch(e=>window.alert(String(e)));return}case`triage-reject`:{let e=s.dataset.triage;e&&Bt({triageId:e}).then(()=>x()).catch(e=>window.alert(String(e)));return}}},w=e=>{let t=e.target;if(t===null||typeof t.closest!=`function`)return;let n=t.closest(`[data-action="list-size"]`);if(n!==null){let e=Number(n.value);if(!z.includes(e))return;d=e,f=1,v(),b();return}},D=async()=>{if(m===void 0)return;let e=m.querySelectorAll(`[data-doc-path]`);await Promise.all(Array.from(e).map(async e=>{let t=e.dataset.docPath;if(!t)return;let n=!1;try{n=(await(await fetch(`/dashboard/api/reqboard/file?path=`+encodeURIComponent(t))).json()).success===!0}catch{n=!1}if(!n){e.classList.add(`is-missing`),e.setAttribute(`title`,`文件不存在（未落盘或路径错误）`);let t=e.querySelector(`[data-action="open-doc"]`);t&&(t.removeAttribute(`data-action`),t.classList.add(`dsh-pm-doc-missing`))}}))},O=async(e,t)=>{let n=document.getElementById(`dsh-pm-stage-detail-container`);if(n!==null){n.innerHTML=`<div class="dsh-pm-empty">详情加载中…</div>`;try{let r=await fetch(`/dashboard/api/reqboard/requirements/`+encodeURIComponent(e)+`/stages`,{signal:AbortSignal.timeout(8e3)});if(!r.ok){n.innerHTML=`<div class="dsh-pm-empty">详情暂不可用（HTTP `+r.status+`）</div>`;return}let i=await r.json();n.innerHTML=i.success===!0&&i.data!==void 0?_n(i.data,t):`<div class="dsh-pm-empty">详情暂不可用</div>`}catch(e){n.innerHTML=`<div class="dsh-pm-empty">详情加载失败：`+String(e)+`</div>`}}},k=async e=>{let t=document.getElementById(`dsh-pm-injection-info-container`);if(t!==null){if(!Je(e)){t.innerHTML=V([]);return}try{let n=await Tt(e);t.innerHTML=V(n.available?n.entries:[])}catch{t.innerHTML=V([])}}},A=i({prefix:`dsh-pm`,panelName:o,activeAttr:`data-dsh-pm-active`,otherActiveAttrs:c,pollMs:2e4,pauseOnHidden:!0,buildContainer:()=>{let e=document.createElement(`div`);return e.dataset.dshPmView=``,e.className=`dsh-pm-view`,e},onMount:e=>(m=e,e.addEventListener(`click`,C),e.addEventListener(`change`,w),x(),S(),()=>{e.removeEventListener(`click`,C),e.removeEventListener(`change`,w),h?.(),h=void 0,m=void 0}),onPoll:()=>{x()},onOpen:()=>{x()}}),j=e;return j.openBoard=A.open,j.closeBoard=A.close,j.toggleBoard=A.toggle,j.getSnapshot=()=>({boardOpen:A.isActive()}),j.refresh=()=>{x()},()=>{A.dispose()}}const En=`dsh-pmboard/footer-action.css`,X=`dsh-pmboard:open-board`,Dn={draft:`立项`,brainstorming:`需求分析`,planning:`技术设计`,decomposing:`拆分`,implementing:`实施中`,accepting:`待验收`,done:`完成`},On=(0,e.createElement)(`svg`,{viewBox:`0 0 16 16`,width:`16`,height:`16`,fill:`none`,stroke:`currentColor`,"stroke-width":`1.4`,"stroke-linecap":`round`,"stroke-linejoin":`round`,"aria-hidden":`true`},(0,e.createElement)(`rect`,{x:`2`,y:`2`,width:`5`,height:`5`,rx:`1`}),(0,e.createElement)(`rect`,{x:`9`,y:`2`,width:`5`,height:`5`,rx:`1`}),(0,e.createElement)(`rect`,{x:`2`,y:`9`,width:`5`,height:`5`,rx:`1`}),(0,e.createElement)(`rect`,{x:`9`,y:`9`,width:`5`,height:`5`,rx:`1`}));function kn(){if(typeof document>`u`||document.querySelector(`style[data-plugin-css="${En}"]`))return;let e=document.createElement(`style`);e.dataset.pluginCss=En,e.textContent=`
.dsh-reqboard-foot {
...
`,document.head.appendChild(e)}const Fn=[`slots`,`sessions`,`workspaces`,`sidebarRight`];function In(e){try{kn(),Pn(),window.__dshReqboardClient?.dispose(),window.__dshPmCtx=e,window.__dshPmSessions=e.sessions,window.__dshPmWorkspaces=e.workspaces;let t=wn(),n=Tn(t),r=e=>{e.detail?.open===!0?t.getSnapshot().boardOpen?t.closeBoard():t.openBoard():t.toggleBoard()};window.addEventListener(X,r),window.__dshReqboardClient={dispose:()=>{window.removeEventListener(X,r),n(),t.closeBoard(),delete window.__dshPmCtx,delete window.__dshPmSessions,delete window.__dshPmWorkspaces}};let i=e.slots;if(i){try{i.inject(`sidebar.footer.action`,()=>i.register({name:`sidebar.footer.action`,id:o,order:110,label:s},An))}catch(e){console.error(`[dsh-pmboard] Failed to register sidebar.footer.action:`,e)}try{i.inject(`conversation.session.header.utilities`,()=>i.register({name:`conversation.session.header.utilities`,id:`dsh-pmboard:progress`,order:5,inject:e=>({sessionId:e})},Nn))}catch(e){console.error(`[dsh-pmboard] Failed to register conversation.session.header.utilities:`,e)}}else console.warn(`[dsh-pmboard] ctx.slots unavailable`)}catch(e){console.error(`[dsh-pmboard] client half failed to start:`,e)}}exports.apply=In,exports.inject=Fn,exports.name=`dsh-pmboard/client`;
