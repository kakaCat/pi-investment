...
export const lhbPrompt: ToolPrompt<LhbParams, LhbResult> = {
  description: '龙虎榜查询（游资/机构席位动向）。date 模式查某日全市场上榜记录（游资炒作标的+机构买卖席位）；symbol 模式查个股上榜明细。适用于：追踪游资动向、识别机构进出、博弈对手行为分析。解读参考：知名游资席位买入=短线情绪标的；机构席位大额卖出=出货预警。数据源为 akshare，盘中/非交易日可能无数据。',
