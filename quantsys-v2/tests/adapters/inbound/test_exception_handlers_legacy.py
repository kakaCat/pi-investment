...
def app():
    ...
    async def test_invalid_symbol():
        raise ValidationError(symbol="INVALID")
    ...
    async def test_data_unavailable():
        raise DataSourceUnavailableError(providers_tried=["akshare", "tushare"])

    @test_app.get("/test/insufficient-data")
    async def test_insufficient_data():
        raise DataQualityError(required_points=20, available_points=5)
