...
class TestUpdateSymbol:
    def test_bootstrap_from_full_history(self):
        repo = FakeRepo(klines={"600519": KLINES})
        svc = ChipDistributionService(repo)
        result = svc.update_symbol("600519")
        assert result["days_applied"] == 3
        assert "600519" in repo.saved_states
        # 最后一天收盘价算指标
...
class TestTurnoverFallback:
    def test_missing_turnover_uses_circulating_mv(self):
        ...
        repo = FakeRepo(klines={"600519": rows}, circulating_mv=2e9)
        svc = ChipDistributionService(repo)
        svc.update_symbol("600519")
        dist, _ = repo.saved_states["600519"]
    ...
    def test_missing_turnover_and_mv_uses_market_median(self):
        ...
        repo = FakeRepo(klines={"600519": rows}, circulating_mv=None, median_turnover=3.0)
        svc = ChipDistributionService(repo)
        svc.update_symbol("600519")
        dist, _ = repo.saved_states["600519"]
...
class TestGetDistribution:
    def test_curve_and_metrics(self):
        ...
        svc = ChipDistributionService(repo)
        svc.update_symbol("600519")
        out = svc.get_distribution("600519")
    ...
    def test_symbol_normalization(self):
        repo = FakeRepo(klines={"600519": KLINES})
        svc = ChipDistributionService(repo)
        svc.update_symbol("600519.SH")  # 带后缀 → 归一化为库内 bare 格式
        assert "600519" in repo.saved_states
