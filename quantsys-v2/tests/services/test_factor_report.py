...
class TestDataServiceReportIntegration:
    ...
    def test_generate_factor_report_basic(self, data_service, tmp_path, monkeypatch):
        """测试基础报告生成流程"""
        from application.services.factor_analysis_service import ALPHALENS_AVAILABLE
        ...
        # Mock _fetch_factor_data 方法
        def mock_fetch_factor_data(factor_name, universe, start_date, end_date):
            np.random.seed(42)
            dates = pd.date_range(start_date, end_date, freq='D')[:30]  # 限制30天
            data = []
            for date in dates:
                for symbol in universe[:3]:  # 限制3只股票
                    data.append({
                        'symbol': symbol,
                        'date': date,
                        'factor': np.random.randn(),
    ...
    def test_generate_factor_report_partial_failure(self, data_service, tmp_path, monkeypatch):
        """测试部分因子失败的情况"""
        from application.services.factor_analysis_service import ALPHALENS_AVAILABLE
        ...
        # Mock _fetch_factor_data 方法（第二个因子返回空数据）
        def mock_fetch_factor_data(factor_name, universe, start_date, end_date):
    ...
    def test_generate_factor_report_without_alphalens(self, data_service):
        """测试 alphalens 不可用时的错误"""
        from application.services.factor_analysis_service import ALPHALENS_AVAILABLE

        if ALPHALENS_AVAILABLE:
            pytest.skip("alphalens is available")
        ...
        assert 'alphalens' in result['error'].lower()
