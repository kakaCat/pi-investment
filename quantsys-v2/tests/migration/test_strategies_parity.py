...
def test_list_user(flask_client, fastapi_client):
    assert_parity(flask_client, fastapi_client, "GET", LIST, params={"page": 1, "pageSize": 5})


def test_list_builtin(flask_client, fastapi_client):
    # builtin 模式走 StrategyFactory（不依赖 DB），是真实数据的 parity 验证
    assert_parity(flask_client, fastapi_client, "GET", LIST, params={"source": "builtin"})


def test_list_invalid_code_type(flask_client, fastapi_client):
    assert_parity(flask_client, fastapi_client, "GET", LIST, params={"codeType": "bogus"})
...
def test_performance_not_found(flask_client, fastapi_client):
    assert_parity(flask_client, fastapi_client, "GET", PERF)


# ---- strategies.py POST 校验路径（不产生真实写入）----
def test_create_missing_body(flask_client, fastapi_client):
    ...
    assert_parity(flask_client, fastapi_client, "POST", CREATE, json_body={})
...
def test_create_invalid_code_type(flask_client, fastapi_client):
    assert_parity(flask_client, fastapi_client, "POST", CREATE,
                  json_body={"name": "x", "code": "y", "codeType": "bogus"})


def test_optimize_missing_params(flask_client, fastapi_client):
    assert_parity(flask_client, fastapi_client, "POST", OPTIMIZE, json_body={"strategyId": 1})


# ---- strategy.py GET ----
def test_strategy_status(flask_client, fastapi_client):
    assert_parity(flask_client, fastapi_client, "GET", STRATEGY_STATUS)
