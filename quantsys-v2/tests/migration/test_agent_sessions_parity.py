...
@pytest.fixture
def seeded(fastapi_client):
    """通过 FastAPI 端写入一条事件（POST 单独有用例，这里只造数据）"""
    resp = fastapi_client.post("/api/sessions/events", json=SEED)
    assert resp.status_code == 200


def test_post_events(fastapi_client):
    ...
    assert fa.json().get("success") is True
    _clean_tables()


def test_post_events_rejects_empty(fastapi_client):
    assert_parity(fastapi_client, "POST", "/api/sessions/events", json_body={"events": []})


def test_list_sessions(fastapi_client, seeded):
    assert_parity(fastapi_client, "GET", "/api/sessions")


def test_get_session(fastapi_client, seeded):
    assert_parity(fastapi_client, "GET", f"/api/sessions/{KEY}")


def test_get_session_not_found(fastapi_client):
    assert_parity(fastapi_client, "GET", "/api/sessions/agent:nope")


def test_get_events(fastapi_client, seeded):
    assert_parity(fastapi_client, "GET", f"/api/sessions/{KEY}/events")


def test_get_diagnosis(fastapi_client, seeded):
    assert_parity(fastapi_client, "GET", f"/api/sessions/{KEY}/diagnosis")


def test_ai_diagnosis(fastapi_client, seeded, monkeypatch):
    ...
    assert_parity(fastapi_client, "POST", f"/api/sessions/{KEY}/ai-diagnosis")
