...
def test_list(fastapi_client):
    assert_parity(fastapi_client, "GET", LIST, params={"limit": 5})
...
def test_pending(fastapi_client):
    assert_parity(fastapi_client, "GET", PENDING, params={"limit": 5})
...
def test_get_not_found(fastapi_client):
    assert_parity(fastapi_client, "GET", BY_ID)


def test_close_missing_params(fastapi_client):
    assert_parity(fastapi_client, "PUT", CLOSE, json_body={})


def test_cancel_not_found(fastapi_client):
    assert_parity(fastapi_client, "PUT", CANCEL, json_body={})
