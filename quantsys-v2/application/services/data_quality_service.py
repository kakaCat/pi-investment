...
class DataQualityService:
    ...
    def _persist_quality_audit(
        ...
        """
        try:
            # 2026-09-14（REQ-24e15d）：原先在这里 get_session() + text() 手写 INSERT，
            # 现收口到 KlineQualityRepository.record_check_result（ORM insert，
            # 表 = public.kline_data_quality，与旧写法靠 search_path 落到的表一致）。
            ...
            KlineQualityRepository().record_check_result(
                symbol=symbol,
                period='daily',
                start_date=start_date,
                end_date=end_date,
