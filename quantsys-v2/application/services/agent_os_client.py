...
Usage:
    ...
    # Report execution result (run_id comes from webhook metadata.run_id)
    await client.report_job_result(run_id, {
        "status": "success",
        "output": "{\"updated\": 100}"
    })
"""
...
class AgentOSClient:
    ...
    async def report_job_result(
        ...
        Agent OS of the REAL outcome (the scheduler's own run record only
        tracks "webhook accepted").

        Args:
            run_id: Run UUID assigned by Agent OS (carried in the webhook
                payload's metadata.run_id — NOT a locally generated one)
            result: Result dictionary with keys:
                - status (str): "success" or "failed"
                - output (str, optional): JSON-encoded job result
