...
CONFIG = {
    ...
    # 🆕 V8激进风控：放宽止盈 + 严格止损 + 适度杠杆
    'risk_management': {
        # 放宽止盈（让利润奔跑）
        'take_profit_levels': [
            {'threshold': 0.20, 'position': 0.4},  # 20%收益：减仓到40%
            {'threshold': 0.15, 'position': 0.6},  # 15%收益：减仓到60%
        ],
        # 严格回撤止损（控制在20%以内）
        'drawdown_stops': [
            {'threshold': -0.20, 'position': 0.3},  # 回撤20%：硬止损到30%
            {'threshold': -0.15, 'position': 0.5},  # 回撤15%：减仓到50%
            {'threshold': -0.10, 'position': 0.7},  # 回撤10%：减仓到70%
        ],
        # 🆕 适度杠杆（收益>10%时加仓）
        'leverage': {
            'profit_threshold': 0.10,  # 收益>10%时触发
            'max_position': 1.2,        # 最大仓位120%
        },
    },
