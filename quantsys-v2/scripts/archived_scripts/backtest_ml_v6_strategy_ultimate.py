...
策略逻辑：
1. 每10个交易日重新预测一次
2. 识别当前市场环境（基于市场20日收益率）
3. 根据市场环境动态调整仓位
   - 熊市（<-10%）: 减仓50%或清仓
   - 震荡市（-10%~10%）: 正常100%持仓
   - 牛市（>10%）: 正常100%持仓
4. 选择预测超额收益Top 20的股票
5. 分档配置：Top 5每只8%，6-10每只6%，11-20每只4%
...
CONFIG = {
    ...
    # 改进5：市场环境识别（修正版：使用60日窗口）
    'market_regime': {
        'lookback_days': 60,      # 回看60日（改为3个月）
        'bear_threshold': -0.15,   # 熊市：跌超15%
        'bull_threshold': 0.15,    # 牛市：涨超15%
        'bear_position': 0.2,      # 熊市持仓比例：20%（大幅降低）
        'normal_position': 1.0,    # 震荡市持仓比例：100%
    },
...
def backtest_strategy(test_df, model, valid_factors):
    ...
    for i, current_date in enumerate(dates):
        ...
        if i % rebalance_days == 0:
            # 改进5：识别市场环境
            market_regime, market_return_20d = identify_market_regime(test_df, current_date)

            # 根据市场环境调整目标仓位
            if market_regime == 'bear':
                position_scale = CONFIG['market_regime']['bear_position']  # 熊市：30%
            else:
                position_scale = CONFIG['market_regime']['normal_position']  # 正常：100%

            print(f"\n[{current_date.date()}] 市场环境: {market_regime.upper()} (20日收益: {market_return_20d:.2%})")
            print(f"  目标仓位比例: {position_scale:.0%}")
