...
echo -n "[$((TEST_COUNT + 1))] V13 新旧接口数据一致性 ... "
TEST_COUNT=$((TEST_COUNT + 1))
...
if [ -n "$old_total" ] && [ -n "$new_total" ]; then
    ...
    if (( $(echo "$diff < 0.01" | bc -l) )); then
        ...
        echo "❌ FAIL (旧=$old_total, 新=$new_total, 差异=$diff)"
        FAIL_COUNT=$((FAIL_COUNT + 1))
    fi
else
...
TEST_COUNT=$((TEST_COUNT + 1))
if python3 -c "import yaml; yaml.safe_load(open('live_trading/configs/strategies/v13.yaml'))" 2>/dev/null; then
    echo "✅ PASS"
    PASS_COUNT=$((PASS_COUNT + 1))
else
    echo "❌ FAIL"
...
TEST_COUNT=$((TEST_COUNT + 1))
if python3 -c "import yaml; yaml.safe_load(open('live_trading/configs/strategies/v14.yaml'))" 2>/dev/null; then
    echo "✅ PASS"
    PASS_COUNT=$((PASS_COUNT + 1))
else
    echo "❌ FAIL"
...
TEST_COUNT=$((TEST_COUNT + 1))
if python3 -c "from application.services.strategy_service import StrategyService" 2>/dev/null; then
    echo "✅ PASS"
    PASS_COUNT=$((PASS_COUNT + 1))
else
    echo "❌ FAIL"
...
TEST_COUNT=$((TEST_COUNT + 1))
result=$(python3 -c "from application.services.strategy_service import StrategyService; s = StrategyService(); print(','.join(s.list_strategies()))" 2>/dev/null)
...
echo -n "[$((TEST_COUNT + 1))] strategy_service.py 语法检查 ... "
TEST_COUNT=$((TEST_COUNT + 1))
if python3 -m py_compile application/services/strategy_service.py 2>/dev/null; then
    echo "✅ PASS"
    PASS_COUNT=$((PASS_COUNT + 1))
else
    echo "❌ FAIL"
...
echo -n "[$((TEST_COUNT + 1))] strategy_trading.py 语法检查 ... "
TEST_COUNT=$((TEST_COUNT + 1))
if python3 -m py_compile adapters/inbound/api/routes/strategy_trading.py 2>/dev/null; then
    echo "✅ PASS"
    PASS_COUNT=$((PASS_COUNT + 1))
else
    echo "❌ FAIL"
...
echo -n "[$((TEST_COUNT + 1))] strategy_trading_async.py 语法检查 ... "
TEST_COUNT=$((TEST_COUNT + 1))
if python3 -m py_compile adapters/inbound/fastapi_app/routes/strategy_trading_async.py 2>/dev/null; then
    echo "✅ PASS"
    PASS_COUNT=$((PASS_COUNT + 1))
else
    echo "❌ FAIL"
...
echo -n "[$((TEST_COUNT + 1))] strategy_trading_job.py 语法检查 ... "
TEST_COUNT=$((TEST_COUNT + 1))
if python3 -m py_compile infrastructure/jobs/strategy_trading_job.py 2>/dev/null; then
    echo "✅ PASS"
    PASS_COUNT=$((PASS_COUNT + 1))
else
    echo "❌ FAIL"
