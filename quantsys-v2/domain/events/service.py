...
_TYPE_KEYWORDS: List[Tuple[EventType, Tuple[str, ...]]] = [
    ...
    (EventType.POLICY, ('国务院', '中共中央', '发展改革委', '国家发改委', '证监会', '财政部',
                        ...
                        '市场监管总局', '指导意见', '政策文件')),
    # ── 2026-09-13（w-a9ec14d7）新增类别：按**真实公告流**补词，且一律追加在末尾 ──
    # 追加的理由：顺序即优先级，插在中间会改变既有 7 类的判定（回归风险）；
    # 追加在末尾则只把原本落到 other 的标题接住，既有判定完全不变。
    # 词表纪律沿用上文：不放「公告/决定/规定/通知」这类任意公告都带的词。
    (EventType.BUYBACK, ('回购股份', '股份回购', '回购公司股份', '回购注销', '集中竞价方式回购')),
    (EventType.EQUITY_INCENTIVE, ('股权激励', '股票期权', '限制性股票', '员工持股计划',
                                  '激励计划', '行权', '授予')),
    (EventType.PLEDGE, ('股份质押', '股权质押', '质押', '司法冻结', '轮候冻结',
                        '股份被冻结', '账户冻结', '财产保全')),
    (EventType.M_AND_A, ('重大资产重组', '资产重组', '收购', '资产购买', '资产出售',
                         '股权转让', '对外投资', '增资', '吸收合并', '要约收购')),
    (EventType.PRODUCT_APPROVAL, ('临床试验', '药品注册', '批准通知书', '补充申请批准',
                                  '注册证书', '上市许可', '审批通过', '获得批准')),
    (EventType.FINANCING, ('超短期融资券', '中期票据', '短期融资券', '公司债券', '可续期',
                           '债券发行', '发行结果', '债务融资工具')),
    (EventType.RELATED_PARTY, ('关联交易', '关联方', '关联人', '担保', '非经营性资金占用')),
    # 增持/减持：控股东与董监高的真金白银行为，研究价值高（区别于"投资者关系活动"这类噪声）
    (EventType.INSIDER, ('增持', '减持', '股份变动', '权益变动', '被动稀释')),
    # 2026-09-13 明确**故意不归类**的噪声类别（保留在 other，不硬塞语义）：
    #   H股公告/翌日披露报表（披露通道产物）、投资者关系活动记录表（无信息量）、
    #   非经营性资金占用汇总表（表格式披露）。给它们编类型只会污染事件研究的分组。
    (EventType.GOVERNANCE, ('董事会决议', '监事会决议', '董事会会议', '监事会会议', '章程',
                            '管理办法', '议事规则', '会计政策变更', '会计差错更正',
                            '高级管理人员', '董事', '监事', '独立董事')),
]
