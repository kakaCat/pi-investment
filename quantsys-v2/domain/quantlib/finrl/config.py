...
def validate_config(config: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    - Valid environment settings (initial_balance > 0, transaction_cost in [0, 1])
    ...
    if 'tau' in config:
        ...
        if not isinstance(tau, (int, float)) or not (0 < tau <= 1):
            errors.append(
                f"Invalid tau: {tau}. Must be in range (0, 1]."
            )
    ...
    if 'env' in config and isinstance(config['env'], dict):
        ...
        if 'initial_balance' in env:
            ...
            if not isinstance(balance, (int, float)) or balance <= 0:
                errors.append(
                    f"Invalid initial_balance: {balance}. Must be a positive number."
                )
