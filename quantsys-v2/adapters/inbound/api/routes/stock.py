...
def enrich_stock_data(stock) -> Dict:
    ...
    Args:
        stock: 基础股票信息（可以是Dict或Stock ORM对象）
...
def get_stock_list():
    ...
    try:
        ...
        if keyword:
            ...
            if industry:
                all_stocks = [stock for stock in all_stocks
                             if (hasattr(stock, 'industry') and stock.industry == industry) or
                                (isinstance(stock, dict) and stock.get('industry') == industry)]
        else:
        ...
        if keyword:
            ...
            all_stocks = [
                ...
                or keyword_lower in str(getattr(stock, 'name', None) or stock.get('name', '')).lower()
            ]
...
@stock_bp.route('/api/stocks/batch', methods=['POST'])
