...
class AkShareAdapter(BaseMarketAdapter):
    ...
    def _fetch_minute_klines(
        ...
        *start_date* / *end_date*: YYYY-MM-DD HH:MM:SS or YYYY-MM-DD
        ...
        logger.debug(f"AkShare: Fetching minute klines: symbol={symbol}, code={code}, period={period_ak}, start={start}, end={end}")

        # Try data source 1: East Money (Primary)
        try:
            logger.debug(f"AkShare: Trying East Money (stock_zh_a_hist_min_em)...")
            ...
            if df is not None and not df.empty:
                logger.debug(f"AkShare: East Money success - Got {len(df)} minute klines")
                return self._process_minute_df(df, symbol)

            logger.debug(f"AkShare: East Money returned empty data")
        except Exception as e:
            logger.debug(f"AkShare: East Money failed: {e}")

        # Try data source 2: Tencent (Fallback 1)
        try:
            logger.debug(f"AkShare: Trying Tencent (stock_zh_a_minute)...")
            ...
            if df is not None and not df.empty:
                logger.debug(f"AkShare: Tencent success - Got {len(df)} minute klines")
                return self._process_minute_df(df, symbol)

            logger.debug(f"AkShare: Tencent returned empty data")
        except Exception as e:
            logger.debug(f"AkShare: Tencent failed: {e}")

        # Try data source 3: Pre-market East Money (Fallback 2)
        try:
            logger.debug(f"AkShare: Trying Pre-market East Money (stock_zh_a_hist_pre_min_em)...")
            ...
            if df is not None and not df.empty:
                logger.debug(f"AkShare: Pre-market East Money success - Got {len(df)} minute klines")
                return self._process_minute_df(df, symbol)

            logger.debug(f"AkShare: Pre-market East Money returned empty data")
        except Exception as e:
            logger.debug(f"AkShare: Pre-market East Money failed: {e}")

        # All data sources failed
        logger.debug(f"AkShare: All data sources failed for {symbol}")
        return []
    ...
    def _process_minute_df(self, df: pd.DataFrame, symbol: str) -> list[dict]:
        ...
        if not required.issubset(df.columns):
            logger.debug(f"AkShare: Missing required columns. Got: {df.columns.tolist()}")
            return []
        ...
        logger.debug(f"AkShare: Processed {len(rows)} minute klines for {symbol}")
        return rows
