    ...
    for i, current_date in enumerate(dates):
        ...
        for symbol, pos in list(portfolio.items()):
            if symbol in current_prices_dict:
                ...
                # 跌破成本价8%，强制平仓
                if loss_pct <= -CONFIG['stop_loss']['individual']:
                    shares = pos['shares']
                    ...
                    stop_loss_trades.append({
                        ...
                        'shares': shares,
                        'price': sell_price,
                        'cost': -shares * sell_price * CONFIG['commission_rate'],
                        'reason': f'stop_loss_{loss_pct:.2%}'
                    })
        ...
        for trade in stop_loss_trades:
            cash = execute_trade(portfolio, trade, cash)
            trade_records.append(trade)
            print(f"  [止损] {trade['symbol']}: {trade['reason']}")
        ...
        # 组合止损检查
        drawdown = (total_value - capital) / capital
        portfolio_stopped = drawdown <= -CONFIG['stop_loss']['portfolio']

        if portfolio_stopped and len(portfolio) > 0:
            print(f"  [组合止损] 回撤{drawdown:.2%}，清空所有持仓")
            # 清空所有持仓
            for symbol, pos in list(portfolio.items()):
                if symbol in current_prices_dict:
                    shares = pos['shares']
                    sell_price = current_prices_dict[symbol] * (1 - CONFIG['slippage_rate'])
