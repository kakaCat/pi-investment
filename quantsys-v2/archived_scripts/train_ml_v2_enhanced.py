...
def prepare_features(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    logger.info(f"特征准备完成: {len(feature_cols)} 个特征")
...
def bayesian_optimize(df: pd.DataFrame, feature_cols: List[str], n_trials=30) -> Dict:
    """贝叶斯优化（模拟版 - 使用智能采样）"""
    logger.info(f"开始参数优化 ({n_trials}次试验，智能采样)...")
...
def train_final_model(df: pd.DataFrame, feature_cols: List[str], params: Dict) -> Tuple:
    ...
    if ic_mean > 0.04 and ir > 1.5:
        ...
        logger.info("❌ IC和IR均未达标，但有改进")

    return model, metrics


def save_model(model, metrics: Dict, feature_cols: List[str]):
    """保存模型"""
    ...
    model_path = model_dir / f'xgboost_v2_enhanced_{timestamp}.pkl'
    import joblib
    joblib.dump(model, model_path)

    if metrics['ic_mean'] > 0.04 and metrics['ir'] > 1.5:
        latest_path = model_dir / 'xgboost_v2_enhanced_latest.pkl'
    ...
    report_path = model_dir / f'training_report_v2_enhanced_{timestamp}.json'
...
def main():
    """主函数"""
    logger.info("="*60)
    logger.info("量化机器学习优化版 V2 - 目标: IC>0.04, IR>1.5")
    logger.info("="*60)
    ...
    # 8. 训练最终模型
    model, metrics = train_final_model(data_df, feature_cols, best_params)
