...
将V6 XGBoost模型的预测能力转化为实际交易策略，进行完整回测。
...
def backtest_strategy(test_df, model, valid_factors):
    ...
    for i, current_date in enumerate(dates):
        ...
        if i % rebalance_days == 0:
            ...
            # 选择Top 20
            top_stocks = latest_data.nlargest(CONFIG['top_n'], 'predicted_excess_return')
            ...
            trades = rebalance_portfolio(
                portfolio, target_symbols, target_weight,
                total_value, current_prices, current_date
            )
...
def rebalance_portfolio(portfolio, target_symbols, target_weight, total_value, current_prices, current_date):
    """
    计算调仓所需的交易
    ...
    for symbol in target_symbols:
        ...
        if shares_diff > 0:
            trades.append({
                ...
                'shares': shares_diff,
                'price': buy_price,
                'cost': shares_diff * buy_price * (1 + CONFIG['commission_rate'])
            })
