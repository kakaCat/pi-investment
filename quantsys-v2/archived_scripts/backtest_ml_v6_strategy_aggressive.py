...
CONFIG = {
    ...
    # 改进5：市场环境识别（激进版）
    'market_regime': {
        'lookback_days': 60,       # 回看60日
        'bear_threshold': -0.08,    # 熊市：跌超8%（更敏感）
        'bull_threshold': 0.12,     # 牛市：涨超12%
        'bear_position': 0.1,       # 熊市持仓：10%（极度保守）
        'normal_position': 1.0,     # 震荡市持仓：100%
        'bull_position': 1.2,       # 牛市持仓：120%（激进加仓）
    },

    # 改进6：止盈机制
    'take_profit': {
        'threshold': 0.15,          # 收益超过15%触发
        'lock_ratio': 0.5,          # 锁定50%利润（减仓到50%）
    },
...
def identify_market_regime(df, current_date):
    ...
    market_return_60d = (recent_prices.iloc[-1] / recent_prices.iloc[0] - 1)

    # 判断市场环境（激进版：更敏感的阈值）
    if market_return_60d < CONFIG['market_regime']['bear_threshold']:
        regime = 'bear'
    elif market_return_60d > CONFIG['market_regime']['bull_threshold']:
        regime = 'bull'
    else:
        regime = 'normal'

    return regime, market_return_60d
...
def backtest_strategy(test_df, model, valid_factors):
    ...
    for i, current_date in enumerate(dates):
        ...
        if i % rebalance_days == 0:
            # 改进5：识别市场环境
            market_regime, market_return_60d = identify_market_regime(test_df, current_date)
            ...
            # 改进6：止盈机制
            take_profit_triggered = current_return > CONFIG['take_profit']['threshold']

            # 根据市场环境和止盈状态调整目标仓位
            if take_profit_triggered:
                # 止盈：锁定利润，减仓到50%
                position_scale = CONFIG['take_profit']['lock_ratio']
                position_reason = f"止盈({current_return:.1%})"
            elif market_regime == 'bear':
                # 熊市：激进减仓到10%
                position_scale = CONFIG['market_regime']['bear_position']
                position_reason = "熊市防守"
            elif market_regime == 'bull':
                # 牛市：激进加仓到120%
                position_scale = CONFIG['market_regime']['bull_position']
                position_reason = "牛市进攻"
            else:
                # 震荡市：正常100%
                position_scale = CONFIG['market_regime']['normal_position']
                position_reason = "震荡市常规"

            print(f"\n[{current_date.date()}] 市场环境: {market_regime.upper()} (60日收益: {market_return_60d:.2%})")
            print(f"  当前收益: {current_return:.2%}")
            ...
            for idx, row in enumerate(top_stocks.itertuples(), 1):
                symbol = row.symbol
                if idx <= 5:
                    base_weight = CONFIG['tier_weights']['top5']
                elif idx <= 10:
                    base_weight = CONFIG['tier_weights']['mid5']
                else:
                    base_weight = CONFIG['tier_weights']['low10']
