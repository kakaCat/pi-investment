...
def get_stock_valuation(symbol):
    ...
    pe = _safe_float(stock.get('pe', 0))
    pb = _safe_float(stock.get('pb', 0))
    market_cap = _safe_float(stock.get('market_cap', 0))
    name = stock.get('name', symbol)

    eps = _safe_float(stock.get('eps', 0))
...
def get_sector_stocks(sector):
    """
    行业个股筛选 - 替代旧 quant_cli screening.sector
    ...
    try:
        ...
        match = sector_frame[sector_frame['板块名称'] == sector]
        if match.empty:
            return jsonify({"success": False, "error": f"Sector not found: {sector}"}), 404

        sector_code = str(match.iloc[0]['板块代码'])
        
        stock_frame = ak.stock_board_industry_cons_em(symbol=sector_code)
        ...
        for _, row in stock_frame.iterrows():
            ...
            pe_val = _safe_float(row.get('市盈率-动态', 0))
            market_cap_val = round(_safe_float(row.get('总市值', 0), decimals=0) / 1e8, 2)

            if max_pe and pe_val > max_pe and pe_val > 0:
                continue
            ...
            stocks.append({
                ...
                'name': name_val,
                'pe': pe_val,
                'market_cap_billion': market_cap_val,
            })
