...
def get_price_action(symbol):
    ...
    参数: period (默认60天)
    封装自旧 CLI 的 analyze_price_action
    """
    ...
    try:
        ...
        result = analyze_price_action(symbol, period=period)
        if 'error' in result:
...
def get_pe_percentile(symbol):
    ...
    try:
        ...
        current_pe = stock_info.get('pe_ratio') or stock_info.get('pe')
...
def get_candlestick(symbol):
    ...
    try:
        ...
        result = analyze_candlestick(symbol)
        if 'error' in result:
            return jsonify({'success': False, 'error': result['error']}), 400
        return api_response(result)
    except ImportError as e:
...
def screen_stocks_v2():
    """多条件选股 - 替代旧 quant_cli stock.screen"""
    try:
        sys.path.insert(0, str(_V2_ROOT.parent / 'quant'))
        from pathlib import Path
        from quantsys.cli.stock_analytics import screen_stocks
        quant_root = Path(_V2_ROOT.parent / 'quant')
        criteria = {}
        for key in ['min_score', 'max_pe', 'min_roe', 'max_debt_ratio', 'limit']:
            val = request.args.get(key)
            if val is not None:
                criteria[key] = float(val) if '.' in str(val) else int(val)
        result = screen_stocks(quant_root, criteria)
        if 'error' in result:
            return jsonify({'success': False, 'error': result['error']}), 400
        return api_response(result)
    except ImportError as e:
...
def screening_quality():
    """行业质量筛选 - 替代旧 quant_cli screening.quality (by sector)"""
    try:
        sys.path.insert(0, str(_V2_ROOT.parent / 'quant'))
        ...
        result = screen_stocks_quality(sector, min_score=min_score, max_pe=max_pe, limit=limit)
        return api_response(result)
    except ImportError as e:
        return jsonify({'success': False, 'error': f'Module not available: {e}'}), 503
...
def get_cash_flow_v2(symbol):
    ...
    try:
        ...
        recent_n = request.args.get('recent_n', 8, type=int)
        result = get_cash_flow(symbol, recent_n)
        if 'error' in result:
            return jsonify({'success': False, 'error': result['error']}), 400
        return api_response(result)
    except ImportError as e:
...
def get_income_statement_v2(symbol):
    ...
    try:
        ...
        recent_n = request.args.get('recent_n', 8, type=int)
        result = get_income_statement(symbol, recent_n)
        if 'error' in result:
            return jsonify({'success': False, 'error': result['error']}), 400
        return api_response(result)
    except ImportError as e:
...
def risk_stress_test():
    """压力测试 - 替代旧 quant_cli stress.test"""
    try:
        sys.path.insert(0, str(_V2_ROOT.parent / 'quant'))
        ...
        result = stress_test(quant_root, data)
        return api_response(result)
    except ImportError as e:
        return jsonify({'success': False, 'error': f'Module not available: {e}'}), 503
