...
describe('simulationApi', () => {
  ...
  it('listAccounts 请求账户发现端点', async () => {
    apiClientMock.get.mockResolvedValue({ accounts: [], total: 0 })
  ...
  it('createAccount 提交开户参数', async () => {
    apiClientMock.post.mockResolvedValue({ account_name: 'x' })
  ...
  it('trade 提交交易到账户端点', async () => {
    apiClientMock.post.mockResolvedValue({ order_id: 1 })
