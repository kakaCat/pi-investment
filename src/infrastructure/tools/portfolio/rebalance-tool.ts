...
export const portfolioRebalanceTool: ToolDefinition = {
  ...
  label: "组合再平衡",
  description:
    "Rebalance and manage the user's portfolio stored in .pi-invest/portfolio.json. " +
    "Actions:\n" +
    ...
    try {
      ...
      if (action === "add") {
        ...
        if (market === "HK") {
          ...
          // Record trade with HK fields via CLI adapter
          try {
        ...
        // A-share: requires avg_cost
          ...
          // Record trade to trades.json via CLI adapter
          try {
            const tradeAdapter = new TradeCliAdapter();
            await tradeAdapter.add({
              symbol,
      ...
      if (action === "sell") {
        ...
        // 使用服务层的 sell 方法（不再需要 TradeService，已废弃）
        try {
