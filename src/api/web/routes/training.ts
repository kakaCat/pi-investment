...
// LEGACY: in-memory task management - now handled by Python backend
...
router.post('/start', requireOpsAuth(), async (req, res, next) => {
  try {
    ...
    // LEGACY: subprocess execution with in-memory task tracking - kept for rollback
    // const {
    //   days = 90,
    //   model = 'xgboost',
    //   cvSplits = 5,
...
router.get('/status/:taskId', async (req, res, next) => {
  try {
    // NEW: proxy to Python backend
    const data = await client.get(`/api/training/status/${req.params.taskId}`);
    res.json(data);

    // LEGACY: in-memory task lookup - kept for rollback
...
router.get('/logs/:taskId', async (req, res, next) => {
  try {
    // NEW: proxy to Python backend
    const data = await client.get(`/api/training/logs/${req.params.taskId}`);
    res.json(data);

    // LEGACY: in-memory logs lookup - kept for rollback
    // const { taskId } = req.params;
    // const task = trainingTasks.get(taskId);
    //
    // if (!task) {
...
router.get('/tasks', async (req, res, next) => {
  try {
    // NEW: proxy to Python backend
    const data = await client.get('/api/training/tasks');
    res.json(data);

    // LEGACY: in-memory tasks list - kept for rollback
...
router.get('/report/:filename', async (req, res, next) => {
  try {
    ...
    // LEGACY: direct file system access with path validation - kept for rollback
...
router.get('/history', async (req, res, next) => {
  try {
    ...
    // LEGACY: direct file system access with parsing - kept for rollback
    // const modelsDir = path.join(__dirname, '../../../../quant/quantsys/ml/models');
    //
    // if (!fs.existsSync(modelsDir)) {
    //   res.json({ history: [] });
