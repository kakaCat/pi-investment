...
class KlineFetcherTests(unittest.TestCase):
    ...
    def test_run_defaults_to_all_symbols_and_continues_after_failures(self) -> None:
        ...
        self.assertEqual(result.succeeded, 54)
        self.assertEqual(result.failed, 1)
        self.assertEqual(result.failures, [{"symbol": "000002", "error": "boom"}])
    ...
    def test_update_symbol_persists_a_share_daily_klines(self) -> None:
        ...
        with patch(
            "quantsys.data.fetchers.klines.ak.stock_zh_a_hist",
            return_value=frame,
        ) as fetch_mock:
    ...
    def test_update_symbol_uses_hk_history_api_for_hk_stocks(self) -> None:
        ...
        with patch(
            "quantsys.data.fetchers.klines.ak.stock_hk_hist",
            return_value=frame,
        ) as fetch_mock:
