...
export class QuantsysV2Client {
   ...
   */
  async getPositions(accountName: string = DEFAULT_AGENT_ACCOUNT): Promise<Position[]> {
   ...
   */
  async getPortfolioSummary(accountName: string = DEFAULT_AGENT_ACCOUNT): Promise<PortfolioSummary> {
    const response = await this.client.get(`/api/simulation/accounts/${encodeURIComponent(accountName)}`);
  ...
  async executeTrade(params: TradeRequest): Promise<TradeResponse> {
    ...
    const account = params.account_name || DEFAULT_AGENT_ACCOUNT;
    const body: Record<string, any> = {
  ...
  async getCircuitBreaker(account_name?: string): Promise<any> {
    const response = await this.client.get('/api/risk/circuit-breaker', {
      params: { account_name: account_name ?? DEFAULT_AGENT_ACCOUNT },
    });
